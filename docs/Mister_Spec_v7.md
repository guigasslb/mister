...
- **Adaptar exercício para a sessão (`AdaptarExercicioDialog`):** cada linha tem um botão **"Adaptar"** (ícone `SlidersHorizontal`) que abre um diálogo para ajustar o exercício **só nesta sessão**, sem tocar na biblioteca original. Campos: **duração** (min), **séries**, **montagem** (descrição própria da sessão) e **notas** do treinador. Persiste via `atualizarExercicioSessao(sessaoExercicioId, dados)` (§7.3), validado por `sessaoExercicioOverrideSchema` (Zod: `duracaoMin`/`series`/`descricaoOverride`/`notas`), com verificação de clube e capacidade **`TREINOS_GERIR`** no escalão. Os valores gravam-se nos campos `SessaoExercicio.series`/`descricaoOverride`/`notas` (§3.5); a `duracaoMin` sobrepõe a duração-base do exercício **para esta sessão**.
- Fonte do conteúdo respeita o **snapshot** quando existe (`snapNome`/`snapDescricao`/`snapObjetivo`/`snapDiagrama` de exercício portátil usado em sessão do clube — §4.2.1); caso contrário, os campos do exercício mestre. Quando existe `descricaoOverride` na sessão, esta **prevalece** sobre a descrição da biblioteca/snapshot na apresentação do exercício desta sessão.
...
- **Atleta que entra a meio da época:** taxa de presença usa como divisor as sessões **já realizadas** do escalão desde a `dataIngresso` (sessões executadas = **`fechado = true`** — nunca as programadas ou passadas por fechar; decisão de produto 2026-08-30, BUG-P1-08).
...
**🔁 Sessões programadas vs executadas (2026-08-26; realizada = `fechado` desde 2026-08-30):** o agregado distingue **sessões programadas** (todas as criadas — `AnaliticoEscalao.sessoes`) de **sessões executadas / já realizadas** (`AnaliticoEscalao.sessoesExecutadas` = sessões com **`fechado = true`**; as programadas e as passadas por fechar ficam por executar). É subconjunto de `sessoes`, coerente com a imutabilidade do passado (§8.9.1). O `PainelEscalao` mostra o KPI **«sessões realizadas»** no formato `executadas/programadas` (ex.: `67/89`). **🔁 Denominador de assiduidade (2026-08-26, BUG-P1-08; realizada = `fechado` desde 2026-08-30):** a taxa de presença (§10.1 atleta, §10.2 escalão) usa como denominador as **sessões executadas** (`sessoesExecutadas`, i.e. `fechado = true`), **não** o total programado — caso contrário um escalão com muitas sessões futuras agendadas mostrava uma taxa artificialmente baixa (1 sessão realizada com todos presentes daria ~1% em vez de 100%). O **numerador** (presenças) é simétrico: só contam presenças em sessões fechadas, pelo que uma sessão com data passada mas por fechar não entra em nenhum dos lados (elimina a assimetria que mostrava 100% com sessões futuras/por fechar com presenças marcadas). A assiduidade do escalão é a média das assiduidades individuais (`presenças em sessões fechadas / sessões fechadas`), com cap a 100%. **🔁 v7:** o escalão tem uma modalidade fixa (a da sua secção), logo o agregado da equipa é naturalmente monomodalidade; o **núcleo estatístico apresentado** é o da modalidade (10.8). **🔁 2026-08-20:** o filtro por competição e a classificação associada consideram **só jogos `REALIZADO`** (jogos `AGENDADO` do quadro não contam — §10.9).
...
type ElementoCampo = Jogador | Bola | Cone | Baliza | Seta | Linha | Texto | Escadinha | Barras;
