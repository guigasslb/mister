...
- **Presença** — estado do atleta numa sessão (Presente, Falta, Falta justificada, Lesionado, Atrasado) e, nos estados de ausência, **tipo de ausência** (Lesão/Doença/Motivo pessoal/Trabalho ou escola/Sem motivo/Outro) com **nota** livre opcional. _(O antigo par `motivo`/`justificacao` foi unificado no `tipoAusencia`/`notaAusencia` em 2026-09-19 — ver §19.)_
...
model Atleta {
  ...
  observacoes         String?
  fotoUrl             String?   // upload de ficheiro (Supabase Storage, WebP 256×256) OU URL externo colado — §8.5
  ...
  // Contactos e dados pessoais adicionais (todos opcionais)
  email               String?
  telefone            String?
  peDominante         PeDominante?   // DIREITO | ESQUERDO | AMBIDEXTRO
  paisNascimento      String?
  nacionalidade       String?
  // Documento de identificação do atleta
  docTipo             DocTipo?       // NIC | PASSAPORTE | AR | CR | TR
  docNumero           String?
  docValidade         DateTime?
  // FPF (Federação Portuguesa de Futebol)
  estatutoFPF         EstatutoFPF?   // PORTUGUES | ESTRANGEIRO | UNIAO_EUROPEIA | IGUALDADE
  numeroLicencaFPF    String?
  ...
  encarregadoDocTipo     DocTipo?
  encarregadoDocNumero   String?
  encarregadoDocValidade DateTime?
  criadoEm            DateTime  @default(now())
  atualizadoEm        DateTime  @updatedAt
...
// Dados pessoais adicionais do atleta (§3.2).
enum PeDominante { DIREITO ESQUERDO AMBIDEXTRO }
// NIC = Cartão de Cidadão · AR = Autorização de Residência · CR = Certidão de Registo (UE) · TR = Título de Residência
enum DocTipo { NIC PASSAPORTE AR CR TR }
// Estatuto do atleta perante a FPF (Federação Portuguesa de Futebol)
enum EstatutoFPF { PORTUGUES ESTRANGEIRO UNIAO_EUROPEIA IGUALDADE }
...
model Presenca {
  ...
  tipoAusencia TipoAusencia?   // só preenchido em estados de ausência
  notaAusencia String?         // nota livre opcional (≤200 chars, validado na app)

  @@unique([sessaoId, atletaId])
}

enum EstadoPresenca { PRESENTE FALTA FALTA_JUSTIFICADA LESIONADO ATRASADO }
enum TipoAusencia { LESAO DOENCA PESSOAL TRABALHO SEM_MOTIVO OUTRO }
```
...
model Jogo {
  ...
  numeroPartes          Int       @default(2) // 🔁 v7 (§8.25.8): 1..4 partes, definido na criação/edição; o Modo Jogo ao Vivo herda-o ao iniciar
  local                 String?
  golosMarcados         Int?
  golosSofridos         Int?
...
model Convocatoria {
  ...
  capitao         Boolean  @default(false) // capitão de equipa; no máx. 1 por jogo (garantido pela action definirCapitao, não por constraint)
...
model EstatisticaAtleta {
  ...
  minutos         Int?        // total de minutos jogados; = soma de minutosPorParte quando este vem preenchido (derivável do bloco no legado)
  // 🔁 2026-09-20 — Editor de tempo por parte (§8.11/§10.4): minutos absolutos por parte
  // (índice 0 = Parte 1, ..., comprimento = Jogo.numeroPartes, 1..4). O total (`minutos`) é
  // a SOMA deste array quando preenchido. `[]` = não registado por parte (retrocompatível:
  // jogos legados só têm o total por bloco).
  minutosPorParte Int[]      @default([])
...
model EventoJogo {
  id                 String        @id @default(cuid())
  jogoId             String
  parte              Int           // 1..4 (registo clássico usa 1|2; Modo Jogo ao Vivo suporta 2..4 partes — §8.25)
  minuto             Int?          // registo clássico (minuto do jogo)
  tipo               TipoEventoJogo
  bloco              BlocoTempo?   // registo clássico por blocos (base legada dos minutos)
  atletaId           String?       // protagonista
  atletaSecundarioId String?       // assistência / substituído
  criadoEm           DateTime      @default(now())

  // 🔁 v7 (§8.25.8): Modo Jogo ao Vivo — colunas aditivas, nullable, retrocompatíveis.
  segundoJogo    Int?             // segundo ABSOLUTO do cronómetro contínuo (0 = apito inicial da Parte 1); base do cálculo de minutos ao vivo
  posicao        Posicao?         // posição ocupada numa ENTRADA (vista de campo)
  clientEventoId String?          // id gerado no cliente para sync idempotente da outbox offline

  @@unique([jogoId, clientEventoId], map: "EventoJogo_clientEventoId_key")
  @@index([jogoId])
  @@index([atletaId])
  @@index([jogoId, segundoJogo]) // leitura ordenada do registo ao vivo
}

// 🔁 v7: tipos de evento comuns + de futebol + primitivas do Modo Jogo ao Vivo. Os tipos
//   futsal-específicos e futebol-específicos coexistem; a UI de registo ao vivo mostra o
//   subconjunto relevante à modalidade do jogo. As primitivas ao vivo (INICIO_PARTE…RETOMA)
//   sustentam o cálculo de minutos por intervalos ao segundo (§10.4, §8.25); SUBSTITUICAO
//   mantém-se para o registo clássico (uma substituição ao vivo materializa-se como
//   SAIDA + ENTRADA no mesmo segundo).
enum TipoEventoJogo {
  ...
  INICIO_PARTE FIM_PARTE ENTRADA SAIDA PAUSA RETOMA
}
...
model MetricaConfig {
  ...
  clubeId String
  nome    String
  tipo    TipoMetrica @default(NUMERO) // NUMERO | BOOLEANO | ESCALA | ESCALA_1_3
...
enum TipoMetrica { NUMERO BOOLEANO ESCALA ESCALA_1_3 } // ESCALA = 1–5; ESCALA_1_3 = 1–3
...
**🔁 Escolha de plano/modalidade no registo + licença `PENDENTE` (fluxo interino, ativo; 🔁 revisto Fase 3).** Desde a **Fase 3** (2026-09-11) a **escolha de plano (tier) e modalidade** faz-se no **passo único de registo** (§8.1), não no onboarding: a action `registar()` cria **atomicamente** a conta + clube + secção + época + membro Administrador e uma `Licenca` com **`estado: PENDENTE`** e o **tier escolhido** (e `ciclo`/`precoCentimos` correspondentes). Como `PENDENTE` não concede acesso, o utilizador é encaminhado (via `callbackUrl`) para o paywall `/sem-licenca`. Aí, em vez da **tabela completa de planos**, o ecrã mostra **o plano escolhido e o valor exato a transferir**, com as instruções de transferência bancária (IBAN + referência + email de comprovativo — §17.5). O **wizard de onboarding fica bloqueado enquanto a licença for `PENDENTE`** (a guarda `deveBloquearPorLicenca` deixou de isentar a rota de onboarding); só com licença **ATIVA** o `destinoOnboarding` encaminha por `onboardingConcluido` (`false` → `/onboarding`; `true` → `/dashboard`). A ativação (`PENDENTE → ATIVA`) é feita **manualmente pelo admin no backoffice** (§21.2) após receção do comprovativo.
...
> **🔁 v7 — modalidade na licença Individual (DEVE; 🔁 resolvido Fase 4, 2026-09-11):** a licença Individual regista a **modalidade contratada** em `Licenca.modalidade`, **persistida no momento da criação da licença** (dentro da transação de registo — **não** por `updateMany` pós-transação). `Licenca.modalidade` é atributo **exclusivo** do produto Individual: é a **fonte de verdade** do produto vendido e mantém-se **`null` em licenças de Clube** (a modalidade de um clube deriva das suas secções, e o pricing escala por secção via `numSeccoes` — secção 17.1).

> **🔁 v7 — titularidade da licença Individual e Carteira (DEVE; 🔁 Fase 4, 2026-09-11):** a licença Individual é titulada **exclusivamente por `utilizadorId`** (nunca por `clubeId`, mesmo existindo um clube técnico invisível em contexto — §3.1). A `Carteira` da pessoa é criada no **arranque do modo Individual**, de forma **idempotente** (`upsert` por `utilizadorId`). A resolução da licença a jusante — `temLicencaValida`, `obterLicencaPendente` e `obterLicenca` (leitura da licença **ATIVA** em Definições · Licença) — resolve o **titular por `clubeId` OU por `utilizadorId`**, derivando ambos **exclusivamente da sessão autenticada** (defesa contra IDOR).

> **🔁 Estados da licença (`EstadoLicenca`):**
> - **`PENDENTE`** — licença **criada aquando do registo** (com o tier escolhido no registo (§8.1)), **aguarda confirmação de pagamento**. **Não concede acesso** (a guarda trata-a como sem licença). É o estado inicial no fluxo interino por transferência bancária (§17.5); transita para `ATIVA` quando o admin confirma o comprovativo (§21.2).
...
- **Consentimento parental** (`Consentimento`, `DADOS`/`IMAGEM`): por atleta (cobre todas as modalidades). 🔁 **2026-09-10:** a **fotografia** do atleta é **dado pessoal de imagem de menor**; o *gate* do upload por consentimento `IMAGEM` é **follow-up** (o upload já está disponível, a recolha de consentimento continua a ser tratada pelo clube fora da app — ver §8.5).
- **Direito ao esquecimento:** soft-delete por defeito; hard-delete a pedido (estatísticas anonimizáveis) — apaga participações em **todas** as secções. 🔁 **2026-09-10:** o hard-delete (`apagarAtletaDefinitivamente`) também **apaga a fotografia do Supabase Storage** (quando o `fotoUrl` é nosso), best-effort — uma falha do Storage não bloqueia o apagamento.
...
- **Upload de ficheiros (fotos de atletas — §8.5):** a `SUPABASE_SERVICE_ROLE_KEY` é **server-only** (nunca `NEXT_PUBLIC_*`); o cliente do Storage vive em `lib/supabase-storage.ts` protegido por `import "server-only"`. O upload **valida os magic bytes** (não a extensão/content-type declarado) — só JPEG/PNG/WebP — e o **tamanho** (≤10 MB antes de compressão), re-encoda com `sharp` para **WebP 256×256** e guarda no bucket **público** `atletas` com **path UUID** não-adivinhável. Substituir uma foto **apaga o ficheiro anterior** do Storage.
...
```
> **🔁 Defesa em profundidade — clube técnico Individual (DEVE; Fase 4, 2026-09-11):** num clube técnico (`Clube.clubeTecnico === true`) o servidor **recusa** o convite de membros (`convidarMembro` valida `!clube.clubeTecnico`) e **bloqueia** a criação de uma 2ª modalidade (`garantirSeccaoParaModalidade` — ver `criarEscalao` abaixo). A ocultação destes fluxos na UI (§8.1, §8.4, §12) é **apenas cosmética**; a autorização real está no servidor.
...
uploadFotoAtleta(atletaId, formData) // PLANTEL_GERIR; valida magic bytes + tamanho, WebP 256×256 → bucket `atletas`, devolve {fotoUrl} (§8.5)
...
definirCapitao(jogoId, atletaId|null)        // CONVOCATORIA_GERIR — capitão de equipa (máx. 1/jogo; null limpa)
...
Cada módulo define **conteúdo**, **ações**, **estado vazio** e **regras**. Estados loading/erro seguem a secção 13. Navegação: barra de topo (logótipo do clube + **seletor de secção quando >1** + seletor de época + menu do utilizador) + sidebar (PC) / bottom-nav (móvel). **A sidebar tem dois estados — expandida (ícone + rótulo, 224px) e colapsada (só ícones, 64px)** com um botão de alternância no topo; a preferência do utilizador é persistida (`localStorage`). **Por defeito arranca colapsada em tablet (md–xl, ~768–1279px, incl. iPad em paisagem) e expandida em desktop (≥xl, ≥1280px)**; em móvel (<md) usa-se a bottom-nav. **A bottom-nav (móvel) expõe os itens primários — Início · Plantel · Agenda · Jogos — sendo os restantes acedidos pelo menu "Mais"** (🔁 2026-08-26b — a **Agenda substitui** o antigo item separado de **Treinos** no menu, redundante com a vista central de eventos; **Treinos deixa de figurar como item de navegação** — a rota `/treinos` mantém-se como **vista de gestão**, acessível a partir da própria Agenda, §8.13.1. **🔁 2026-09-13 — «Jogos» (`/jogos`) volta a ser item de navegação**, a seguir à Agenda e antes de Exercícios, com ícone `Shield`; passa também a integrar os 4 primários da bottom-nav móvel, remetendo **Exercícios** para o menu «Mais». O item «Jogos» **não** fica ativo em `/jogos/competicoes` (item próprio «Competições», via `excluir`).). **No modo Individual (clube técnico, `clube.clubeTecnico === true`), as entradas de gestão de clube — Clube/Branding, Secções, Equipa técnica e Perfis — são ocultadas na *página de Definições* (via flag `ocultarIndividual`, derivada de `clubeTecnico`); são sub-entradas de Definições (§8.4), não itens da navegação de topo, que se mantém inalterada.**

### 8.1 Onboarding e contas
> **Princípio (decisão 2026-08-05; 🔁 revisto Fase 3, 2026-09-11):** o **registo consolidou-se num passo único** que recolhe o essencial **mais a escolha de plano e modalidade** — **sem processar pagamento** (este continua à parte, no paywall). O **setup fino do clube** (branding, escalões) mantém-se para o **onboarding guiado pós-primeiro-login**, que só desbloqueia com licença **ATIVA**.
- **Login** (`/login`): email + password. Erros inline; toast em falha.
- **Registo — passo único (🔁 Fase 3):** o formulário (`registarSchema`, unificado) recolhe **`nome, email, password, nomeClube, tier` (`INDIVIDUAL | PEQUENO | MEDIO | GRANDE`) e `modalidade` (`FUTSAL | FUTEBOL`)**. A `modalidade` é **escolha obrigatória** (deixou de haver default silencioso "FUTSAL"). A action **`registar()`** cria **atomicamente, numa transação**: `Utilizador` + `Clube` + **Secção** da modalidade + **época ativa** + **perfis de arranque** + **membro Administrador** (o criador) + **`Licenca` PENDENTE** (com o tier escolhido). Após o registo o utilizador **autentica-se** e o `callbackUrl` encaminha-o para o **paywall `/sem-licenca`**.
  - ❌ **Não** se recolhem no registo: logótipo, cores, escalões (ficam para o onboarding).
  - **Fallback autenticado:** `/criar-clube` (+ `criarClubeSchema`) mantém-se para criação de clube já autenticado, com `default("FUTSAL")` por retrocompatibilidade.
  - **🔁 Ramo Individual (Fase 4, 2026-09-11):** quando `tier=INDIVIDUAL`, o ramo Individual da transação de `registar()`/`criarClube()` produz **atomicamente** — **`Clube.clubeTecnico=true`** + **`Licenca` titulada por `utilizadorId`** (com a `modalidade` contratada persistida, §3.11) + **`Carteira`** (idempotente por `upsert`) + **uma única `Secção`** da modalidade escolhida. `clube.clubeTecnico === true` é o **single source of truth** do modo Individual (§3.1, §5.2): clube invisível/pessoal, de utilizador único.
- **Acesso ao onboarding condicionado à licença (🔁 Fase 3):** o **wizard de onboarding fica bloqueado enquanto a licença for `PENDENTE`** — só fica acessível após ativação (licença **ATIVA**). Com licença válida, o encaminhamento (`destinoOnboarding`) resolve-se por `onboardingConcluido`: **`false` → `/onboarding`**, **`true` → `/dashboard`**. **Nota de arquitetura:** `/sem-licenca` e `/criar-clube` vivem **fora** do grupo `(app)`, logo **não** passam pela guarda de licença — evita ciclos de redirect por construção.
...
> **🔁 v7 — Painel de Definições em 4 grupos (DEVE; 2026-09-12):** a página `/definicoes` organiza os cartões em **quatro grupos rotulados** (cabeçalho discreto por grupo: texto pequeno, maiúsculas, cor *muted*), em vez de uma grelha plana:
> - **Clube** — Marca (`/definicoes/clube`), Secções, Escalões, Épocas.
> - **Catálogos** — Métricas, Tipos de exercício (`/definicoes/subcategorias`).
> - **Pessoas e acessos** — Equipa técnica (`/definicoes/utilizadores`), Perfis e permissões (`/definicoes/perfis`).
> - **Conta e sistema** — Licença, Integrações.
>
> **Rótulos (renomeações):** «Clube» → **Marca**, «Subcategorias» → **Tipos de exercício**, «Perfis» → **Perfis e permissões** (as rotas mantêm-se inalteradas). Um grupo cujos cartões estejam **todos ocultos** (gating por capacidade / `ocultarIndividual`) **não é renderizado** — sem cabeçalhos vazios. O gating por cartão mantém-se intacto (só muda o agrupamento visual). Todas as **sub-páginas** têm um **breadcrumb consistente** «‹ Definições» (componente `CabecalhoDefinicoes`).
...
- **Tipos de exercício** (rótulo do painel; rota `/definicoes/subcategorias`, `SubcategoriaExercicio`): CRUD (seed instala predefinidas).
- **Templates de comunicação** (`COMUNICACOES_GERIR`): ver/editar variantes.

> **🔁 Modo Individual — ocultação na página de Definições (DEVE; Fase 4, 2026-09-11):** num clube técnico (`clube.clubeTecnico === true`) a **página de Definições** oculta as entradas de gestão de clube — **Marca (Branding), Secções, Equipa técnica e Perfis e permissões** — via flag **`ocultarIndividual`** (derivada de `clubeTecnico`). **Permanecem visíveis** ao Individual: **Escalões, Épocas, Métricas, Tipos de exercício, Licença e Integrações**. Como o grupo **Pessoas e acessos** fica sem cartões visíveis no modo Individual, o seu cabeçalho **não é renderizado**. A navegação de topo não muda (estes módulos são sub-entradas de Definições, não itens de nav — §8). A ocultação é **cosmética**; os guards de servidor (§7.3) são a defesa real (recusa de convite de membros e de 2ª modalidade em clube técnico).
...
- **Estado no plantel (`ativo`) 🔁 novo 2026-08-20:** o atleta tem um campo booleano **`ativo`** (default `true`) que distingue quem está **no plantel** de quem **saiu** ou ainda está em **período experimental** (nos primeiros treinos aparecem atletas a experimentar — são criados e as suas presenças registadas antes de se saber se ficam). A **lista do plantel só mostra atletas ativos por defeito** (`listarAtletas` filtra `ativo:true`); um parâmetro opcional **`incluirInativos`** mostra todos. A ação **`toggleAtivoAtleta`** alterna o estado (verifica `PLANTEL_GERIR` num escalão do atleta); ao contrário de `apagarAtleta` (que força `ativo:false`), faz toggle — reativa um inativo ou desativa um ativo. A **edição de dados pessoais não altera `ativo`** (só as ações dedicadas o fazem). **Na UI:** a lista do plantel expõe o toggle *«Mostrar atletas inativos»* (`FiltroInativos`, estado em `?incluirInativos=1`, preservando os restantes filtros) e apresenta os atletas inativos esbatidos (`opacity-60`) com badge **«Inativo»**; o **formulário de edição do atleta** expõe o switch de estado (`ToggleAtivoAtleta`, visível só com `PLANTEL_GERIR`, atualização otimista + `toast`) 🔁 **(2026-09-04: migrado da antiga aba «Dados» do perfil)**. As **queries de histórico** (estatísticas, presenças e convocatórias passadas em `obterEstatisticasAtleta`) **não** filtram por `ativo` — um atleta inativo mantém o histórico válido.
- **Apagar:** soft-delete («Arquivar atleta») e hard-delete («Apagar definitivamente», zona de perigo só por RGPD — 5.5, remove participações em todas as secções) ficam ambos no **formulário de edição do atleta**; o hard-delete só é visível com `PLANTEL_GERIR`.
...
- **Novo/Editar:** **dois modos** — *Treino avulso* (sessão única, fluxo atual) ou *Horário de treinos* (§8.8.1). No modo avulso: data/hora, escalão, duração, objetivo, local, notas, ligação a semana (opcional), criar a partir de template, **modalidade da actividade** (opcional) 🔁.
...
#### 8.8.1 Horário de treinos (`TREINOS_GERIR`) — 🔁 novo 2026-08-20
Dois modos de criação de treinos. **Modo 1 — Treino avulso:** sessão única (§8.8), inalterado. **Modo 2 — Horário de treinos:** agenda o horário recorrente do escalão e o sistema gera automaticamente as sessões da época.

> **Nota de terminologia (🔁 2026-09-18):** na UI este modo chama-se **«Horário de treinos»** (antes «Plano semanal»), para quebrar a colisão de nomes com a **Periodização/Planeamento** (§8.9). A **entidade técnica mantém-se** `PlanoSemanal`/`PlanoSemanalDia`, tal como as Server Actions (`criarPlanoSemanal`, `atualizarPlanoSemanal`, `apagarPlanoSemanal`, …), os schemas Zod, as rotas (`/treinos/planos`, `/treinos/novo?modo=plano`) e os campos `Sessao.planoSemanalId`/`planoSemanalDiaId` — só os **labels visíveis** ao utilizador mudaram.

- **Rotas:** toggle "Treino avulso | Horário de treinos" em **`/treinos/novo`**; gestão dos horários em **`/treinos/planos`** (lista por escalão).
- **Criar horário:** escolher escalão + selecionar os **dias da semana** com treino; por **cada dia** definir **hora de início**, **hora de fim**, **local** e **tipo de sessão** (`TipoSessao`, default `NORMAL`). Nome do horário opcional (fallback = nome do escalão).
...
**Gerir o horário (`/treinos/planos`):**
...
- **Apagar horário:** modo **Desvincular** (`planoSemanalId = null`, mantém sessões) ou **Apagar futuras vazias** (apaga futuras sem conteúdo e desvincula as restantes). **Nunca apaga sessões com conteúdo em silêncio.**
- **No máximo um horário `ativo` por (escalão, época).** Escalões diferentes têm horários diferentes.
...
**0. Impressão / PDF do plano de treino.** Para levar o treino impresso quando não há tablet, o botão **"Exportar PDF"** do detalhe abre numa **nova aba** a rota **`/treinos/[id]/print`** — uma página **fora do grupo `(app)`** (sem sidebar nem barra de topo), otimizada para `@media print`. Por viver fora do `(app)`, a página **força o modo claro no ecrã** (`ForcarModoClaroImpressao` acrescenta `light`/remove `dark` no `<html>` enquanto está montada, **sem** persistir a preferência do utilizador em `localStorage`) — a pré-visualização sai sempre em **fundo branco/texto escuro**, tal como a impressão em papel (o bloco `@media print` do `globals.css` já força claro para poupar tinta); e **injeta a cor do clube** (`--cor-primaria`, a partir de `Clube.corPrimaria`) no contentor da página, para os **diagramas de campo** (`MiniaturaCampo`/`CampoDesenho`) usarem a cor do clube — sem isto caíam no **laranja da marca** (fallback de `var(--cor-primaria, #F0531E)`), por a rota não herdar o `--cor-primaria` do layout `(app)`. Verifica a sessão via `auth()` (redirect para `/login` se não houver sessão) e reforça a autorização por escalão/clube ao ler a sessão com `obterSessao` (§7.3). O template (`TreinoPrintTemplate`, *server component* — o diagrama SVG sai fiel **sem depender de JavaScript**) apresenta: **cabeçalho** com o logótipo **Mister** (produto) e o **nome do clube + época** no contexto da página (§BRAND — o logótipo do clube não faz lockup ao lado do produto), acrescido (**quando disponível**) da **nomenclatura técnica federativa** da sessão — **Microciclo** (a UI mostra "Semana"), **Mesociclo**, **Período** (`PeriodoEpoca`) e **Momento** (MD-X, `MomentoSemana`) — e do **nº de presentes/registados** na sessão (presenças marcadas / atletas elegíveis); **resumo** (duração total — `Sessao.duracaoMin` ou o somatório dos exercícios —, nº de exercícios, tipo de sessão `TipoSessao`, local, objetivo e notas da sessão); **lista de exercícios em sequência numerada** — **ordenada por fase do treino** (`ParteTreino`, ordem canónica: Aquecimento → Parte principal → Jogo → Retorno à calma; sem fase no fim) e, dentro de cada fase, pela `ordem` da tabela de junção (`SessaoExercicio.ordem`), espelhando o agrupamento da visualização no ecrã (`GestorExercicios`) —, cada um com número de ordem, nome, metadados (duração, fase `ParteTreino`, categoria `CategoriaExercicioPrincipal`, séries e, **quando não nulos**, **nº de jogadores** 🔁 e **espaço** 🔁 como badges — respeitando os overrides da sessão e o snapshot), **miniatura do diagrama** (`MiniaturaCampo`, quando existe), **objetivo**, **descrição** (respeitando `descricaoOverride` e o **snapshot** §4.2.1) e notas; e **rodapé** com o clube e a data de impressão. Os cabeçalhos de secção usam o **laranja Mister** (`#F0531E`). Cada exercício usa `break-inside-avoid` para **não partir a meio** entre páginas. A barra de ações (link "Voltar" + botão "Imprimir / Guardar PDF", `BotaoImprimir`) tem `print:hidden`/`data-print-hidden`; ao abrir, o `AutoImprimir` dispara o diálogo de impressão do browser (**"Guardar como PDF"**) após um curto atraso para as fontes/SVG estarem pintados. Abordagem **serverless-safe** (sem motor nativo/WASM), coerente com o export dos analíticos (§8.15).
...
- **Tipo de ausência (`Presenca.tipoAusencia`, enum `TipoAusencia`; `notaAusencia` livre opcional ≤200 chars) — fonte única desde 2026-09-19:** classificação **única e estruturada** da ausência que surge **contextualmente** em **todos** os estados de não-comparência (`FALTA`/`FALTA_JUSTIFICADA`/`LESIONADO` — helper `estadoImplicaAusencia`, `lib/schemas/treino.ts`), com botões rápidos **Lesão · Doença · Motivo pessoal · Trabalho / escola · Sem motivo · Outro** (`LABEL_TIPO_AUSENCIA`) e uma **nota livre opcional**. Os campos **limpam-se automaticamente** quando o atleta passa a presente/atrasado, tanto na UI (`MarcadorPresencas`) como no servidor (`marcarPresencas` grava `null` sempre que `estadoImplicaAusencia` é falso) e no schema (`presencaSchema.superRefine` rejeita valores incoerentes vindos do cliente). **Decisão de produto 2026-09-19:** o antigo par `motivo`/`justificacao` (enum `MotivoFalta`) foi **eliminado** — o `tipoAusencia`/`notaAusencia` é agora o **único** mecanismo de classificação da ausência (o marcador deixa de mostrar dois seletores). Ver §19.
...
- **Guardar só com alterações pendentes (DEVE):** o botão **"Guardar presenças"** compara o estado atual com o estado inicial carregado do servidor (helper puro `presencasAlteradas` em `lib/presencas.ts`, que compara estado, `tipoAusencia` e `notaAusencia` — o campo de texto normalizado, `null`/`""`/espaços equivalem) e **fica desativado quando não há nada por guardar** (rótulo "Sem alterações"). Guarda também no servidor (`guardar` só submete se houver alterações).
- **Repor = limpar presenças (DEVE):** o botão **"Repor"** repõe **todos** os atletas a **"por marcar"** (estado `null`) — serve para desfazer marcações feitas por engano, **inclusive as já guardadas**. Fica **desativado apenas quando não há nenhuma presença marcada** (nada a limpar); quando existem presenças (mesmo já persistidas), fica **ativo**. A limpeza é **local**; para a **persistir**, o utilizador confirma em **"Guardar presenças"** (que, após "Repor", fica ativo por o estado diferir do servidor). No servidor, `marcarPresencas` interpreta um registo com `estado: null` como **remoção** (`deleteMany` idempotente por `sessaoId`+`atletaId`); registos com estado válido continuam a fazer upsert em lote. O schema `presencaSchema` (`lib/schemas/treino.ts`) aceita `estado` **nullable** para exprimir a limpeza.
- **Sessão fechada é só-leitura (DEVE):** quando a sessão está **fechada** (`Sessao.fechado = true`, §8.8 — fechar/reabrir), a marcação de presenças entra em **modo só-leitura** — os segmentos de estado, os botões de tipo de ausência e o campo de nota livre ficam **desativados**, os atalhos "Marcar todos presentes"/"Repor" e a barra "Guardar presenças" ficam **ocultos**, e surge um **badge "Sessão concluída · só leitura"** (ícone `Lock`) com a indicação de reabrir a sessão para editar. **Reforço no servidor:** `marcarPresencas` (`lib/actions/treinos.ts`) rejeita a escrita numa sessão fechada (devolve `Resultado` de erro) — defesa em profundidade além da UI. **Reabrir** a sessão (botão dedicado no cabeçalho do detalhe) volta a permitir editar.
...
**Fechar a sessão no próprio dia (DEVE).** A disponibilidade do botão **"Fechar sessão"** (§8.8 — fechar/reabrir) é **independente** do estado visual "concluído": o treinador pode **fechar a sessão no próprio dia** em que ela ocorre (assim que tiver marcado presenças e validado tudo), não só a partir do dia seguinte. O gate usa o helper puro `treinoFechavel(data, agora?)` em `lib/semana` — **verdadeiro quando o dia do treino é hoje ou anterior** (`inicioDoDia(data) <= inicioDoDia(hoje)`) — distinto de `treinoConcluido` (estritamente passado), que continua a governar apenas o **tratamento visual** (badge "Concluído", CTA "Ver treino", confirmação ao editar). **Treinos futuros** não podem ser fechados. O bloco de fecho no detalhe (`FecharSessaoButton`) aparece sob esta condição; a ação de fecho/reabertura em si (`fecharSessao`/`reabrirSessao`) não impõe restrição de data no servidor.

**Casos-limite:** sessão **sem exercícios** → estado vazio na lista e **"Iniciar treino" desativado** (com dica "Adiciona exercícios para conduzir o treino"); num treino concluído sem exercícios o botão "Ver treino" fica igualmente desativado; sessão **sem atletas elegíveis** → estado vazio nas presenças; a condução respeita a **ordem canónica das fases** (§8.8.2 ponto 3) sobre a `ordem` persistida, mesmo que a lista tenha sido reordenada momentos antes.
...
- **Quadro tático do "Plano de jogo" (interativo):** o separador *Convocatória → Plano de jogo* (§8.11) inclui um **quadro tático interativo** que reutiliza o editor de campo. **Semeia-se** com a formação prevista (titulares posicionados por linha) e permite **arrastar** os tokens dos titulares, **desenhar setas/jogadas** e **adicionar tokens genéricos do adversário** (equipa `adversario` — token neutro/escuro tracejado com rótulo «A», distinto dos jogadores próprios coloridos/numerados). É um **único quadro por jogo**, identificado pelo nome canónico `NOME_QUADRO_PLANO_JOGO` («Plano de jogo»), do tipo `GERAL`, persistido em `QuadroTatico.diagrama` (upsert por nome). **Enquanto não houver quadro gravado**, o campo **acompanha a formação viva** dos titulares — ao marcar/posicionar titulares na tab, os tokens aparecem/atualizam-se de imediato (por defeito); **depois de gravado**, o quadro passa a ser **independente** da formação (mostra o diagrama guardado). O botão **"Repor formação"** volta a semeá-lo com a formação atual. **Exceção — nome do atleta (identidade):** o nome de cada titular (`nomeAtleta`, letra pequena por cima da figura) é uma sobreposição de **identidade** (que atleta é o token), independente da posição, pelo que é **sempre** injetado nos tokens de jogador — inclusive num quadro **já gravado** — emparelhando por `id` de token via `sobreporNomesTitulares` (`lib/formacao.ts`); sem isto o nome só apareceria enquanto o quadro nunca tivesse sido guardado. As posições gravadas não são alteradas. **Capitão de equipa (identidade):** um dos titulares pode ser designado **capitão** (no máximo **um por jogo**) — na lista do plano, cada titular tem um botão de braçadeira **«C»** (só titulares; um suplente nunca pode ser capitão); clicar num titular sem capitão passa-lhe a braçadeira (removendo-a do anterior), clicar no capitão atual remove-a. A designação persiste de imediato via `definirCapitao(jogoId, atletaId|null)` sob `CONVOCATORIA_GERIR` (a action limpa o `capitao` de todos os convocados do jogo e marca o indicado — 0 ou 1) e é independente do «Guardar plano». No campo, o capitão exibe uma **braçadeira «C»** (disco âmbar) junto à figura; tal como o nome, é sobreposta por identidade (via `sobreporNomesTitulares`) inclusive num quadro **já gravado** — incluindo a **limpeza** de um «C» gravado que já não corresponde ao capitão atual. **Edição sob `MODELO_JOGO_GERIR`**; sem essa capacidade, o quadro é **só-de-leitura** (render estático). A **posição/titularidade previstas por convocado** (que alimentam a formação semeada) continuam a ser guardadas à parte por `definirPlanoTatico` sob `CONVOCATORIA_GERIR` — os dois estados são independentes. **Limite de titulares:** o plano aceita no máximo os **jogadores em campo** do formato do jogo (**futsal = 5**; futebol usa o nº real do formato — `maxTitulares`/`JOGADORES_EM_CAMPO` em `lib/estatisticas.ts`); ao tentar marcar mais, o botão «Titular» fica **desativado** e é mostrado o aviso «Já tens N titulares selecionados».

### 8.11 Jogos, competições, estatísticas, classificação e scouting
- **Calendário/Lista** (`JOGOS_GERIR`): **agrupamento por secção quando >1** 🔁, tabs por escalão; data, adversário, Casa/Fora, resultado, competição, tipo, **formato** 🔁. **Atalho «Ver na Agenda»** (`/agenda?vista=lista&tipo=JOGO`) para a vista central unificada (§8.13.1). **🔁 2026-09-13 — a rota `/jogos` é alcançável diretamente pelo menu** (item «Jogos», §8), além do acesso pela Agenda. **Filtros da lista** (todos via `searchParams`, aplicados server-side em `listarJogos`, preservando-se entre si): por **escalão** (`?escalaoId=`, limitado aos escalões legíveis), por **modalidade** (`?modalidade=FUTSAL|FUTEBOL`, só quando o clube é multi-secção), por **tipo** (`?tipo=OFICIAL|AMIGAVEL` — «Todos · Oficial · Amigável», rótulos de `LABEL_TIPO_JOGO`) e por **estado de fecho** (`?estado=aberto|fechado` — «Todos · Por fechar · Fechados»).
...
- **Detalhe do jogo:** cabeçalho + resultado + **campo «Formato»** 🔁 (pré-preenchido pela secção, editável); **faltas acumuladas por parte só em FUTSAL** 🔁 (ocultas em futebol) + **4 separadores** (UX-P3-04 — os mais usados primeiro; sub-separadores agrupam o dia de jogo e a análise):
  ...
  - **Estatísticas** (`ESTATISTICAS_GERIR`): por atleta — utilização, tempo de jogo por blocos, **núcleo por modalidade** 🔁 (futsal: golos, assistências, e se GR defesas/sofridos/faltas; futebol: golos, assistências, **remates, cantos, foras-de-jogo, desarmes**, e se GR defesas/sofridos), **cartões (🟨 amarelo 0–5, 🟥 vermelho 0–2 — comuns às duas modalidades, a seguir às faltas)** + **métricas configuráveis**. Aviso se soma de golos ≠ resultado. Ver 10.8.
    - **🔁 2026-09-20 — Vista consolidada única (Fase C):** as **Estatísticas** são a **vista única e a verdade final** do registo do jogo. Ao **carregar** a vista (loader server-side), a grelha vem **já preenchida com os valores derivados dos eventos** (motor único `derivarEstatisticas` — minutos/utilização, golos, assistências, cartões e **secundários por atleta**: remates, cantos, faltas, desarmes, defesas, foras-de-jogo — §10.4), sem necessidade de qualquer ação manual. Onde existe **edição manual persistida** em `EstatisticaAtleta`, essa prevalece **por atleta** sobre o derivado (*last-write-wins*, §13.4); a combinação faz-se em `combinarEstatisticasIniciais` (`lib/derivar-estatisticas.ts`, função pura). Isto **substitui o antigo botão «Preencher do registo ao vivo»** (que passava a ação a manual) — a derivação é agora **automática**. `guardarEstatisticas` mantém-se como o ponto de gravação da edição manual (a verdade final persistida). O **registo clássico deixa de ser uma superfície própria** de captura de secundários: estes são registados como `EventoJogo` (registo ao vivo/Modo Jogo ao Vivo) e **editáveis na grelha por atleta**. O **timeout** (`TIMEOUT`) **não tem campo individual por atleta** — mantém-se como `EventoJogo` ao **nível do jogo/equipa** (visível na **cronologia do jogo**, §10.4), pelo que **não se perde**; simplesmente não é agregado numa coluna por atleta.
  - **~~Ao Vivo~~ (separador removido — 🔁 2026-09-20, Fase C — UI):** a tab **«Ao Vivo»** (registo clássico por eventos: golo, assistência, falta, cartão, substituição com bloco, defesa, timeout — **futsal**; + **remate, canto, fora-de-jogo, desarme** — **futebol**) **foi removida** da UI. Os eventos **continuam a existir** (`EventoJogo`) e são registados pelo **Modo Jogo ao Vivo** (§8.25); são **derivados automaticamente para a grelha de Estatísticas** (vista consolidada única, acima) e mostrados na **cronologia do jogo** (§10.4). O **CTA «Modo Jogo ao Vivo»** (§8.25 — ecrã dedicado com cronómetro contínuo por partes, substituições por *tap* e cálculo automático de minutos, offline-first) foi **reposicionado para o topo do separador Convocatória** (deixou de depender da tab removida; visível apenas com convocatória gravada e jogo não terminado — `mostrarModoAoVivo`). O antigo botão **«Preencher do registo ao vivo»** foi **removido** (a derivação passou a ser automática ao carregar a vista).
...
A **Agenda** (`/agenda`) é a **vista central de eventos** do clube, unificando num só ecrã os três tipos de evento: **treinos** (sessões), **jogos** e **reuniões**. Substitui a antiga agenda condicional (dependente de flag de visibilidade) e passa a item primário da navegação (§8). **🔁 2026-08-26b — a Agenda substitui os itens separados de Treinos e Jogos no menu:** por ser a vista de tempo que agrega esses eventos, os antigos itens de navegação de **Treinos** e **Jogos** foram removidos (eram redundantes ao mesmo nível). As rotas `/treinos` e `/jogos` mantêm-se como **vistas de gestão** (criar/editar/detalhe, horário de treinos, classificação, scouting) e são alcançadas a partir da própria Agenda — cada evento liga ao seu detalhe (`/treinos/{id}`, `/jogos/{id}`) e os botões **«Nova sessão»**/**«Novo jogo»** abrem os formulários de criação. Os atalhos **«Ver na Agenda»** em Treinos e Jogos continuam a ser o caminho de volta. **🔁 2026-09-13 — «Jogos» volta a ser item de navegação:** a rota `/jogos` (vista de gestão de jogos) regressa ao menu como item de primeiro nível — ícone `Shield`, **a seguir à Agenda e antes de Exercícios** — sidebar no desktop e um dos 4 primários da bottom-nav móvel (remetendo Exercícios para «Mais»); o acesso pela Agenda mantém-se. **Treinos** continua a **não** figurar como item de navegação (só via Agenda). O item «Jogos» exclui `/jogos/competicoes` do seu estado ativo (item próprio «Competições»). **🔁 2026-09-13 — Competições como item primário:** a rota de competições (`/jogos/competicoes`), antes acessível apenas por dentro de Jogos, passa a **item de primeiro nível da navegação** (ícone `Trophy`, entre **Mano-a-Mano** e **Analytics**) — sidebar no desktop e menu «Mais» na bottom-nav móvel, como os restantes itens não primários; a rota mantém-se inalterada.
...
- **Dois modos de vista** (via `?vista=`): **Lista** (`vista=lista`, por defeito) e **Calendário** mensal (`vista=calendario`), com toggle na própria página.
  - **Lista:** os eventos surgem em lista cronológica, cada um identificado pelo seu tipo (treino, jogo, reunião), com ligação para o respetivo detalhe. A janela da lista abrange **toda a época ativa** (de `Epoca.dataInicio` a `Epoca.dataFim`, inclusive) — **não** um limite fixo de dias — pelo que mostra todos os eventos da época, passados e futuros. As entradas **já realizadas** (`data` anterior ao início do dia de hoje, via `treinoConcluido`) surgem com **estilo amortecido** (fundo `cinza-50`, texto `cinza-500`, selo «Realizado»), distinguindo-se visualmente das futuras (estilo normal/ativo).
...
- **Ações rápidas:** botões **«Nova sessão»**, **«Novo jogo»** e **«Nova reunião»** a partir da própria Agenda. **🔁 2026-09-16 — portas de gestão de treinos na Agenda:** como o item «Treinos» não figura na navegação, a Agenda expõe as vistas de gestão que este herdaria através de um **menu overflow «⋯»** (dropdown shadcn, no cabeçalho, junto às ações rápidas) com atalhos para **«Horário de treinos»** (`/treinos/planos`, horários recorrentes que geram sessões) e **«Periodização»** (`/treinos/periodizacao`). Agrupados no overflow para não sobrecarregar o cabeçalho no mobile (iPhone SE).
- **Ligação a partir do detalhe da sessão (🔁 2026-09-16):** quando uma sessão foi gerada por um horário semanal recorrente (`Sessao.planoSemanalId != null`), o cabeçalho do detalhe (`/treinos/{id}`) mostra um badge/link contextual **«Faz parte do horário semanal · Gerir horário»** que navega para `/treinos/planos` — dando descoberta ao plano recorrente a partir de qualquer sessão que ele gerou.
...
#### 8.20.1 Métricas de treino por atleta (`TREINOS_GERIR`, `RELATORIOS_VER`) — 🔁 novo v7 (2026-09-11)

> **Estatuto:** nova funcionalidade. As **métricas configuráveis** (§3.7 / `CATALOGO_METRICAS`) deixam de ser exclusivas dos jogos e passam a poder ser registadas **também nos treinos**, por atleta e por sessão. Motivação: o treinador quer atribuir pontuações qualitativas (ex.: **empenho**, **desempenho**) em cada treino e acompanhar a evolução ao longo da época.

**Modelo de dados (DEVE):**
- `MetricaConfig` ganha o campo **`contexto`** (`ContextoMetrica` = `JOGO` | `TREINO` | `AMBOS`; default **`JOGO`** — retrocompatível: métricas existentes continuam só de jogo).
- Novo model **`ValorMetricaSessao`** (`metricaId` + `sessaoId` + `atletaId` + `valor Int`), `@@unique([metricaId, sessaoId, atletaId])`, cascade em métrica/sessão/atleta. Análogo a `ValorMetrica` (jogo), mas ligado à `Sessao`.

**Registo (DEVE):**
- Na **Definições → Métricas**, ao criar a métrica escolhe-se o **contexto** («Onde se regista»: Jogos / Treinos / Jogos e treinos).
- No **detalhe da sessão de treino** (`/treinos/[id]`) surge a grelha **«Métricas do treino»** com as métricas de contexto `TREINO`/`AMBOS` ativas, uma linha por **atleta presente** (`PRESENTE`/`ATRASADO`); guarda em lote via `guardarMetricasSessao` (upsert delete-then-create pelos atletas submetidos; valores limpos são removidos). Só métricas de treino ativas do clube e atletas do clube são aceites. Sessão **fechada** ⇒ grelha só-leitura.
- Input adapta-se ao tipo (§3.7): `NUMERO` (número), `BOOLEANO` (Sim/Não), `ESCALA` (1–5), `ESCALA_1_3` (1–3, botões toggle inline 1/2/3 com cor da marca no selecionado).
...
| A2 | Volume/cadência de exercícios: total de exercícios dados, média exercícios/sessão, **treinos/semana** e **treinos/mês** (média das semanas/meses ativos) | KPI cards (`totalExercicios`, `mediaExerciciosPorSessao`, `frequenciaSemanalMedia`, `frequenciaMensalMedia`) | **Recomendado** |
...
| E2 | Distribuição por **subcategoria** (customizável por clube; `null` → «Sem subcategoria») | `GraficoBarrasH` (top 10) | **Recomendado** |
...
As dimensões B, D, E e E2 são compostas numa única secção **«Composição dos treinos»** em grelha 2-up (`grid gap-6 sm:grid-cols-2`), cada gráfico só desenhado quando tem dados; a dimensão A2 é uma segunda linha de KPIs da secção «Volume de treino».

**Server Action:** `obterAnaliticoTreinoEscalao(escalaoId, epocaId?)` → `Resultado<AnaliticoTreinoEscalao>` — inclui `totalExercicios`, `mediaExerciciosPorSessao`, `frequenciaSemanalMedia`, `frequenciaMensalMedia`, `distribuicaoParteTreino` e `distribuicaoSubcategoria` (`{ subcategoriaId, subcategoria, categoria, totalUsos }`, ordenada desc).
...
| # | Indicador | Visualização | Prioridade |
|---|-----------|--------------|------------|
| A | Taxa de presença (sessões NORMAL) | KPI de assiduidade (`presenças/sessões`) | **Essencial** (unificar) |
| B | RPE médio (só se `RpeAtleta` preenchido) | KPI + `GraficoLinhas` (≥3 registos) | **Recomendado** |
| C | Exercícios por `categoriaPrincipal` (onde participou) | `GraficoBarrasH` (grelha 2 colunas no desktop, compacto) | **Recomendado** |
| C2 | Exercícios por **subcategoria** (onde participou; `null` → «Sem subcategoria») | `GraficoBarrasH` (mesma grelha 2 colunas, top 10) | **Recomendado** |
| D | Evolução mensal de presenças | `GraficoBarrasV` mensal — apresentado **uma vez** no painel principal (`PainelAtleta`), **não** repetido na secção «Treino» | **Essencial** (reutilizar existente) |

**Server Action:** `obterAnaliticoTreinoAtleta(atletaId, escalaoId?, epocaId?)` → `Resultado<AnaliticoTreinoAtleta>` — inclui `exerciciosPorSubcategoria` (`{ subcategoriaId, subcategoria, categoria, totalExercicios }`, ordenada desc).

**🔁 2026-09-04 (correções de UI da vista de treino do atleta):** (1) o **KPI de presenças** dos tiles do `PainelAtleta` mostra **percentagem + contagem absoluta** no formato `X% (N/T)` — N = sessões presentes (`agregado.presencas`), T = sessões totais (`agregado.sessoesTotais`) —, para o treinador ver de imediato quantas presenças suportam a taxa; (2) o gráfico **«Taxa de presença por mês»** deixa de ser **duplicado** — era desenhado no `PainelAtleta` **e** no `PainelTreinoAtleta`; passa a aparecer **só** no `PainelAtleta` (o `PainelTreinoAtleta` deixa de o desenhar, alinhado com «unificar»/«reutilizar existente» das linhas A/D); (3) o gráfico **«Exercícios por categoria»** (`GraficoBarrasH`) passa a viver numa **grelha de 2 colunas** (`lg:grid-cols-2`) ao lado do gráfico de RPE, para ficar **compacto** e à mesma escala dos rankings dos restantes painéis (o `GraficoBarrasH` é partilhado por vários painéis e **não** foi alterado — só o contentor).
...
- **Bloco de Guarda-redes numa sessão de equipa** (§8.8.2): no detalhe `/treinos/[id]`, os exercícios de `categoriaPrincipal=GUARDA_REDES` são **agrupados e rotulados "Bloco de Guarda-redes"** e mostrados **exclusivamente nesse bloco** — ficam **excluídos da lista principal de exercícios numerada** (`GestorExercicios`), para não aparecerem duplicados; a apresentação do bloco lista **apenas os guarda-redes presentes** na sessão (`Presenca` = `PRESENTE`/`ATRASADO` e `posicoes` inclui `GUARDA_REDES`). A separação é **só de apresentação**: as linhas `SessaoExercicio` de GR continuam a existir na sessão, participam do modo de condução (§8.8.2, que percorre todos os exercícios na ordem canónica das fases, sem alterações) e da ordenação `ordem` — a reordenação da lista principal reatribui `ordem` a **todos** os `SessaoExercicio` (incl. os de GR) para preservar o unique `[sessaoId, ordem]`. O agrupamento é **derivado da categoria** — sem flag nova em `SessaoExercicio`.
...
- **Acesso:** no **detalhe do jogo** (§8.11), o **topo do separador Convocatória** apresenta o botão **«Modo Jogo ao Vivo»** (🔁 2026-09-20, Fase C — antes vivia no separador «Ao Vivo», entretanto removido), que abre um **ecrã dedicado de condução** (`/jogos/[id]/ao-vivo`), a ecrã inteiro, otimizado para telemóvel, com alvos de toque ≥44px e **offline-first**.
...
- **Relação com o registo clássico por eventos:** 🔁 2026-09-20 (Fase C) o **separador «Ao Vivo» foi removido** da UI; o registo de eventos passa a fazer-se pelo **Modo Jogo ao Vivo** e a captura/correção de secundários (golo, assistência, cartão, remate…) faz-se na **grelha de Estatísticas** por atleta (§8.11). Tudo escreve em `EventoJogo` e converge para `EstatisticaAtleta` (§10.4).
...
1. **Arranque** — o treinador seleciona os **titulares** (o nº exato do formato — 5 em `FUTSAL_5`, 7/9/11 em futebol) e as respetivas **posições** (GR incluído), a partir da grelha de convocados; toca **«Iniciar Parte 1»** → o cronómetro arranca (`INICIO_PARTE`, `ENTRADA` de cada titular no segundo 0). O ecrã de arranque vem **pré-carregado** com os **titulares e posições do plano tático** do jogo (`Convocatoria.titularPrevisto`/`posicaoPrevista`, §8.10), se existir; sem plano tático, fica vazio e o treinador escolhe a partir do pool de convocados. A pré-seleção é sempre editável antes de iniciar. O **nº de partes** já **não** se escolhe no arranque: é definido na **criação/edição do jogo** (`Jogo.numeroPartes`, 1..4) e a `SessaoJogoAoVivo` herda-o ao iniciar.
2. **Durante a parte** — o ecrã mostra **em campo** (com posição) e **banco**; cada jogador exibe uma **legenda com o tempo de jogo acumulado** (minutos), atualizada **ao segundo** enquanto o cronómetro corre (deriva de `calcularMinutosDeEventos` sobre os eventos locais + segundo corrente). **Tap num atleta em campo** abre um **menu de ações rápidas** — **Golo**, **Cartão** (amarelo/vermelho) ou **Substituir**:
   - **Golo** — regista `GOLO` com `atletaId` (marcador), `parte` e segundo correntes; opção de escolher o **assistente** (outro atleta em campo, gravado em `atletaSecundarioId`) ou **saltar**. Um botão **«Golo sofrido»** regista `GOLO_SOFRIDO` (sem atleta obrigatório). No servidor (`sincronizarJogoAoVivo`), o golo com assistente **materializa também um evento `ASSISTENCIA` autónomo** (idempotente, `clientEventoId` derivado do golo) — é este que o motor único (§10.4) contabiliza como assistência; o `atletaSecundarioId` no golo fica só como rasto do par (o motor ignora-o, logo **não há dupla contagem**), coerente com o registo clássico.
   - **Cartão** — regista `CARTAO_AMARELO`/`CARTAO_VERMELHO` com `atletaId`, `parte` e segundo correntes (sem seleção manual de parte/minuto).
   - **Substituir** — escolher **quem entra** do banco → a substituição é registada com o **segundo corrente** (`SAIDA` do que sai + `ENTRADA` do que entra, no mesmo segundo).
   Todos estes eventos entram na **mesma *outbox*** append-only (§8.25.4) e sincronizam pelo caminho normal (`sincronizarJogoAoVivo`): *upsert* idempotente por `clientEventoId` e, quando o lote inclui `GOLO`/`GOLO_SOFRIDO`, **recálculo do placar** (`Jogo.golosMarcados`/`golosSofridos`) a partir da contagem de eventos (§10.4). O ecrã mostra ainda um **marcador ao vivo** derivado da contagem local de `GOLO`/`GOLO_SOFRIDO`.
...
- **Escrita nas estatísticas:** o resultado escreve `EstatisticaAtleta.minutos`, `minutosPorParte` e `utilizacao` (uma linha por convocado). Converge com o preenchimento manual da grelha (§8.11) por *last-write-wins*. O `blocoTempo` (§3.7) permanece disponível como alternativa por blocos para quem não usa o modo ao vivo.
- **🔁 2026-09-20 (BUG — golos e minutos por parte somem após terminar): snapshot COMPLETO no CREATE.** `persistirMinutos` (partilhado por `terminarJogoAoVivo` e `editarEventosJogoAoVivo`) faz *upsert* de `EstatisticaAtleta` para **todos os convocados**. Como o loader (§8.11, `combinarEstatisticasIniciais`) faz o registo persistido **sobrepor-se por inteiro** ao derivado (pressupõe que um registo persistido é um snapshot **completo**, tal como o grava `guardarEstatisticas`), o *upsert* distingue os dois caminhos: no **CREATE** (atleta ainda sem registo — típico de um jogo conduzido ao vivo cuja grelha nunca foi aberta) grava o **snapshot completo derivado** (`minutos`, `minutosPorParte`, `utilizacao`, `golos`, `assistencias`, `cartaoAmarelo`/`cartaoVermelho`, GR e núcleo de futebol); no **UPDATE** (registo já existe) só refresca `minutos`/`minutosPorParte`/`utilizacao`, **preservando** os contadores da grelha já gravados (*last-write-wins* da edição manual, §13.4). Antes, o CREATE gravava um registo **parcial** (golos=0, `minutosPorParte`=[]) que, ao sobrepor-se ao derivado no loader, **apagava** os golos e os minutos por parte capturados ao vivo. As métricas configuráveis (`ValorMetrica`) são linhas próprias e **nunca** são tocadas aqui.
...
- **🔁 2026-09-20 (Fase D — UI): editor SEMPRE disponível** na tab **Estatísticas** (§8.11), independentemente de ter existido sessão do Modo Jogo ao Vivo. Se houve **sessão terminada**, as linhas vêm **pré-preenchidas com os minutos derivados ao segundo** (título «Editar minutos (Modo Jogo ao Vivo)»); se **não** houve sessão, o editor é **semeado a partir da convocatória gravada** (uma linha por convocado, entrada/saída a começar vazias, título «Tempo de jogo»), fechando o fluxo **«preencher tudo à mão sem nunca ter ligado o modo ao vivo»**. A gravação usa `editarEventosJogoAoVivo` (§8.25.9), que persiste os intervalos/minutos **retroativamente** (cria sessão/eventos se ainda não existirem). Os **golos e restantes estatísticas** preenchem-se/corrigem-se na **grelha de Estatísticas** por atleta (já editável). Fecha a **RN-JV-15**.
- **🔁 2026-09-20 (Fase D — backend): `editarEventosJogoAoVivo` não pressupõe sessão prévia.** A action substitui apenas os eventos do cronómetro/quintetos (`INICIO_PARTE`/`FIM_PARTE`/`ENTRADA`/`SAIDA`/`PAUSA`/`RETOMA`) — os **eventos clássicos** (golos, cartões, assistências…) **não são tocados** — e recalcula `EstatisticaAtleta` pelo **motor único** (§10.4) com **upsert de todos os convocados** (RN-JV-13 alargada). Quando **nunca existiu** `SessaoJogoAoVivo` e há eventos, materializa uma **sessão mínima em estado `TERMINADO`** (nº de partes herdado do jogo; parte e segundo final derivados dos eventos), para o editor reabrir pré-preenchido. Uma **sessão já existente** (ex.: terminada ao vivo) é **preservada intacta**, continuando a permitir edição. Os minutos inseridos à mão **convergem** exatamente com os do caminho ao vivo para os mesmos intervalos.
- **🔁 2026-09-20 — Correção manual por parte:** o tempo de jogo passa a corrigir-se por **minutos absolutos por parte** (`EstatisticaAtleta.minutosPorParte`, uma célula por parte até `Jogo.numeroPartes`), sendo o **total = soma das partes** (§8.11/§10.4). O editor recebe os valores iniciais em `estatisticasIniciais[atletaId].minutosPorParte` (persistido → derivado dos eventos → `[]`) e o `numeroPartes` do jogo; a gravação por `guardarEstatisticas` é a **verdade final** (*last-write-wins*) e escreve `minutos = soma(minutosPorParte)`.
...
- **Placar e outros eventos:** 🔁 **Fase B** — golos/assistências/cartões passam a poder ser **capturados diretamente no Modo Jogo ao Vivo** (por toque, §8.25.3), além do separador **Ao Vivo** clássico e da grelha, que se mantêm. Independentemente da origem, os eventos são o mesmo `EventoJogo`: o **placar** (`Jogo.golosMarcados`/`golosSofridos`) sincroniza pela contagem de eventos (`recalcularResultadoJogo`, partilhado por `registarEventoJogo` e `sincronizarJogoAoVivo`) e os **contadores por atleta** (golos/assistências/cartões) são derivados de **todos** os eventos pelo motor único (§10.4) — **derivados automaticamente para a grelha** ao carregar a vista (§8.11), exatamente como para os golos do registo clássico. Assim, os golos/cartões deixam de viver só numa superfície de eventos própria (🔁 2026-09-20, Fase C — o separador «Ao Vivo» foi removido). O Modo Jogo ao Vivo continua a ser a fonte autoritária do **tempo de jogo e quintetos**.
...
model SessaoJogoAoVivo {
  ...
  jogo               Jogo              @relation(fields: [jogoId], references: [id], onDelete: Cascade)
  numeroPartes       Int               @default(2)  // 1..4 — herdado de Jogo.numeroPartes ao iniciar
  duracaoParteMins   Int               @default(20) // duração de cada parte, em minutos
...
- **`Jogo`** ganha a coluna `numeroPartes Int @default(2)` (1..4 — definido na criação/edição; a `SessaoJogoAoVivo` herda-o ao iniciar), a relação inversa `sessaoAoVivo SessaoJogoAoVivo?` e a chave `notasAoVivo` no JSON de `relatorio` (sem coluna nova; via `lib/relatorio-jogo.ts`).
...
- `iniciarJogoAoVivo(jogoId, { titulares: [{ atletaId, posicao }] })` — cria/reinicia a `SessaoJogoAoVivo` (`POR_INICIAR`); o `numeroPartes` é **herdado de `Jogo.numeroPartes`** (o input pode trazê-lo por retrocompatibilidade, mas o do jogo prevalece); valida que os titulares são convocados e que o nº = tamanho do `formato` (RN-JV-1/2).
...
**Rotas:** `/jogos/[id]/ao-vivo` (condução) — alcançada pelo botão «Modo Jogo ao Vivo» no **topo do separador Convocatória** (§8.11; 🔁 2026-09-20, Fase C — antes no separador «Ao Vivo», removido).
...
- **RN-JV-5** — Suporta **1 a 4 partes** (definidas em `Jogo.numeroPartes`, na criação/edição); o **segundo absoluto é contínuo** entre partes (só a numeração de parte avança).
...
- **RN-JV-15** — **Inserção retroativa** permitida: criar/editar sessão e eventos após o jogo, via editor tabular. 🔁 2026-09-20 (Fase D): o editor de tempo de jogo está **sempre disponível** na tab Estatísticas (§8.11, §8.25.6) — sem sessão prévia é semeado a partir da convocatória, permitindo registar minutos/golos **inteiramente à mão**.
...
- **Título do confronto (apresentação):** sempre que um jogo é apresentado como confronto na UI (cabeçalho do detalhe, breadcrumb, listas de jogos, cartões do dashboard, marcador do registo ao vivo, cabeçalho do Modo Jogo ao Vivo, rótulos de seleção de jogo), o título é composto na forma **`<Nós> vs <Adversário>`** respeitando a ordem casa–fora (**em casa:** `Nós` à esquerda; **fora:** `Nós` à direita). `Nós` é **sempre** o **nome do clube** do utilizador (`Clube.nome`, resolvido pela adesão ativa — nunca a palavra "Nós" nem vazio; recurso `"A nossa equipa"` só se o nome faltar). O nome da nossa equipa é renderizado a **negrito** e com a **cor do clube** (`--cor-primaria`), exceto sobre fundos que já usam a cor do clube (cartão-herói do dashboard, overlay escuro do Modo Jogo ao Vivo), onde fica só a negrito para preservar o contraste. Fonte única: `lib/jogo-confronto.ts` (string) e `components/jogos/TituloConfronto.tsx` (JSX). **Nota:** os textos externos/notificações (mensagens de comunicação, evento de Google Calendar, lembretes do dashboard, título do evento na agenda, feed de atividade) mantêm o formato compacto `vs <Adversário>` já especificado — não são o título de confronto na UI.
...
**🔁 KPI «jogos» conta só jogos realizados (2026-09-04, BUG):** o KPI **«jogos»** do balanço da época (`AnaliticoEscalao.jogos`) conta **apenas jogos já realizados** (`Jogo.data <= agora`), em simetria com o V/E/D e com `sessoesExecutadas`. Antes contava também **jogos futuros/agendados** (ex.: mostrava «2 jogos» quando esses jogos ainda não tinham acontecido). O filtro `data: { lte: agora }` está no `filtroJogo` de `obterAnaliticoEscalao`, pelo que se aplica de forma consistente a **tudo o que deriva dos jogos** (jogos, V/E/D, golos e médias por jogo, marcadores/assistentes, mais utilizados, eventos e rankings por métrica). Jogos futuros ficam agendados na Agenda/lista de jogos, mas não entram nos analíticos.
...
**🔁 Tabela por atleta + ativos/inativos (2026-09-04):** o agregado do escalão expõe uma **tabela com TODOS os participantes da época** (`AnaliticoEscalao.tabelaAtletas`, tipo `LinhaAtletaEscalao`) — inclui o **histórico persistente** de quem já saiu do escalão (estados `INATIVO`/`TRANSICAO_PERMANENTE` ou atleta desativado), coerente com §10.1. Cada linha traz `atletaId`, `nome`, `posicoes`, `estadoParticipacao` (`EstadoParticipacao`), `atletaAtivo`, `golos`, `assistencias`, `jogosUtilizados`, `jogosConvocado`, `presencas`, `taxaPresenca` (0–1, mesmo denominador — sessões executadas — do `rankingAssiduidade`, com cap a 1) e `tempoJogo` (minutos acumulados). O agregado distingue ainda **`nAtletasAtivos`** (participantes `ATIVO` **e** atleta ativo) de **`nAtletasInativos`** (os restantes); **`nAtletas` mantém a semântica original** (= `nAtletasAtivos`), continuando a ser o **denominador das médias** (taxa de presença média, presença mensal). A tabela vem ordenada por nome (pt).
...
**Motor ÚNICO de derivação (`lib/derivar-estatisticas.ts`, 🔁 2026-09-19).** Existe **um só** derivador puro `derivarEstatisticas(eventos, convocados, eFutebol, formato)` que transforma os eventos nas linhas de `EstatisticaAtleta` (uma por convocado) e devolve o resultado do jogo (`golosMarcados`/`golosSofridos`). Este motor **entende os dois registos** e converge-os: o registo **clássico por blocos** (`bloco`/`blocoTempo`) e o registo do **Modo Jogo ao Vivo por intervalos ao segundo** (`segundoJogo`, pares `[ENTRADA, SAIDA]`). Substituiu os dois motores que antes coexistiam e se ignoravam (`calcularMinutosDeEventos` sobre `segundoJogo` **e** o antigo `derivarEstatisticasDeEventos` sobre `bloco`), corrigindo um bug **P0** em que carregar «Preencher do registo ao vivo» sobre um jogo conduzido ao vivo devolvia `minutos: null`/blocos grosseiros e **sobrescrevia** os minutos precisos já gravados.

**Regra de precedência dos minutos (CONFIRMADA):** para cada atleta, **intervalos (cronómetro, ao segundo) > blocos (legado) > vazio/null**. A **edição manual** na grelha de Estatísticas **sobrepõe-se sempre a tudo** (last-write-wins, §13.4): o automatismo só preenche o rascunho pela precedência acima; uma correção manual à mão ganha.

**Regras de contadores/utilização:** cada convocado começa `NAO_UTILIZADO` com contadores a zero (`TITULAR` se `titularPrevisto`); `GOLO`/`GOLO_SOFRIDO` contam sempre para o placar do jogo e, com `atletaId`, para o atleta (`golos`/`golosSofridosGR`); `ASSISTENCIA`, `FALTA`, `CARTAO_AMARELO`, `CARTAO_VERMELHO`, `DEFESA` incrementam o respetivo campo do atleta; o núcleo de futebol só conta com `eFutebol=true` (em futsal fica `null`); `SUBSTITUICAO` marca o atleta que entra como `UTILIZADO` (sem despromover um titular) e regista o `bloco`. Para os **minutos e a utilização**: se o atleta tem intervalos ao vivo, os minutos vêm do cronómetro (via a primitiva pura `calcularMinutosDeEventos`) e a utilização é `TITULAR` (entrou no `INICIO_PARTE` da Parte 1) ou `UTILIZADO` (entrou depois) — o cronómetro **prevalece** sobre o plano previsto; senão, se há `blocoTempo`, os minutos derivam do bloco via `blocoParaMinutos` (§10.1/§10.8); senão, `minutos` fica `null` (não registado, ≠ zero).

**Três pontos de entrada convergem no mesmo motor** (produzem `minutos` idêntico para o mesmo conjunto de eventos): (1) `previewEstatisticasDeEventos(jogoId)` — o botão «Preencher do registo ao vivo» — expõe a derivação **sem persistir** (auth + multi-tenant + `ESTATISTICAS_GERIR`), para o treinador rever antes de guardar; (2) `terminarJogoAoVivo` e (3) `editarEventosJogoAoVivo` (§8.25.9) persistem os minutos/utilização. A persistência ao vivo faz **`upsert` de `EstatisticaAtleta` para todos os convocados** (RN-JV-13 alargada, 2026-09-19) — não perde os minutos ao vivo quando a grelha nunca foi aberta — escrevendo **apenas** `minutos`/`utilizacao` (os contadores da grelha manual são preservados). O segundo final (fecho de intervalos abertos) é derivado internamente pelo motor (maior `FIM_PARTE`, senão maior `segundoJogo`). **🔁 2026-09-20 — paridade com o loader:** `persistirMinutos` deriva do **registo COMPLETO do jogo** (clássico **e** ao vivo), lido na transação — **não** apenas dos `TIPOS_AO_VIVO`. Antes lia só os eventos de cronómetro e forçava `bloco:null`, pelo que gravar o editor retroativo de um **jogo legado por blocos** (`SUBSTITUICAO`/`blocoTempo`) escrevia `minutos:null` por cima dos minutos legados (perda silenciosa). Como os eventos de bloco **sobrevivem** ao `deleteMany` (que só filtra `TIPOS_AO_VIVO`), o motor re-deriva os minutos dos blocos pela mesma precedência (**intervalos > blocos > null**) e o valor persistido passa a ser **idêntico** ao que o loader mostra.

**Placar sincronizado.** O recálculo do placar vive numa função partilhada **`recalcularResultadoJogo(tx, jogoId)`** (`lib/placar-jogo.ts`), reutilizada por `registarEventoJogo`/`removerEventoJogo` (registo clássico) **e** por `sincronizarJogoAoVivo` (Modo Jogo ao Vivo, 🔁 Fase B). Corre em transação e, quando há `GOLO`/`GOLO_SOFRIDO` em jogo, recalcula `Jogo.golosMarcados`/`golosSofridos` a partir da **contagem de eventos**, mantendo o resultado sempre coerente com o registo — seja ele feito no separador clássico ou por toque no Modo Jogo ao Vivo. É **idempotente**: como o *sync* faz *upsert* por `clientEventoId` (`skipDuplicates`), re-sincronizar não duplica eventos nem altera o placar.

**Captura ao vivo → contadores por atleta (🔁 Fase B).** Os golos/assistências/cartões capturados no Modo Jogo ao Vivo são o mesmo `EventoJogo` (trazem `segundoJogo`, mas a **fase de contagem do motor não filtra por `segundoJogo`**) e são agregados por `derivarEstatisticas` exatamente como os do registo clássico. Um golo com assistente materializa um evento `ASSISTENCIA` autónomo no *sync* (o motor conta a `ASSISTENCIA` e ignora `atletaSecundarioId`, sem dupla contagem). Os contadores por atleta chegam a `EstatisticaAtleta` pelo caminho normal da grelha — `previewEstatisticasDeEventos` lê **todos** os eventos e deriva-os, à imagem dos golos do registo clássico.

**Derivação automática ao carregar a vista (🔁 Fase C — 2026-09-20).** A vista de **Estatísticas** passa a ser a **vista consolidada única** (§8.11): o **loader server-side** (`app/(app)/jogos/[id]/page.tsx`) combina, por atleta, o **rascunho derivado dos eventos** (`derivarEstatisticas`, incluindo os **secundários por atleta** — remates, cantos, faltas, desarmes, defesas, foras-de-jogo) com a **edição manual persistida** em `EstatisticaAtleta`, que **prevalece** por atleta (*last-write-wins*, §13.4). A combinação é a função pura **`combinarEstatisticasIniciais(eventos, convocados, persistidas, eFutebol, formato)`** (`lib/derivar-estatisticas.ts`): deriva tudo e depois **sobrepõe** integralmente os registos persistidos (preservando `valoresMetricas`). Assim, ao abrir a grelha os valores **já vêm derivados sem ação manual** — substituindo o botão **«Preencher do registo ao vivo»** (agora redundante; `previewEstatisticasDeEventos` mantém-se apenas enquanto a UI referir o botão, marcada para remoção). `guardarEstatisticas` continua a ser o ponto de gravação da edição manual (verdade final). **Timeout:** `TIMEOUT` **não tem campo por atleta** — mantém-se como `EventoJogo` ao nível do jogo/equipa (cronologia do jogo) e **não é perdido**; o motor único simplesmente não o mapeia para nenhuma coluna individual. *(🔁 2026-09-20 — Fase C/D UI concluídas: a tab «Ao Vivo» foi removida, o CTA «Modo Jogo ao Vivo» foi reposicionado para o topo da Convocatória e o botão «Preencher do registo ao vivo» foi removido. Em consequência, `previewEstatisticasDeEventos` deixou de ser referenciado pela UI e fica marcado para remoção pelo backend.)*

**Minutos por parte (🔁 2026-09-20 — editor de tempo por parte).** Além do **total** de minutos, o motor único deriva por atleta os **minutos por parte** (`EstatisticaAtleta.minutosPorParte: Int[]`, índice 0 = Parte 1, ..., comprimento = `Jogo.numeroPartes`, 1..4). O cálculo (`derivarEstatisticas` / `combinarEstatisticasIniciais`, `lib/derivar-estatisticas.ts`, funções puras) parte dos intervalos `[ENTRADA, SAIDA]` ao segundo do Modo Jogo ao Vivo e **divide-os pelos limites de cada parte** (`INICIO_PARTE`/`FIM_PARTE` dão o segundo de fronteira). Cada posição do array são os minutos jogados nessa parte, arredondados por **maior resto** (largest remainder) para garantir que **`soma(minutosPorParte) === minutos`** (o total mantém-se byte-idêntico ao motor de minutos). **Precedência** por atleta: **persistido (BD) > derivado dos eventos > `[]`**. Jogos **legados** sem eventos ao vivo (só blocos) ficam com `minutosPorParte = []` (só há total). O loader (`app/(app)/jogos/[id]/page.tsx`) entrega ao editor `estatisticasIniciais[atletaId].minutosPorParte` (persistido/derivado) e o `numeroPartes` do jogo. **Correção manual** faz-se por **minutos absolutos por parte** e é a **verdade final** (*last-write-wins*): `guardarEstatisticas` persiste `minutosPorParte` e, quando vem preenchido (`length > 0`), grava **`minutos = soma(minutosPorParte)`**; vazio → mantém o comportamento legado de `minutos`.
...
> **🏷️ v7 — Rótulo personalizado do jogador (DEVERIA):** o elemento `Jogador` tem um campo **opcional** `rotulo?: string` (máx. **6** caracteres) que substitui a etiqueta apresentada dentro do círculo. Ausente/vazio → mantém-se o comportamento anterior: mostra o **número** (`numero`) ou, para o adversário, **"A"** (retrocompatível — diagramas gravados antes do rótulo continuam válidos). Permite nomear jogadores por função (ex.: «Ala», «Pivot», «Fixo») em vez do número. Render em `ElementoSVG` (`components/campo/desenho.tsx`): a `etiqueta` deriva de `rotulo ?? numero ?? "A"` e o `fontSize` **reduz** para caber (8 até 2 chars, 6 até 3, 5 acima — ex.: «Pivot»/«Ala»). Valida por `jogadorSchema` (`rotulo: z.string().max(6).optional()`, `lib/schemas/exercicio.ts`) e persiste no JSON do diagrama. No editor, ao selecionar um jogador (modo Selecionar) aparece um painel contextual com um `Input` (máx. 6, placeholder «ex: Ala») e um botão **X** para limpar (volta ao número); a alteração regista-se no histórico/undo (`mudarRotuloJogador` em `EditorCampo.tsx`, análogo a `mudarCorCone`). Um rótulo vazio remove a propriedade do elemento.
...
> **▶️ v7 — Auto-reprodução ao ver o diagrama em grande (DEVERIA):** quando o treinador abre o diagrama de um exercício **em grande** (modal do plano de exercícios da sessão, §8.8.2 — `ModalDiagramaExercicio`), o exercício **auto-reproduz-se** e clicar na imagem volta a reproduzir. O componente unificado **`DiagramaReproduzivel`** (`components/campo/DiagramaReproduzivel.tsx`) decide o modo: (a) diagrama **com passos/keyframes** → reutiliza o motor `CampoAnimado` (`autoPlay`, movimento interpolado dos jogadores/bola + controlos); (b) diagrama **sem passos mas com setas/linhas** → **desenha as setas progressivamente e em sequência** (efeito «draw»: `stroke-dashoffset` 1→0 sobre um trajecto normalizado com `pathLength=1`, atraso incremental por seta), com um botão **play/replay** sobreposto (▶ / ↻); (c) sem nada para animar → miniatura estática. O desenho progressivo é aditivo em `ElementoSVG` (`components/campo/desenho.tsx`, prop opcional `animarTrajeto`) e usa o keyframe CSS `campo-desenhar-trajeto` (`app/globals.css`). Respeita `prefers-reduced-motion` (não anima; mostra o diagrama já desenhado). Cada seta desenha-se em ~820 ms com atraso de ~420 ms entre setas (dentro do intervalo 600–1200 ms), com easing suave.

> **↔️ v7 — Reordenar passos para qualquer posição (DEVE):** na timeline de passos (`components/campo/TimelinePassos.tsx`) um passo pode ser movido para **qualquer** posição da sequência — não apenas trocado com o vizinho. Cada chip de passo tem uma **pega de arrasto** (ícone `GripVertical`, `draggable`) que permite **arrastar-e-largar** (drag-and-drop por ponteiro) sobre o passo de destino; em paralelo, os **botões de seta ‹ ›** (acessíveis, alvo ≥44px, cobrem toque e teclado) movem uma posição de cada vez. A reordenação usa a função **pura** `moverPasso(passos, de, para)` (`components/campo/animacao.ts`): remove o passo da posição de origem, insere-o no destino e **reindexa `ordem` sequencialmente (0..N-1)**. Como cada `PassoAnimacao` guarda posições **absolutas**, mover o passo **preserva o seu movimento** (o keyframe reconstruído por `construirKeyframes` mantém-se coerente). O **keyframe activo acompanha** o passo movido via `ajustarKeyframeAposMover(activo, de, para)` (a base/"Início" `-1` nunca é afetada). Ambas as funções são testadas em `tests/campo.test.ts`. Corrige o caso em que, ao adicionar um passo novo (que surge no fim da lista), não era possível **arrastá-lo para o meio** da sequência.

> **🎯 v7 — Seleção no modo animação (DEVE):** apenas **elementos-ponto** (jogadores/pessoas, bola, cones, arcos, escadinhas, barras…) participam nos passos de animação — as **setas e linhas não são selecionáveis** enquanto o **modo animação** está ativo. Sem esta restrição, uma seta desenhada por cima de uma bola (índice mais alto no array → avaliada primeiro no hit-test, com faixa de toque larga) intercepta o clique e impede selecionar a bola para animar. Implementação: `elementoEmPonto` (`components/campo/animacao.ts`) aceita a opção `{ apenasPontos }` que ignora setas/linhas no hit-test; o `EditorCampo` passa `apenasPontos: modoAnimacao` na seleção por ponteiro e desativa a faixa de hit/foco do trajecto (`pathSelecionavel={!modoAnimacao}` em `ElementoSVG`, `components/campo/desenho.tsx`) para eliminar também a seleção por foco de teclado. **Fora do modo animação**, setas e linhas continuam totalmente selecionáveis, arrastáveis e apagáveis (comportamento inalterado).
...
**🔁 Ocultação do modo Individual (Fase 4, 2026-09-11):** o padrão de ocultação da gestão de clube segue a flag **`clube.clubeTecnico`** (via `ocultarIndividual`), materializado na **página de Definições** (§8.4) — **não** há itens de gestão de clube na **navegação de topo** a ocultar. A ocultação é cosmética; a autorização vive nos guards de servidor (§7.3).

**🔁 Padrão «aguarda ação» (Fase 5, 2026-09-11):** registos que aguardam uma ação do admin (ex.: licenças `PENDENTE` no backoffice, §21.2) usam um destaque consistente — **borda esquerda + fundo âmbar suave** (tokens `ambar-500`/`ambar-600`, contraste **AA**) — e, no topo da vista, um **banner `role="status"` âmbar** com a **contagem de pendentes**. É o mesmo vocabulário visual do «aviso» de §12.1, reservado para itens que pedem intervenção humana.

**🔁 Seletor de plano/modalidade no registo (Fase 3, 2026-09-11):** o formulário de registo (§8.1) usa **cartões de plano** — nome do tier, **limite de escalões**, **preço mensal/anual** (lido de `lib/billing.ts`), com **«Clube Pequeno» marcado como «mais popular»** — e um **seletor de modalidade** (2 cartões, **sem pré-seleção**). Radios `sr-only` (acessível), **alvos ≥44px**, estado ativo `border-primary`. **Reutiliza o componente-cartão do onboarding** (consistência visual entre registo e wizard).
...
**Obrigatório testar:**
- **Schemas Zod** (válidos/inválidos) — todos os módulos, incl. **novos v7** (secção, formato de jogo, posições de futebol, estatísticas de futebol, modalidade no exercício/template/métrica).
...
### 15.1 Stack
Next.js 15 (App Router) · React 19 · TypeScript strict · Prisma + PostgreSQL (Supabase) · Auth.js v5 · Zod · Tailwind + shadcn/ui · Vitest · PWA. **Supabase Storage** (`@supabase/supabase-js`, cliente **server-only** em `lib/supabase-storage.ts`) para logótipos/ficheiros — bucket **público** `atletas` (fotos de atletas, paths UUID não-adivinháveis, §8.5); processamento de imagem com `sharp` (server) + `browser-image-compression` (cliente). Integrações: **Google Calendar** (OAuth) e **Paddle** (billing, futuro). Sem IA no núcleo.
...
#### Fase 35 — Upload de fotografias de atletas (2026-09-10)

**Dependências:** Plantel (Fase 11 / §8.5); Supabase Storage (bucket público `atletas`).
**Schema:** zero alterações (reutiliza `Atleta.fotoUrl`).
**Entregas:**
1. `@supabase/supabase-js` + cliente **server-only** `lib/supabase-storage.ts` (bucket `atletas`, helpers de path/hostname).
2. Server Action `uploadFotoAtleta(atletaId, formData): Promise<Resultado<{ fotoUrl }>>` em `lib/actions/atletas.ts` — auth + isolamento por clube + `PLANTEL_GERIR`, **validação de magic bytes** (JPEG/PNG/WebP) e **tamanho** (≤10 MB), re-encode `sharp` → **WebP 256×256** (q80, `cover`, sem upscale), path UUID, upload ao bucket, **apagar foto anterior** e persistir `fotoUrl`.
3. Ciclo de vida no hard-delete (`apagarAtletaDefinitivamente`): apaga a foto do Storage (best-effort) antes do `delete`.
4. `next.config.js` — `serverActions.bodySizeLimit: "5mb"`; `.env.example` — `NEXT_PUBLIC_SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY`.
...
**Schema:** `MetricaConfig.aplicaSoGuardaRedes Boolean @default(false)` (DEVE); `TipoSessao.EXTERNA_GR` (DEVE); `Sessao.entidadeExterna String?` (DEVE). Migração aditiva `gr_treino_especifico`.
**Entregas:**
1. Seed de subcategorias/métricas GR por modalidade
...
7. secção "Desenvolvimento do guarda-redes" no painel do atleta (evolução de métricas GR + núcleo de jogo).
...
- A licença técnica fica modelada em `Licenca` (3.11); o enforcement efetivo entra com o billing. **🔁 (resolvido Fase 4):** `Licenca.modalidade` regista a modalidade Individual contratada — **persistida na criação** e **exclusiva do Individual** (`null` em Clube); é a **fonte de verdade** do produto vendido (3.11). A licença Individual é titulada por **`utilizadorId`** (nunca `clubeId`), ainda que o clube técnico seja o tenant de topo.
...
- **Fluxo de pagamento interino (enquanto o Paddle está deferido) — completo:**
  1. **Escolha de plano/modalidade no registo (🔁 Fase 3).** No **passo único de registo** (§8.1), o utilizador **escolhe o plano** (tier: `INDIVIDUAL | PEQUENO | MEDIO | GRANDE`) e a **modalidade** (obrigatória). O formulário mostra os cartões de plano com preço lido de `lib/billing.ts` (§12). *(Antes da Fase 3 esta escolha vivia no wizard de onboarding.)*
  2. **Licença `PENDENTE` na criação da conta.** No registo, a action **`registar()`** (§8.1) cria **atomicamente** conta + clube + secção + época + membro Administrador e uma `Licenca` com **`estado: PENDENTE`**, o **tier escolhido** e o `ciclo`/`precoCentimos` correspondentes. `PENDENTE` **não concede acesso** (§3.11), pelo que o utilizador é encaminhado para o paywall. *(O fallback autenticado `/criar-clube` mantém `criarClube` para criação de clube já autenticado.)*
  ...
  4. **Ativação manual pelo admin.** O utilizador **transfere** e **envia o comprovativo por email**. O admin, ao receber o comprovativo, **ativa a licença no backoffice** (§21.2) via ação dedicada **`ativarLicenca`**: estado **`PENDENTE → ATIVA`**. A partir daí a guarda de licença (§3.11) concede acesso. **🔁 Fase 5:** a ativação **dispara automaticamente o email «A tua conta Mister está pronta»** ao titular (Clube via admin do clube; Individual via utilizador), em modo **best-effort** (a falha de email não desfaz a ativação). A licença fica válida até ao fim do ciclo pago — a ativação **define `dataFim` = +1 ciclo** (MENSAL +1 mês / ANUAL +1 ano) **se ainda não estiver definida**.
...
- **Resolução do plano pendente (`obterLicencaPendente`).** O paywall `/sem-licenca` resolve o plano **`PENDENTE`** por **duas vias**, ambas derivadas **exclusivamente da sessão autenticada** (defesa contra IDOR): licença de **Clube** (por `clubeId`) **ou** licença **Individual** (por `utilizadorId`). Em qualquer dos ramos mostra o **valor exato a transferir** (passo 3 do fluxo interino acima).
- **Ativação exclusiva do backoffice.** A ativação de licença (`PENDENTE → ATIVA`) é **prerrogativa exclusiva do backoffice** (admin de plataforma, §21.2) e ocorre sempre **após confirmação de pagamento** (passo 4 do fluxo interino). Foi **removida** a ativação self-service de «licença de demonstração» por admin de clube (antiga action `criarLicencaDemostracao` + botão `ActivarDemoButton`), que emitia licença **`ATIVA` fora do fluxo de pagamento**. A página **Definições · Licença** deixa de oferecer ativação e passa a **sinalizar** que a ativação ocorre após confirmação de pagamento pela plataforma.
- **Helper interno de criação de licença de onboarding.** A criação da licença de onboarding foi **centralizada** num helper interno que aceita o titular por **`clubeId`** ou por **`utilizadorId`** (preparação para o ramo **Individual** das fases seguintes), **sem alterar** o comportamento do ramo **Clube** já descrito (passo 2 do fluxo interino).
- **🔁 Materialização do ramo Individual (Fase 4, 2026-09-11).** O ramo **Individual** do registo cria a licença **titulada por `utilizadorId`** com a **modalidade contratada persistida no momento da criação** (dentro da transação — **não** por `updateMany` pós-transação), mais a **`Carteira`** da pessoa (idempotente por `upsert`) e uma **única secção** da modalidade (§3.11, §8.1). Licenças de **Clube** **não** guardam modalidade (`Licenca.modalidade = null`). A **leitura da licença ativa** (`obterLicenca`, usada em **Definições · Licença**) resolve a titularidade por **`clubeId` OU `utilizadorId`**, para que a página funcione também no ramo Individual — **fix de code review M1** (antes só resolvia por `clubeId`, deixando Definições · Licença sem licença para utilizadores Individual).
- **Seed de desenvolvimento (dev-only).** O `prisma/seed.ts` cria, para o clube de demonstração, uma `Licenca` com **`estado: ATIVA`** (`tipo: CLUBE`, `tier: MEDIO`, `ciclo: MENSAL`, `dataFim` a 1 ano) e a `Carteira` do administrador (`saldoCentimos: 0`), de forma **idempotente** (`upsert` pelas chaves únicas `clubeId`/`utilizadorId`), para **passar a guarda de licença** (§3.11) e entrar **sem paywall** em desenvolvimento/testes. **Nunca em produção:** protegido por **`NODE_ENV !== "production"`** — em produção a ativação continua a ser apenas pela via legítima de billing. É um **artefacto de conveniência de dev, não estado de negócio**.
- **Email transacional (Resend).** Nova camada server-side **`lib/email/`** (SDK Resend), **distinta e independente** de `lib/email.ts` (recuperação de password, §6.2). `enviarEmail` é **best-effort** — nunca parte o fluxo e degrada graciosamente sem `RESEND_API_KEY`. Dois **templates transacionais PT-PT**, consumidos no registo (Fase 3, → admin) e na ativação (Fase 5, → utilizador): **(a) novo registo → admin** (assunto `Novo registo — [clube]`; destinatário `ADMIN_EMAIL`, com fallback para o email de suporte); **(b) ativação → utilizador** (assunto `A tua conta Mister está pronta`). Templates com **marca Mister**; valores do utilizador **escapados** (anti-injeção). Nova variável de ambiente **`ADMIN_EMAIL`** (reutiliza `RESEND_API_KEY`/`EMAIL_FROM`).
- **Notificação ao admin no registo (best-effort) 🔁 Fase 3.** No registo, a action `registar()` dispara, **best-effort**, o email ao admin (template **«novo registo»**, alínea (a) acima) com `{ nome, email, clube, plano, modalidade }`. A **falha de email não aborta o registo** — `enviarEmail` degrada graciosamente e a conta/licença `PENDENTE` fica criada na mesma.
- **Critérios distintos: mostrar o registo vs. autorizar acesso (comportamento deliberado).** `obterLicenca` (Definições · Licença) seleciona a licença por **`estado === "ATIVA"`** **sem** avaliar `dataFim`, enquanto a **guarda de acesso** (`temLicencaValida`/`licencaValida`) avalia o par **`(estado, dataFim)`**. Consequência **intencional**: uma licença `ATIVA` com `dataFim` já no passado **continua a ser mostrada** na página de licença, mas o **acesso é negado** pela guarda — os dois critérios respondem a perguntas diferentes (mostrar o registo existente vs. autorizar acesso).
...
- **2026-09-20** — **§10.1, §10.4, §10.8 (BUG — minutos de jogo a zero nas Analytics porque a agregação lia só `blocoTempo` e ignorava o campo autoritativo `minutos`)** — **Corrigido: nas analytics (perfil do atleta, comparação, médias da equipa e análise por escalão) o tempo de jogo aparecia a zero/vazio para jogos conduzidos no Modo Jogo ao Vivo ou editados pelo editor de minutos por parte.** **Causa raiz:** esses fluxos gravam o tempo no campo autoritativo `EstatisticaAtleta.minutos`/`minutosPorParte` com `blocoTempo = null`, mas toda a agregação de tempo derivava exclusivamente dos blocos (`blocoParaMinutos(blocoTempo, formato)`), pelo que um registo sem bloco somava 0. **Fix (preferir `minutos`, bloco como fallback):** **(1) `lib/estatisticas.ts` (`agregarEstatisticas`)** — `tempoJogoAcumulado` passa a somar `e.minutos ?? blocoParaMinutos(e.blocoTempo, e.formato)` (a interface `LinhaEstatistica` já expunha `minutos` e `formato`; sem mudança de contrato). Corrige `obterAnaliticoAtleta`/`obterResumoAtletaParaComparacao`, que já selecionavam `minutos` mas viam-no descartado na agregação. **(2) `lib/actions/analise.ts` (`calcularComparacaoEquipa` — médias da equipa para comparação individual)** — `select` da query de estatísticas passa a incluir `minutos: true`; o reduce de `totalTempo` usa `e.minutos ?? blocoParaMinutos(e.blocoTempo, e.jogo?.formato)`. **(3) `lib/actions/analise.ts` (análise por escalão, `obterAnaliticoEscalao`)** — `select` passa a incluir `minutos: true`; o acumulador `u.tempo` usa `e.minutos ?? blocoParaMinutos(e.blocoTempo, e.jogo?.formato)`. **Zero-regressão:** o fallback `?? blocoParaMinutos(...)` preserva o comportamento exato dos jogos legados que só têm `blocoTempo` (com `minutos = null`). Só leitura/agregação — sem alteração de schema, migração ou escrita. `npm run test` verde (**1653 testes**, 77 ficheiros) · `npm run typecheck` limpo. **Não toca em auth.**
- **2026-09-20** — **§8.25.5, §8.11 (BUG — golos e minutos por parte somem das Estatísticas depois de terminar o jogo ao vivo)** — **Corrigido: um jogo conduzido no Modo Jogo ao Vivo (com golos capturados por toque e minutos por parte) mostrava, na tab Estatísticas, os golos a 0 e os minutos por parte a zero logo após «Terminar jogo».** **Causa raiz:** `persistirMinutos` (`lib/actions/jogo-ao-vivo.ts`, partilhado por `terminarJogoAoVivo` e `editarEventosJogoAoVivo`) fazia *upsert* de `EstatisticaAtleta` para **todos os convocados** escrevendo apenas `minutos`/`utilizacao`. Quando o atleta **ainda não tinha registo** (jogo ao vivo cuja grelha nunca foi aberta), o `create` nascia com `golos=0`, `minutosPorParte=[]` e restantes contadores a 0/null. Como o loader (§8.11, `combinarEstatisticasIniciais`) faz o registo persistido **sobrepor-se POR INTEIRO** ao derivado (assume que um registo persistido é um snapshot completo, tal como o grava `guardarEstatisticas`), esse registo parcial **apagava** os golos e os minutos por parte que tinham sido derivados dos eventos. **Fix (opção B — persistido = snapshot completo):** o *upsert* passa a distinguir os caminhos — no **create** grava o **snapshot COMPLETO derivado** (`minutos`, `minutosPorParte`, `utilizacao`, `golos`, `assistencias`, `cartaoAmarelo`/`cartaoVermelho`, `defesas`/`golosSofridosGR`/`faltasCometidas` e núcleo de futebol `remates`/`cantos`/`forasDeJogo`/`desarmes`); no **update** (registo já existe) mantém-se o comportamento anterior de só refrescar `minutos`/`minutosPorParte`/`utilizacao`, **preservando** os contadores da grelha já gravados (*last-write-wins* da edição manual, §13.4). As métricas configuráveis (`ValorMetrica`) são linhas próprias e nunca são tocadas. Assim o registo persistido fica **coerente** com o derivado e o loader deixa de apagar golos/minutos por parte. **Zero-regressão:** o caminho `update` (edição manual, re-terminar) preserva contadores como antes; `editarEventosJogoAoVivo` mantém a preservação de eventos clássicos (o `deleteMany` só filtra `TIPOS_AO_VIVO`). **Só backend** (`lib/actions/jogo-ao-vivo.ts`). **Testes:** `tests/jogo-ao-vivo-retroativo.test.ts` (+2 e reforço de 1: `terminarJogoAoVivo` grava snapshot completo no create — golo capturado ao vivo + `minutosPorParte=[20,20]`, e o `update` NÃO traz `golos`; `editarEventosJogoAoVivo` idem com invariante soma(partes)==total; a inserção retroativa persiste `minutosPorParte` não-vazio). `NODE_OPTIONS=--max-old-space-size=6144 npm run typecheck` limpo · `npm run lint` limpo · `npm run test` verde. **Não toca em auth.**
- **2026-09-20** — **§3.7, §8.11, §8.25.6, §10.4 (Editor de tempo por parte — camada de dados: `EstatisticaAtleta.minutosPorParte`)** — **Adicionada a derivação e persistência de minutos por parte, mantendo o total (`minutos`) como soma das partes.** **§3.7:** documentado o campo `EstatisticaAtleta.minutosPorParte Int[] @default([])` (índice 0 = Parte 1, ..., comprimento = `Jogo.numeroPartes`, 1..4); `minutos` = **soma** de `minutosPorParte` quando preenchido (retrocompatível: `[]` no legado). **§10.4:** o motor único (`derivarEstatisticas` / `combinarEstatisticasIniciais`, `lib/derivar-estatisticas.ts`, funções puras) passa a derivar, por atleta, os minutos por parte a partir dos intervalos `[ENTRADA, SAIDA]` ao segundo, dividindo-os pelos **limites de parte** (`INICIO_PARTE`/`FIM_PARTE`); arredondamento por **maior resto** garante `soma(minutosPorParte) === minutos` (total byte-idêntico ao motor de minutos). **Precedência** por atleta: **persistido > derivado > `[]`**; jogos legados (só blocos, sem eventos ao vivo) → `minutosPorParte = []`. **§8.11/§8.25.6:** a **correção manual** faz-se por **minutos absolutos por parte** e é a **verdade final** (*last-write-wins*): `guardarEstatisticas` persiste `minutosPorParte` e, quando `length > 0`, grava `minutos = soma(minutosPorParte)`; vazio → mantém o comportamento legado. O **loader** (`app/(app)/jogos/[id]/page.tsx`) entrega ao editor `estatisticasIniciais[atletaId].minutosPorParte` (persistido/derivado) e o prop `numeroPartes={j.numeroPartes}`. **Contrato para o frontend:** `estatisticasIniciais[id].minutosPorParte: number[]` + `numeroPartes: number`. **Migração:** `prisma/migrations/20260920120000_add_minutos_por_parte_estatistica` (coluna `INTEGER[] DEFAULT ARRAY[]::INTEGER[]`). **Só backend/dados** (`lib/derivar-estatisticas.ts`, `lib/actions/jogos.ts`, `app/(app)/jogos/[id]/page.tsx`; **não toca em `components/*`**, que é do frontend em paralelo). **Testes:** `tests/eventos-para-estatisticas.test.ts` (derivação por parte 2–4 partes, atleta numa só parte, atleta em várias partes, invariante soma==total por maior resto, legado → `[]`, precedência persistido > derivado) e `tests/actions-producao.test.ts` (`guardarEstatisticas` persiste `minutosPorParte` e grava `minutos = soma`; vazio mantém legado). **Não toca em auth.**
- **2026-09-20** — **§10.4, §8.25.6 (fix de integridade — perda silenciosa de minutos legados por blocos ao gravar o editor retroativo)** — **Corrigido um achado de QA (severidade MÉDIA): gravar o editor de tempos de jogo num jogo legado cuja utilização foi registada por blocos apagava os minutos derivados dos blocos.** **Causa:** `persistirMinutos` (`lib/actions/jogo-ao-vivo.ts`), chamado por `terminarJogoAoVivo` e `editarEventosJogoAoVivo`, derivava minutos lendo **apenas** os `TIPOS_AO_VIVO` (`INICIO_PARTE`/`FIM_PARTE`/`ENTRADA`/`SAIDA`/`PAUSA`/`RETOMA`) e forçava `bloco:null` (mapper `paraEventoDerivacao`); depois fazia `upsert` de **todos** os convocados, escrevendo `minutos:null` para quem não tinha intervalos ao vivo. Como o `deleteMany` do editor só filtra `TIPOS_AO_VIVO`, os eventos `SUBSTITUICAO`/`blocoTempo` **sobreviviam** mas eram ignorados na persistência — havia assimetria entre o que o loader mostrava (lê **todos** os eventos, incl. blocos) e o que se gravava. **Opção escolhida: (c) paridade total com o loader.** `persistirMinutos` passa a **ler o registo COMPLETO do jogo dentro da transação** (com `bloco`/`minuto`/`atletaSecundarioId`/`segundoJogo`/`parte`) e a derivá-lo com o motor único (§10.4), tal como `combinarEstatisticasIniciais` faz no loader. Assim os minutos de blocos legados são **re-derivados** pela mesma precedência (**intervalos > blocos > null**) e nunca sobrescritos por `null`. Preferiu-se (c) a (b) («não escrever null por cima de valor existente») porque (b) preservaria valores obsoletos quando o treinador remove de facto a participação de um atleta, reintroduzindo assimetria loader↔persistido; (c) garante que o persistido é **sempre** igual ao derivado/mostrado. Removidos o mapper `paraEventoDerivacao` e as leituras `eventosFinais` (dead code); a assinatura de `persistirMinutos` deixa de receber a lista de eventos. Doc-comment de `terminarJogoAoVivo` corrigido (dizia «só nos registos existentes, RN-JV-13» — o comportamento real desde 2026-09-19 faz upsert de todos os convocados). **Zero-regressão:** os caminhos ao vivo (sem blocos) produzem os mesmos minutos/utilização de antes. **Testes:** `tests/jogo-ao-vivo-retroativo.test.ts` (+1: jogo legado só com blocos → gravar o editor retroativo preserva os minutos de bloco, não escreve `null`). **Só backend** (`lib/actions/jogo-ao-vivo.ts`). `NODE_OPTIONS=--max-old-space-size=6144 npm run typecheck` limpo · `npm run lint` limpo · `npm run test` verde (**1641 testes**, 77 ficheiros). **Não toca em auth.**
- **2026-09-20** — **§8.11, §8.25.1, §8.25.6, §8.25.7, §8.25.9, §8.25.10, §10.4 (Fases C e D — UI: unificação do registo de jogo na vista de Estatísticas)** — **Reorganização da UI do detalhe do jogo em cima da camada de dados já aterrada (Fase C-dados). Fecha a RN-JV-15.** **Fase C — UI (`components/jogos/JogoDetalhe.tsx`):** **(1)** removido o separador **«Ao Vivo»** (a `TabsTrigger` e o `TabsContent value="aovivo"` com o `RegistoAoVivo`) — o detalhe do jogo passa a ter **3 separadores** (Convocatória · Estatísticas · Análise). **(2)** o CTA **«Modo Jogo ao Vivo»** (entrada em `/jogos/[id]/ao-vivo`) foi **reposicionado para o topo do separador Convocatória**, mantendo a lógica de visibilidade (`mostrarModoAoVivo` — só com convocatória gravada e jogo não terminado). **(3)** removido o botão **«Preencher do registo ao vivo»** e o handler `importarDeEventos` (+ transição `pendingPreview`/`startPreview`) — a derivação é automática ao carregar (Fase C-dados). **(4)** removidos imports/props mortos: `RegistoAoVivo`, `previewEstatisticasDeEventos`, ícone `Wand2` e as props `casaFora`/`adversario`/`clubeNome` (só usadas pelo `RegistoAoVivo`; também removidas do call-site em `app/(app)/jogos/[id]/page.tsx`). O ficheiro `components/jogos/RegistoAoVivo.tsx` **mantém-se** mas deixa de ser referenciado pela navegação. **Fase D — UI (§8.25.6, RN-JV-15):** **(5)** o **editor de tempo de jogo** (`EditorManualMinutos`) passa a estar **sempre disponível** na tab Estatísticas: com sessão do Modo Jogo ao Vivo terminada usa as linhas derivadas ao segundo (título «Editar minutos (Modo Jogo ao Vivo)»); **sem sessão**, é **semeado a partir da convocatória gravada** (uma linha por convocado, entrada/saída vazias, título «Tempo de jogo»), permitindo preencher minutos à mão sem nunca ter ligado o modo ao vivo. Novo prop opcional `titulo` em `EditorManualMinutos`. **(6)** golos e restantes estatísticas continuam a preencher-se/corrigir-se na grelha de Estatísticas (já editável e pré-preenchida com o derivado). A gravação dos minutos usa a action existente `editarEventosJogoAoVivo` (persistência retroativa a garantir pelo backend). **UX:** o CTA fica no topo da Convocatória (fluxo natural: definir convocatória → abrir modo jogo); o editor de tempo de jogo fica no topo das Estatísticas, acima da grelha por atleta; alvos ≥44px preservados; PT-PT. **Pendente do backend:** remover `previewEstatisticasDeEventos` de `lib/actions/jogos.ts` (já sem referências na UI). **Verificação:** `NODE_OPTIONS=--max-old-space-size=6144 npm run typecheck` limpo · `npm run lint` limpo · `npm run test` verde (**1637 testes**, 76 ficheiros). **Não toca em auth.**
- **2026-09-20** — **§8.11, §10.4 (Fase C — camada de dados: Estatísticas como vista consolidada única, derivação automática dos eventos ao carregar)** — **A vista de Estatísticas passa a carregar já com os valores derivados dos eventos (via `derivarEstatisticas`), servindo de valores iniciais da grelha; a edição manual persistida em `EstatisticaAtleta` prevalece por atleta (*last-write-wins*, §13.4). Substitui a ação manual do botão «Preencher do registo ao vivo» por derivação automática.** **(1) Motor (`lib/derivar-estatisticas.ts`):** nova função pura **`combinarEstatisticasIniciais(eventos, convocados, persistidas, eFutebol, formato)`** — deriva tudo com `derivarEstatisticas` e **sobrepõe** integralmente os registos persistidos por atleta (preserva `valoresMetricas`). **(2) Loader (`app/(app)/jogos/[id]/page.tsx`):** `estatisticasIniciais` passa a ser o resultado de `combinarEstatisticasIniciais` sobre o registo completo (clássico + Modo Jogo ao Vivo) e sobre `j.estatisticas` (persistidas); expõe também `atletasComStatsPersistidas` (atletas com edição manual efetivamente guardada). **(3) `JogoDetalhe.tsx`:** nova prop `atletasComStatsPersistidas` — a confirmação de «remover convocado com estatísticas» (§22.4) passa a disparar **apenas** para os persistidos (não para os que só têm valores derivados), evitando falso-positivo; o botão «Preencher do registo ao vivo» fica **marcado para remoção** (redundante com a derivação automática) — a sua remoção e a reorganização da UI (remover a tab «Ao Vivo», reposicionar o CTA «Modo Jogo ao Vivo») ficam para a fase seguinte do frontend. **(4) Secundários por atleta:** confirmado que `derivarEstatisticas` já deriva todos os secundários (remates, cantos, faltas, desarmes, defesas, foras-de-jogo). **Timeout:** `TIMEOUT` não tem campo por atleta — mantém-se como `EventoJogo` ao nível do jogo/equipa (cronologia), sem perda de dado. `guardarEstatisticas` mantém-se como a verdade final. **Testes:** `tests/eventos-para-estatisticas.test.ts` (+6: sem persistidas bate com `derivarEstatisticas`/preview; edição manual sobrepõe-se por atleta; `valoresMetricas` preservados; zero-regressão de jogo legado só com blocos; secundários por atleta em futebol e em futsal). `npm run typecheck` limpo · `npm run lint` limpo · `npm run test` verde. **Não toca em auth.**

Do mais recente para o mais antigo.

- **2026-09-20** — **§8.25.6, RN-JV-15 (Fase D do backend — inserção retroativa de tempos de jogo sem sessão ao vivo prévia)** — **Fecho da parte de backend da unificação do registo de jogo: preencher/corrigir os tempos de jogo à mão passa a funcionar mesmo quando o Modo Jogo ao Vivo nunca foi ligado.** **(1) `editarEventosJogoAoVivo` (`lib/actions/jogo-ao-vivo.ts`):** confirmado que a action **não pressupõe** uma `SessaoJogoAoVivo` prévia (`aceder` nunca falha por sessão nula) e que o `deleteMany` filtra apenas `TIPOS_AO_VIVO` (`INICIO_PARTE`/`FIM_PARTE`/`ENTRADA`/`SAIDA`/`PAUSA`/`RETOMA`), pelo que os **eventos clássicos** (golos, cartões, assistências…) **não são apagados** pela edição retroativa. Ajuste: quando **nunca existiu** sessão e há eventos inseridos, a action passa a **materializar uma `SessaoJogoAoVivo` mínima em estado `TERMINADO`** (nº de partes herdado de `Jogo.numeroPartes`; `parteAtual` e `segundosDecorridos` — maior `FIM_PARTE`, senão maior segundo — derivados dos eventos), fechando a **RN-JV-15** e permitindo ao editor reabrir pré-preenchido (§8.25.6). Uma **sessão já existente** (ex.: terminada ao vivo) é **preservada intacta** — a edição continua disponível sem a alterar. Os minutos são persistidos pelo **motor único** (§10.4) via `persistirMinutos` (upsert de **todos os convocados**, RN-JV-13 alargada), garantindo **convergência** exata com o caminho ao vivo para os mesmos intervalos. Doc-comment da action corrigido (estava desatualizado desde a Fase A). **Testes:** `tests/jogo-ao-vivo-retroativo.test.ts` (novo — 6: inserção retroativa sem sessão persiste minutos/utilização com upsert de todos os convocados; cria sessão mínima TERMINADO; delete só toca no cronómetro — golos/cartões preservados; limpeza total não cria sessão mas normaliza a grelha; convergência à mão vs terminado ao vivo com minutos idênticos; não-regressão com sessão terminada existente). `npm run typecheck` limpo · `npm run lint` limpo · **testes verdes**. **Não toca em auth.**
- **2026-09-20** — **§8.25.3, §8.25.7, §10.4 (Fase B do backend — captura de golos/assistências/cartões no Modo Jogo ao Vivo, placar coerente e agregação)** — **Fecho da parte de backend da unificação do registo de jogo: o Modo Jogo ao Vivo passa a persistir golos/assistências/disciplina (além dos minutos/substituições), reutilizando `EventoJogo` e o motor único da Fase A.** **(1) Schemas (`lib/schemas/jogo-ao-vivo.ts`):** novos conjuntos `TIPOS_EVENTO_CAPTURA_AO_VIVO` (`GOLO`/`GOLO_SOFRIDO`/`ASSISTENCIA`/`CARTAO_AMARELO`/`CARTAO_VERMELHO`) e `TIPOS_EVENTO_SYNC` (cronómetro + captura); o `eventoAoVivoClienteSchema` aceita os novos tipos e um campo **`atletaSecundarioId`** (assistente, só usado no `GOLO`); a validação exige `atletaId` em `ENTRADA`/`SAIDA`/`ASSISTENCIA`/`CARTAO_*` e permite-o a `null` em `GOLO`/`GOLO_SOFRIDO` (golo sem autor / sem GR) — modelo simples por toque (decisão do supervisor). **(2) `sincronizarJogoAoVivo` (`lib/actions/jogo-ao-vivo.ts`):** persiste os eventos de captura (idempotente por `clientEventoId`, `createMany`+`skipDuplicates`); um `GOLO` com `atletaSecundarioId` guarda o assistente no golo (rasto) **e materializa um evento `ASSISTENCIA` autónomo** (`clientEventoId` derivado `…::assist`) — é este que o motor único contabiliza (ignora `atletaSecundarioId`, sem dupla contagem); quando o lote inclui `GOLO`/`GOLO_SOFRIDO`, **recalcula o placar**. **(3) Placar partilhado (`lib/placar-jogo.ts`):** `recalcularResultadoJogo(tx, jogoId)` foi extraído de `lib/actions/jogos.ts` para um módulo partilhado (fora de `"use server"`), agora reutilizado por `registarEventoJogo`/`removerEventoJogo` **e** por `sincronizarJogoAoVivo`. **(4) Agregação (§10.4):** confirmado que a fase de contagem do motor único **não filtra por `segundoJogo`**, pelo que os golos/assistências/cartões capturados ao vivo entram nos contadores por atleta de `EstatisticaAtleta` pelo caminho normal (`previewEstatisticasDeEventos` → grelha), tal como os do registo clássico. `terminarJogoAoVivo`/`editarEventosJogoAoVivo` mantêm-se focados em minutos/utilização (contadores preservados) — sem regressão. **Testes:** `tests/eventos-para-estatisticas.test.ts` (+2: captura com `segundoJogo` conta por atleta e no placar sem afetar minutos; golo sem autor só no placar), `tests/jogo-ao-vivo-captura.test.ts` (novo — 7: split golo+assistência, sem assistência não materializa, recálculo de placar só com golos, cronómetro não mexe no placar, cartão exige atleta, idempotência de `clientEventoId`, `recalcularResultadoJogo` coerente), `tests/jogos.test.ts` (+1: `registarEventoJogo` recalcula placar via helper partilhado). **Contrato do evento de captura (para o frontend):** `{ tipo, segundoJogo, atletaId?, atletaSecundarioId?, parte?, posicao?, clientEventoId }`. `npm run typecheck` limpo · `npm run lint` limpo · **1631 testes verdes** (76 ficheiros). **Não toca em auth.**
- **2026-09-20** — **§8.25.3 (Fase B do frontend — captura de golos/assistências/cartões por toque, tempo de jogo por jogador, arranque pré-carregado e seletor de nº de partes)** — **Fecho da parte de UI da unificação do registo de jogo no Modo Jogo ao Vivo (a entrega anterior de nº de partes deixara a UI para o frontend-specialist).** **(1) Registo desportivo por toque (`components/jogos/ao-vivo/`):** novo componente **`ModalAcoesJogador.tsx`** — o *tap* num atleta em campo abre um *bottom sheet* com **Golo** (com sub-fluxo de escolha de **assistente** entre os outros jogadores em campo, ou «Sem assistência»), **Cartão amarelo/vermelho** e **Substituir** (delega no `ModalSubstituicao` existente). O orquestrador **`JogoAoVivo.tsx`** ganha `registarGolo`/`registarGoloSofrido`/`registarCartao`, que criam eventos `GOLO` (com `atletaSecundarioId` para a assistência, embutida — motor único da Fase A), `GOLO_SOFRIDO` (botão dedicado, sem atleta obrigatório), `CARTAO_AMARELO`/`CARTAO_VERMELHO`, todos com `parte`/segundo correntes, entrando na **mesma *outbox*** append-only e sincronizando pelo caminho normal (`sincronizarJogoAoVivo`). Adicionado **marcador ao vivo** derivado da contagem local de `GOLO`/`GOLO_SOFRIDO`. **(2) Tempo de jogo por jogador:** cada cartão (em campo e banco) mostra a legenda de minutos acumulados, atualizada ao segundo (via `calcularMinutosDeEventos` + tick de 1s já existentes); o banco passa a mostrar sempre os minutos (antes só se >0). **(3) Arranque pré-carregado (`ArranqueAoVivo.tsx` + `app/(app)/jogos/[id]/ao-vivo/page.tsx`):** a seleção de titulares/posições vem pré-preenchida do **plano tático** do jogo (`Convocatoria.titularPrevisto`/`posicaoPrevista`), com *fallback* ao pool de convocados; editável antes de iniciar. **(4) Nº de partes na UI:** `components/jogos/JogoForm.tsx` ganha o **seletor «Número de partes» (1–4)** na criação/edição (envia `numeroPartes` no payload); `ArranqueAoVivo` **deixa de escolher** o nº de partes (removidos o `useState`, o seletor 2–4 e o parâmetro de `onIniciar`); `JogoAoVivo`/página passam a usar **`Jogo.numeroPartes`** (prop `numeroPartes`, antes `numeroPartesSugerido` derivado do escalão). **(5) Tipos locais (`lib/jogo-ao-vivo-local.ts`):** novo `TipoEventoRegisto` (`GOLO`/`GOLO_SOFRIDO`/`CARTAO_AMARELO`/`CARTAO_VERMELHO`) unido em `TipoEventoLocal`; `EventoLocal` ganha `atletaSecundarioId?`. O adaptador `components/jogos/ao-vivo/acoes.ts::paraEventoCliente` mapeia estes eventos (incl. `atletaSecundarioId`) para o input da action de sync (camada de adaptação local→wire). UI 100% pt-PT, mobile-first, alvos de toque ≥44px. `npm run typecheck` limpo · `npm run lint` limpo · **1621 testes verdes** (75 ficheiros). **Não toca em auth.**

- **2026-09-20** — **§9, §8.11, §10.x, §8.25 (feature — título do confronto com o nome do clube a negrito e na cor do clube)** — **Sempre que um jogo é apresentado como confronto na UI, o nome da nossa equipa passa a ser o `Clube.nome` (nunca "Nós" nem vazio), a negrito e na cor do clube, respeitando a ordem casa–fora.** Antes, o título mostrava apenas `vs <Adversário>` (a nossa equipa era implícita) e o marcador do registo ao vivo mostrava literalmente "Nós". **(1) Fonte única — novos ficheiros:** `lib/jogo-confronto.ts` (helpers puros `nomeClubeSeguro`, `ordemConfronto`, `tituloConfronto(clubeNome, adversario, casaFora)` → string "A vs B" na ordem casa–fora; recurso `NOME_EQUIPA_FALLBACK = "A nossa equipa"` quando o nome falta) e `components/jogos/TituloConfronto.tsx` (componente sem estado, usável em Server e Client Components; o nome do clube a `font-bold` + `color: var(--cor-primaria)`; prop **`aplicarCor`** — default `true` — a passar `false` sobre fundos que já usam a cor do clube, mantendo só o negrito para não perder contraste). **(2) Detalhe do jogo (`app/(app)/jogos/[id]/page.tsx`):** `<h1>` passa a usar `TituloConfronto` (colorido; `clubeNome` de `membro.clube.nome`, já disponível); breadcrumb usa `tituloConfronto` (string). **(3) Lista de jogos (`app/(app)/jogos/page.tsx`):** cada item usa `TituloConfronto` (colorido; `membro.clube.nome`). **(4) Dashboard (`app/(app)/dashboard/page.tsx`):** cartão-herói do próximo jogo e sublista de próximos jogos usam `TituloConfronto` com **`aplicarCor={false}`** (fundo `hero-card` = cor do clube); o widget `ProximosJogosSecundario` (fundo branco) usa a versão colorida — ganhou props `casaFora` (no tipo de `jogos`) e `clubeNome` (de `clube.nome`, via `obterClubeAtivo`). **(5) Registo ao vivo (`components/jogos/RegistoAoVivo.tsx` + `JogoDetalhe.tsx`):** o marcador deixa de mostrar "Nós" e mostra o nome do clube (novo prop `clubeNome`, encadeado por `JogoDetalhe`); a nossa equipa fica a negrito + cor do clube (fundo `bg-cinza-50`, contraste ok). **(6) Modo Jogo ao Vivo (`components/jogos/ao-vivo/JogoAoVivo.tsx` + `app/(app)/jogos/[id]/ao-vivo/page.tsx`):** cabeçalho do overlay (`bg-ink` escuro) usa `TituloConfronto` com `aplicarCor={false}`; a página passa `clubeNome` via `obterClubeAtivo`. **(7) Rótulos de seleção de jogo:** `app/(app)/vitoria-rapida/page.tsx` (novo `obterClubeAtivo`) e `app/(app)/comunicacoes/gerar/page.tsx` (usa `membro.clube.nome`) passam a compor o rótulo do dropdown com `tituloConfronto`. **§9** ganha a regra transversal «Título do confronto (apresentação)». **Fora de âmbito (deliberado):** textos externos/notificações mantêm o formato compacto `vs <Adversário>` já especificado e com testes que o fixam — `lib/actions/agenda.ts` (título na agenda), `lib/comunicacao-utils.ts` (mensagens), `lib/google-calendar.ts` (evento), `lib/dashboard-lembretes.ts` (lembretes), `lib/actions/analise.ts` (feed de atividade) e o cartão social (`app/api/social/card/route.tsx`) — não são o título de confronto na UI. `npm run typecheck` limpo · `npm run lint` limpo · **1621 testes verdes** (75 ficheiros). **Não toca em auth.**
- **2026-09-20** — **§3.7, §8.25.3, §8.25.8, §8.25.10 (feature — nº de partes configurável no jogo)** — **O nº de partes deixa de ser escolhido no arranque do Modo Jogo ao Vivo e passa a ser definido na criação/edição do jogo (`Jogo.numeroPartes`, 1..4); a `SessaoJogoAoVivo` herda-o ao iniciar.** **(1) Schema/BD (`prisma/schema.prisma`):** nova coluna **`numeroPartes Int @default(2)`** no modelo `Jogo`. Migração **`prisma/migrations/20260919140000_add_numero_partes_jogo/migration.sql`** (`ALTER TABLE "Jogo" ADD COLUMN "numeroPartes" INTEGER NOT NULL DEFAULT 2;`) — **criada e aplicada** à BD (`prisma migrate deploy`) + `prisma generate`. Jogos existentes ficam com o default 2. **(2) Zod (`lib/schemas/jogo.ts`):** `jogoSchema` (criar/editar) ganha **`numeroPartes: z.number().int().min(1).max(4).default(2)`**. **(3) Actions de jogo (`lib/actions/jogos.ts`):** `criarJogo` e `atualizarJogo` passam a persistir `numeroPartes`. **(4) Modo Jogo ao Vivo (`lib/actions/jogo-ao-vivo.ts`, `lib/schemas/jogo-ao-vivo.ts`):** `iniciarJogoAoVivo` deixa de receber o nº de partes como fonte — a `SessaoJogoAoVivo` **herda `Jogo.numeroPartes`** ao criar/reiniciar; o `iniciarJogoAoVivoSchema` mantém `numeroPartes` **opcional** (1..4, antes 2..4) só por retrocompatibilidade do input, mas o valor do jogo prevalece. **RN-JV-5** alargada de **2–4** para **1–4 partes**. **UI não alterada** nesta entrega (o formulário de jogo e o `ArranqueAoVivo` ficam para o `frontend-specialist`: adicionar o seletor de nº de partes na criação/edição do jogo e remover a escolha de partes do `ArranqueAoVivo`, que hoje força `Math.min(4, Math.max(2, …))` e um seletor 2–4). `npm run typecheck` limpo · **1621 testes verdes** (75 ficheiros). **Não toca em auth.**
- **2026-09-19** — **§3.7, §10.4, §8.25.9 (Fase A — correção do bug P0: motor único de derivação de minutos)** — **Colapsados os dois motores de cálculo de minutos num só derivador puro, corrigindo a corrupção de dados em que «Preencher do registo ao vivo» sobrescrevia com `null`/blocos grosseiros os minutos precisos gravados pela condução ao vivo.** Contexto do P0: coexistiam dois derivadores que se ignoravam — `lib/minutos-jogo.ts::calcularMinutosDeEventos` (intervalos ao segundo, `segundoJogo`, usado pelas actions ao vivo) e `lib/eventos-para-estatisticas.ts::derivarEstatisticasDeEventos` (blocos, `blocoTempo`, usado por `previewEstatisticasDeEventos`). Um jogo conduzido ao vivo só tem `segundoJogo` (não tem `bloco`), pelo que o motor de blocos devolvia `minutos: null` e o «Guardar» apagava os minutos do cronómetro. **(1) Novo motor único `lib/derivar-estatisticas.ts`:** função pura `derivarEstatisticas(eventos, convocados, eFutebol, formato)` que entende **intervalos E blocos** e devolve `{ estatisticas: Map<atletaId, EstatisticaInput>, golosMarcados, golosSofridos }`. Contadores idênticos ao motor anterior (zero-regressão); minutos/utilização pela **regra de precedência confirmada — intervalos (cronómetro ao segundo) > blocos (legado) > vazio/null** — reutilizando `calcularMinutosDeEventos` como primitiva de intervalos e derivando o segundo final internamente (maior `FIM_PARTE`, senão maior `segundoJogo`). A edição manual na grelha continua a sobrepor-se a tudo (last-write-wins, §13.4). **`lib/eventos-para-estatisticas.ts` foi removido** (consolidado no novo módulo). **(2) `lib/actions/jogos.ts`:** `previewEstatisticasDeEventos` passa a ler também `segundoJogo`/`parte` e a usar `derivarEstatisticas` — o botão «Preencher do registo ao vivo» devolve agora os minutos precisos. **(3) `lib/actions/jogo-ao-vivo.ts`:** `terminarJogoAoVivo` e `editarEventosJogoAoVivo` convergem no mesmo motor via `persistirMinutos`, que passa a fazer **`upsert` de `EstatisticaAtleta` para todos os convocados** (RN-JV-13 alargada, decisão do supervisor) — não perde os minutos ao vivo quando a grelha nunca foi aberta — escrevendo apenas `minutos`/`utilizacao` (contadores da grelha preservados). Removido o derivador local `derivarMinutosEUtilizacao` (redundante). **(4) Bíblia:** §3.7 `EventoJogo` alinhado com o schema real (`segundoJogo`/`posicao`/`clientEventoId`, `@@unique([jogoId, clientEventoId])`, `@@index([jogoId, segundoJogo])`, `TipoEventoJogo` com `INICIO_PARTE/FIM_PARTE/ENTRADA/SAIDA/PAUSA/RETOMA`, `parte` 1..4); §10.4 descreve o motor único, a convergência intervalos+blocos e a precedência. **Testes:** `tests/eventos-para-estatisticas.test.ts` (motor único: baseline legado de blocos preservado + intervalos ao vivo + precedência + convergência + não-sobrescrita P0), `tests/minutos-jogo.test.ts` (+2: 4 partes, PAUSA/RETOMA sem efeito) e `tests/jogos.test.ts` (+3: preview com minutos precisos do cronómetro, zero-regressão de blocos, gate de capacidade). **Sem alteração de schema/migração** (colunas já existiam). `npm run typecheck` limpo · `npm run lint` limpo · **1621 testes verdes** (75 ficheiros). **Não toca em auth.**
- **2026-09-19** — **§8.10, §8.11, §11.5 (feature — plano de jogo: capitão de equipa)** — **Designação de capitão de equipa no plano de jogo, com no máximo um capitão por jogo.** **(1) Schema/BD (`prisma/schema.prisma`):** novo campo **`capitao Boolean @default(false)`** no modelo `Convocatoria`. A unicidade (0 ou 1 capitão por jogo) é garantida pela **lógica da action**, não por constraint de BD. Migração **`prisma/migrations/20260919130000_add_capitao_convocado/migration.sql`** (`ALTER TABLE "Convocatoria" ADD COLUMN "capitao" BOOLEAN NOT NULL DEFAULT false;`) — **criada e aplicada** à BD (`prisma migrate deploy`) + `prisma generate`. **(2) Zod (`lib/schemas/jogo.ts`):** novo `definirCapitaoSchema` (`{ atletaId: cuid().nullable() }`). **(3) Server action (`lib/actions/jogos.ts`):** nova **`definirCapitao(jogoId, atletaId|null)`** no padrão §7.1 (`obterClubeIdAtual` → jogo isolado por clube → capacidade `CONVOCATORIA_GERIR` → validação Zod → `revalidatePath`): faz `updateMany` de `capitao=false` a todos os convocados do jogo e, quando `atletaId` não é null, `update` de `capitao=true` no indicado (rejeita atleta não convocado); `atletaId=null` apenas limpa (toggle off). Tudo em transação. **(4) Diagrama/render (`lib/schemas/exercicio.ts`, `lib/formacao.ts`, `components/campo/desenho.tsx`):** novo campo **opcional** `capitao` (boolean) no `jogadorSchema` (retrocompatível — só se mantém quando true); `TitularFormacao` ganha `capitao`; `construirDiagramaFormacao` marca `capitao:true` só no token do capitão; `ElementoSVG` desenha uma **braçadeira «C»** (disco âmbar `#F5C542` com contorno escuro) no ombro esquerdo do titular capitão. `sobreporNomesTitulares` passa a sobrepor **identidade completa** (nome **e** capitão) — injeta o «C» vivo mesmo num quadro **já gravado** e **limpa** um «C» gravado obsoleto; mantém o *no-op* por referência quando nada muda. **(5) UI (`components/jogos/PlanoTatico.tsx`, `JogoDetalhe.tsx`, `app/(app)/jogos/[id]/page.tsx`):** na lista do plano, cada **titular** tem um botão de braçadeira «C» (âmbar preenchido quando capitão, outline cinza caso contrário; `aria-pressed`, alvo ≥44px) — suplentes não o têm; despromover um capitão a suplente limpa a braçadeira. Estado com *update otimista* + reversão em erro; `capitaoInicial` derivado das convocatórias e passado até ao `PlanoTatico`. Persiste de imediato, independente do «Guardar plano». **Testes:** `tests/jogos-f5-actions.test.ts` (+6: isolamento por clube, capacidade, rejeição de não-convocado, designar com `updateMany`+`update`, toggle off sem `update`) e `tests/formacao.test.ts` (+4: `capitao` no diagrama só no capitão; sobreposição/limpeza em quadro gravado). `npm run typecheck` limpo · `npm run lint` limpo · **1603 testes verdes** (75 ficheiros). **Não toca em auth.**
- **2026-09-19** — **§11.5 (Correção — plano de jogo: nome do atleta não aparecia em quadro já gravado)** — **O nome do atleta por cima da figura (entrada de changelog anterior, mesma data) não aparecia em runtime assim que existia um quadro tático gravado.** Diagnóstico: o schema, a derivação (`construirDiagramaFormacao` preenche `nomeAtleta`) e o render (`ElementoSVG` desenha o `<text>`) estavam corretos — confirmado por render ponta-a-ponta de `EditorCampo`/`CampoDesenho`. A falha estava no `QuadroTaticoJogo` (`components/jogos/QuadroTaticoJogo.tsx`): a base visível era `gravado ?? diagramaFormacao`, pelo que, **depois da primeira gravação do quadro**, a formação viva (única portadora dos nomes) ficava **sombreada** e os tokens gravados não traziam `nomeAtleta` — o nome só aparecia enquanto o quadro nunca tivesse sido guardado (o caso raro). Como o nome é uma sobreposição de **identidade** (que atleta é o token), independente da posição no campo, tem de ser aplicado **também** ao diagrama gravado. **Correção (só apresentação/derivação — sem migração, dados de negócio ou auth):** nova função pura e testável **`sobreporNomesTitulares(alvo, fonte)`** em `lib/formacao.ts` que injeta os nomes vivos (lidos de `fonte`, a formação derivada dos titulares atuais) nos tokens de jogador de `alvo` por `id` de token; tokens sem correspondência (adversários, jogadores extra) ficam intactos; sem nomes na fonte devolve `alvo` inalterado (no-op). O `QuadroTaticoJogo` passa a calcular `baseVisivel = sobreporNomesTitulares(gravado ?? diagramaFormacao, diagramaFormacao)`, cobrindo de forma consistente o render só-de-leitura (`CampoDesenho`), a semente do editor e o *snapshot* de cancelar. Na formação viva é um no-op (já traz os nomes); num quadro gravado, injeta-os. As **posições** gravadas não são tocadas (só a identidade/nome). Novos testes em `tests/formacao.test.ts` (injeção em quadro gravado sem nomes sem alterar posições; tokens sem correspondência intactos; no-op sem nomes). `npm run typecheck` limpo · `npm run lint` limpo · **1593 testes verdes** (75 ficheiros). **Não toca em auth.**
- **2026-09-19** — **§8.25.1–§8.25.6 (feature — Modo Jogo ao Vivo: UI de condução)** — **Implementação do ecrã dedicado `/jogos/[id]/ao-vivo` e do editor manual de minutos, ligados às server actions de §8.25.9.** Nova rota **`app/(app)/jogos/[id]/ao-vivo/page.tsx`** (Server Component: carrega os convocados como *pool* via `obterJogo`/`listarAtletas`, deriva `tamanhoFormato`/`duracaoParteMins`/`numeroPartesSugerido`, hidrata a sessão existente a partir de `SessaoJogoAoVivo` + eventos ao vivo, lê `notasAoVivo` de `Jogo.relatorio`). O componente cliente renderiza-se como camada `fixed inset-0` (foco total, sem sidebar/bottom-nav; auth/licença/isolamento multi-tenant herdados do grupo `(app)` e de `obterJogo`). Novos componentes em **`components/jogos/ao-vivo/`**: `JogoAoVivo` (orquestrador — máquina de estados §8.25.3, tick de 1s), `ArranqueAoVivo` (seleção de titulares/posições + nº de partes 2–4, bloqueio RN-JV-1 com CTA à convocatória quando há convocados insuficientes), `CronometroJogo` (cronómetro contínuo `MM:SS` + «Parte N/N», cálculo local sem servidor §8.25.2), `CampoAoVivo` (em-campo/banco, substituição por *tap* RN-JV-3, minutos por atleta), `ModalSubstituicao`, `BarraControloJogo` (botões só do estado atual + *bottom sheet* de notas) e `EditorManualMinutos` (§8.25.6, tabela atleta|entrada|saída|minutos com recálculo → `editarEventosJogoAoVivo`). **Persistência offline-first** em **`lib/jogo-ao-vivo-local.ts`** (IndexedDB com *fallback* `localStorage`: `guardarEstadoLocal`/`obterEstadoLocal`/`limparEstadoLocal`; *outbox* append-only com `clientEventoId`; helpers `segundoCorrente`/`reconstruirEmCampo`/`eventosPendentes`). **Sincronização (RN-JV-11):** o estado local é autoritário durante o jogo; havendo rede, a *outbox* cria a sessão no servidor uma só vez (`iniciarJogoAoVivo` — as ENTRADA dos titulares no segundo 0 não são reenviadas, evitando duplicados) e faz *upsert* idempotente por `clientEventoId` (`sincronizarJogoAoVivo`); ao terminar, `terminarJogoAoVivo` regista as SAIDA finais e persiste minutos/utilização em `EstatisticaAtleta`. Botão **«Modo Jogo ao Vivo»** adicionado ao separador *Ao Vivo* do `JogoDetalhe` (só com o jogo não terminado) e o `EditorManualMinutos` surge nas Estatísticas depois de a sessão terminar. UI 100% pt-PT, alvos de toque ≥44px, acento `--cor-primaria` do clube, mobile-first. `npm run typecheck` limpo · `npm run lint` limpo · **testes verdes** (1590/1590). **Nota:** o *round-trip* cliente↔servidor da condução ao vivo (fluxo offline→online completo) foi implementado segundo os contratos das actions mas **não foi validado em integração com base de dados** (sem BD no ambiente) — recomenda-se cobertura de QA/integração. **Não toca em auth.**

- **2026-09-19** — **§2, §3.5, §8.8.2 (decisão de produto — unificação da classificação de ausências)** — **Eliminado o sistema antigo `motivo`/`justificacao` (enum `MotivoFalta`); `tipoAusencia`/`notaAusencia` (enum `TipoAusencia`) passa a ser a fonte única.** Contexto: desde o wiring do `tipoAusencia` (entrada de changelog anterior, mesma data), a marcação de presenças mostrava **dois seletores lado a lado** em `FALTA`/`FALTA_JUSTIFICADA`/`LESIONADO` — o antigo «Motivo» (`MotivoFalta`: LESAO/DOENCA/OUTRO/SEM_JUSTIFICACAO/JOGO_FUTEBOL + texto livre `justificacao`) e o novo «Tipo de ausência» — o que confundia o utilizador. **Análise de impacto antes da decisão:** confirmado que `Presenca.motivo`/`justificacao` **não** eram consumidos em analytics (`lib/actions/analise.ts`), relatórios, exports, comunicações (`lib/comunicacao-*`) nem noutros ecrãs — apenas no marcador de presenças. Os `motivo`/`justificacao` de jogos/suspensões/comunicações são conceitos **distintos** (`MotivoSuspensao`, etc.) e ficam intactos. O valor `JOGO_FUTEBOL` (dupla modalidade) era **drift de código não documentado na spec** (as definições de enum de §3.5 e o glossário §2 nunca o incluíram) e **não** tinha consumidores em analytics. **Decisão:** manter o sistema novo (mais rico e geral — cobre `PESSOAL`/`TRABALHO` explicitamente) e **remover** o antigo. **Alterações de código:** `prisma/schema.prisma` (removidas as colunas `Presenca.motivo`/`justificacao` e o enum `MotivoFalta`); `lib/schemas/treino.ts` (removidos `MOTIVOS_FALTA`, `LABEL_MOTIVO_FALTA` e os campos `motivo`/`justificacao` do `presencaSchema`); `lib/actions/treinos.ts` (`marcarPresencas` deixa de gravar `motivo`/`justificacao`); `components/treinos/MarcadorPresencas.tsx` (removido o seletor de «Motivo», o texto livre associado, as funções `mudarMotivo`/`mudarJustificacao`, a constante `COM_JUSTIFICACAO` e a prop `praticaDuplaModalidade` — que só servia para filtrar `JOGO_FUTEBOL`); `lib/presencas.ts` (`RegistoPresenca` e `presencasAlteradas` deixam de incluir/comparar `motivo`/`justificacao`); `app/(app)/treinos/[id]/page.tsx` (deixa de hidratar/passar os campos antigos); `prisma/seed-rico.ts` (as presenças de ausência passam a semear `tipoAusencia` — `SEM_MOTIVO`/`DOENCA`/`LESAO` — em vez de `motivo`). **Migração:** criada `prisma/migrations/20260919120000_remove_motivo_falta_presenca/migration.sql` (`ALTER TABLE "Presenca" DROP COLUMN "motivo", DROP COLUMN "justificacao"; DROP TYPE "MotivoFalta";`) — **ficheiro criado, NÃO aplicado à BD** (a aplicar por `database-specialist`/ops; migração destrutiva de colunas, backfill opcional antes de aplicar). **Testes:** `tests/presencas.test.ts` e `tests/schemas.test.ts` alinhados (removidos os testes de `motivo`/`justificacao`; adicionada cobertura de `tipoAusencia` válido/inválido e rejeição em estado de comparência). `npm run typecheck` limpo · `npm run lint` limpo · **1590 testes verdes** (75 ficheiros). **Não toca em auth.**
- **2026-09-19** — **§11.5, §13.1 (Correção — plano de jogo: balizas de futsal + legenda de posição)** — **Balizas sempre visíveis no campo de futsal e legenda da posição tática nos titulares do plano de jogo.** Duas correções ao quadro tático do **plano de jogo** (§8.10), **só apresentação/derivação — sem alteração de migração, dados de negócio ou auth**. **(1) Balizas no fundo de futsal (`components/campo/desenho.tsx`, `FundoFutsal5`):** o fundo de futsal desenhava as áreas (quartos de círculo de 6 m) mas **não** as balizas; só os fundos de futebol as tinham. Passa a desenhar **duas balizas** (uma em cada extremidade, `<Baliza lado="esq|dir" altura={30}>` — baliza de futsal ≈ 3 m = 30 unidades no espaço 400×200), reutilizando o componente `Baliza` já existente (só `<line>`, logo **não altera** as contagens de `<circle>`/`<rect>`/`<path>` validadas em `tests/campo-fundos.test.ts`). Aparecem sempre, incluindo no plano de jogo. **(2) Legenda da posição tática (`lib/schemas/exercicio.ts`, `lib/formacao.ts`, `components/campo/desenho.tsx`):** novo campo **opcional** `etiquetaPosicao` (string ≤6) no `jogadorSchema` (retrocompatível — exercícios e diagramas antigos não o têm; **não** substitui o número, que continua no tronco). `construirDiagramaFormacao` passa a preencher `etiquetaPosicao` com a **abreviatura** (`ABREV_POSICAO` de `lib/schemas/atleta.ts`) da posição **efetiva** de cada titular (prevista ou padrão atribuída), e o render do jogador (`ElementoSVG`) desenha uma **pílula escura com a abreviatura por baixo dos pés** (ex.: «GR», «Ala», «Pivô»). O posicionamento por linha/coordenada da posição já existia; a legenda torna-o legível. Os campos extra são **preservados** ao arrastar no editor (o `EditorCampo` faz *spread* de `{...el, x, y}`), pelo que o ponto de partida da formação mantém a legenda e a **posição manual continua a prevalecer** após arrastar/gravar. `npm run typecheck` limpo · `npm run lint` limpo · **1590 testes verdes** (75 ficheiros). **Não toca em auth.**
- **2026-09-19** — **§11.5 (Melhoria — plano de jogo: nome do atleta por cima da figura)** — **Nome do atleta em letra pequena por cima da cabeça de cada titular no quadro tático do plano de jogo (§8.10).** Complemento à legenda de posição (entrada anterior, mesma data), **só apresentação/derivação — sem alteração de migração, dados de negócio ou auth**. **(1) Schema (`lib/schemas/exercicio.ts`):** novo campo **opcional** `nomeAtleta` (string ≤20) no `jogadorSchema`, a seguir a `etiquetaPosicao` (retrocompatível — exercícios e diagramas antigos não o têm; **não** substitui o número, que continua no tronco, nem a pílula de posição, que fica aos pés). **(2) `lib/formacao.ts`:** `TitularFormacao` ganha o campo opcional `nome`; `construirDiagramaFormacao` passa a preencher `nomeAtleta` com o **primeiro nome** do atleta truncado a 12 chars (`atleta.nome.split(" ")[0].slice(0, 12)`) para não transbordar a figura; ausente/vazio → sem nome. **(3) `components/campo/desenho.tsx` (`ElementoSVG`, `case "jogador"`):** quando `nomeAtleta` existe, desenha um `<text>` centrado **por cima da cabeça** (baseline em `headCy − headR − 3`, `fontSize` 4.6, branco com contorno escuro via `stroke`+`paintOrder` para contraste sobre o fundo do campo), sem interferir com o número (tronco) nem com a pílula de posição (pés). **(4) Wiring (`components/jogos/PlanoTatico.tsx`):** o `map` de titulares passa a incluir `nome: c.nome`. Retrocompatível (elementos sem `nomeAtleta` não renderizam nada extra) e preservado ao arrastar no editor (*spread* de `{...el, x, y}`). `npm run typecheck` limpo · `npm run lint` limpo · **1590 testes verdes** (75 ficheiros). **Não toca em auth.**
- **2026-09-19** — **§8.25.9, §8.25.10 (feature — Modo Jogo ao Vivo: server actions)** — **Implementação das 12 server actions do Modo Jogo ao Vivo.** Criado **`lib/actions/jogo-ao-vivo.ts`** com as 12 actions de §8.25.9, todas no padrão §7.1 (`"use server"` → validação Zod → `auth()`/clube → época ativa → capacidade `ESTATISTICAS_GERIR` no escalão → `Resultado<T>` → `revalidatePath`) e escritas multi-registo em transação: **`iniciarJogoAoVivo`** (cria/reinicia a `SessaoJogoAoVivo` `POR_INICIAR`, valida RN-JV-1 nº titulares = tamanho do formato e RN-JV-2 titulares convocados, regista ENTRADA dos titulares no segundo 0); **`iniciarParte`** (`POR_INICIAR`/`INTERVALO`→`EM_CURSO`, `INICIO_PARTE`, RN-JV-5 não ultrapassa `numeroPartes`); **`terminarParte`** (`EM_CURSO`→`INTERVALO`, `FIM_PARTE`, consolida `segundosDecorridos`); **`substituirEmCampo`** (`SAIDA`+`ENTRADA` no mesmo segundo, RN-JV-3/9/10/11/12); **`trocaEmBloco`** (várias substituições, validação atómica antes de aplicar, transação); **`pausarJogoAoVivo`**/**`retomarJogoAoVivo`** (`PAUSA`/`RETOMA`; `PAUSADO`/`EM_CURSO`; tempo pausado não conta, RN-JV-4); **`terminarJogoAoVivo`** (`SAIDA` no segundo final para quem está em campo + `FIM_PARTE`, calcula minutos via `calcularMinutosDeEventos` e faz *upsert* de `minutos`/`utilizacao` em `EstatisticaAtleta` só nos registos existentes, RN-JV-6/9/10/13); **`guardarNotaJogoAoVivo`** (chave `notasAoVivo` em `Jogo.relatorio`, merge sem perder secções); **`sincronizarJogoAoVivo`** (*upsert* idempotente da *outbox* por `clientEventoId` via `createMany`/`skipDuplicates`, RN-JV-8, + reconciliação de estado *last-write-wins*); **`editarEventosJogoAoVivo`** (substitui todos os eventos ao vivo e recalcula estatísticas, RN-JV-14/15); **`previewMinutosJogo`** (cálculo sem persistir, §8.25.5). Criado **`lib/schemas/jogo-ao-vivo.ts`** (schemas Zod partilhados: arranque, substituições, sync/edição de eventos com `clientEventoId` obrigatório no sync). Estendido **`lib/relatorio-jogo.ts`** com o campo `notasAoVivo` em `RelatorioEstruturado` (serialização byte-idêntica quando vazio → zero-regressão; preservado por `parseRelatorio`/`serializarRelatorio`). `npm run typecheck` limpo · `npm run lint` limpo · **1592 testes** verdes. **Não toca em auth.**
- **2026-09-19** — **§8.8.2, §2 (feature — presenças)** — **Conclusão do wiring de `tipoAusencia`/`notaAusencia` na marcação de presenças.** O commit `4864f7c` adicionou o enum `TipoAusencia` (`LESAO`/`DOENCA`/`PESSOAL`/`TRABALHO`/`SEM_MOTIVO`/`OUTRO`) e as colunas `Presenca.tipoAusencia`/`Presenca.notaAusencia` ao schema + a validação Zod (`presencaSchema` com `superRefine`, `lib/schemas/treino.ts`), mas a feature ficou **incompleta**: a action não persistia os campos e a UI não os recolhia. Concluído: **(1) `lib/actions/treinos.ts` (`marcarPresencas`):** o upsert (create+update) passa a gravar `tipoAusencia`/`notaAusencia`, limpando-os para `null` sempre que `estadoImplicaAusencia(estado)` é falso (nota livre também `trim`ada → `null` se vazia) — defesa em profundidade além do schema. **(2) `components/treinos/MarcadorPresencas.tsx`:** em qualquer estado de ausência (`FALTA`/`FALTA_JUSTIFICADA`/`LESIONADO`) surge um **seletor de tipo de ausência** (botões `LABEL_TIPO_AUSENCIA`, toggle, alvo ≥44px, `aria-pressed`) e um **campo de nota livre opcional** (≤200 chars); ambos limpam ao voltar a presente/atrasado, e ficam desativados/ocultos em sessão fechada (só-leitura). **(3) `lib/presencas.ts` (`RegistoPresenca`, `presencasAlteradas`):** passam a incluir/comparar `tipoAusencia` (via `??`) e `notaAusencia` (texto normalizado). **(4) `app/(app)/treinos/[id]/page.tsx`:** `presencasIniciais` passa a hidratar os novos campos. Convive com `motivo`/`justificacao` (persistem independentes; unificação num único campo fica em aberto como decisão de produto). **Testes:** `tests/presencas.test.ts` — **+4** (mudança de tipo de ausência, nota null/vazia/espaços equivalentes, mudança real de nota, campos ausentes ≡ null). `npm run typecheck` limpo · `npm run lint` limpo. **Não toca em auth.**
- **2026-09-19** — **§8.25.8 (migração — Modo Jogo ao Vivo)** — **Criação da migração em falta para o Modo Jogo ao Vivo (correção de drift schema↔BD).** O commit `4864f7c` introduziu no `prisma/schema.prisma` os deltas do Modo Jogo ao Vivo (entrada de changelog anterior, mesma data) **sem gerar a migração correspondente**, deixando a base de dados dessincronizada do schema. Criada a migração **`prisma/migrations/20260919000000_add_modo_jogo_ao_vivo/migration.sql`** que aplica exatamente esses deltas: `CREATE TYPE "EstadoJogoAoVivo"`; `ALTER TYPE "TipoEventoJogo" ADD VALUE` para `INICIO_PARTE`/`FIM_PARTE`/`ENTRADA`/`SAIDA`/`PAUSA`/`RETOMA`; `ALTER TABLE "EventoJogo"` a adicionar as colunas nullable `segundoJogo INTEGER`, `posicao "Posicao"`, `clientEventoId TEXT`; `CREATE TABLE "SessaoJogoAoVivo"` (1:1 com `Jogo`, PK `id`, `numeroPartes`/`duracaoParteMins`/`parteAtual`/`segundosDecorridos` com defaults, `aCorrerDesde` nullable, `criadoEm`/`atualizadoEm`); índices `SessaoJogoAoVivo_jogoId_key` (unique), `EventoJogo_jogoId_segundoJogo_idx` e `EventoJogo_clientEventoId_key` (unique em `[jogoId, clientEventoId]`); e a FK `SessaoJogoAoVivo_jogoId_fkey` para `Jogo(id)` com `ON DELETE CASCADE`. **NÃO** duplica os deltas de `TipoAusencia`/`Presenca` (já cobertos por `20260917000000_add_tipo_ausencia_presenca`). Migração aditiva, nullable e retrocompatível; SQL validado por `prisma migrate diff` contra o schema. **Não toca em auth.**
- **2026-09-19** — **§8.25.8 (schema — Modo Jogo ao Vivo)** — **Implementação dos deltas de schema do Modo Jogo ao Vivo + campo `duracaoParteMins`.** Aplicados ao `prisma/schema.prisma` os deltas aditivos de §8.25.8: novo enum **`EstadoJogoAoVivo`** (`POR_INICIAR`/`EM_CURSO`/`INTERVALO`/`PAUSADO`/`TERMINADO`); extensão de **`TipoEventoJogo`** (`INICIO_PARTE`, `FIM_PARTE`, `ENTRADA`, `SAIDA`, `PAUSA`, `RETOMA`); colunas nullable em **`EventoJogo`** (`segundoJogo Int?`, `posicao Posicao?`, `clientEventoId String?`) + `@@unique([jogoId, clientEventoId])` + `@@index([jogoId, segundoJogo])`; novo model **`SessaoJogoAoVivo`** (1:1 com `Jogo`) e relação inversa `Jogo.sessaoAoVivo`. O model `SessaoJogoAoVivo` ganha o campo **`duracaoParteMins Int @default(20)`** (duração de cada parte, em minutos) — necessário para o cronómetro do Modo Jogo ao Vivo — acrescentado também ao bloco de modelo em §8.25.8. Os mapas exaustivos de `TipoEventoJogo` (rótulos, ícones, agregação estatística) foram estendidos com as novas primitivas. **Não toca em auth.**
- **2026-09-18** — **§8.8.1, §8.13.1 (UX — renomeação de labels)** — **«Plano semanal» → «Horário de treinos» na UI + diferenciação visual do horário recorrente.** Três melhorias de UX sobre a feature de horários recorrentes, **só na apresentação — sem alteração de schema, migração, Server Actions, nomes de função/ficheiro ou auth**. **(1) Renomeação de labels (Correção #3):** todos os labels visíveis ao utilizador que diziam **«Plano semanal»/«Planos semanais»** passam a **«Horário de treinos»/«Horários de treinos»**, para quebrar a colisão de nomes com a **Periodização/Planeamento** (§8.9). Afeta: `app/(app)/treinos/planos/page.tsx` (título de página/metadata, `<h1>`, botões «Novo horário»/«Criar horário», empty state), `app/(app)/treinos/novo/page.tsx` (`<h1>` e tab do toggle), `app/(app)/treinos/page.tsx` (botão do hub), `app/(app)/agenda/page.tsx` (item do overflow «⋯»), `components/treinos/EditarPlanoDialog.tsx` (título, «Nome do horário», «Horário ativo», toast), `components/treinos/ApagarPlanoDialog.tsx` (título, descrição da opção, botão, toast), `components/treinos/PlanoSemanalForm.tsx` («Nome do horário», «Criar horário», dica), `components/treinos/AjudaPlaneamento.tsx` (tooltip + aria-label) e `components/treinos/DialogoAlcance.tsx` (texto). A **entidade técnica mantém-se** `PlanoSemanal`/`PlanoSemanalDia`, tal como Server Actions (`criarPlanoSemanal`, …), schemas, rotas (`/treinos/planos`) e o campo `Sessao.planoSemanalId` — apenas os labels mudaram (as strings de testes ficam inalteradas). **(2) Empty state auto-referente (Correção #4):** em `/treinos/planos`, o texto do estado vazio deixou de apontar para «Treinos > Novo treino» (menu que já não existe) e passa a indicar o botão **«Novo horário»** presente na própria página. **(3) Ícone do horário recorrente (Correção #5):** o ícone dos pontos de entrada do horário recorrente muda de `CalendarClock` para **`Repeat`** (lucide-react), comunicando «recorrente» e diferenciando-o da **Periodização** (que mantém `CalendarRange`): botão «Horário de treinos» no hub `/treinos`, item «Horário de treinos» no overflow da Agenda e badge «Faz parte do horário semanal · Gerir horário» no detalhe da sessão (`/treinos/[id]`). §8.8.1 (heading + nota de terminologia + labels) e §8.13.1 (ponto de entrada na Agenda) alinhadas. UI 100% pt-PT, alvos de toque ≥44px. `npm run typecheck` limpo · `npm run lint` limpo · **1580 testes verdes** (74 ficheiros). **Não toca em auth.**
- **2026-09-16** — **§11.3 (Correção — BUG)** — **Editor de animação — reordenar passos para qualquer posição (arrastar o passo novo para o meio).** No editor de campo, ao **adicionar um passo novo** a uma sequência de animação existente, o passo surgia (corretamente) no **fim** da timeline mas **não era possível arrastá-lo para o meio** da sequência: a timeline (`components/campo/TimelinePassos.tsx`) só oferecia **botões de seta** que trocavam com o vizinho (troca adjacente, `trocar`, lógica inline e não testada) e **nenhuma pega de arrasto** — o utilizador tentava arrastar o chip do passo e nada acontecia. **Correção — sem alteração de schema, migração, Server Actions, dados de negócio ou auth (só editor cliente + funções puras).** **(1) `components/campo/animacao.ts`:** duas funções **puras** novas — **`moverPasso(passos, de, para)`** (remove o passo da posição de origem, insere-o em **qualquer** destino e **reindexa `ordem` 0..N-1**; robusta a arrays desordenados por `ordem`, a `de === para`, a índices inválidos e **não muta** a entrada) e **`ajustarKeyframeAposMover(activo, de, para)`** (o keyframe activo **acompanha** o passo movido; a base/"Início" `-1` nunca é afetada). Como cada `PassoAnimacao` guarda posições **absolutas**, mover o passo **preserva o seu movimento** (o keyframe reconstruído por `construirKeyframes` mantém-se coerente). **(2) `components/campo/TimelinePassos.tsx`:** cada chip de passo ganha uma **pega de arrasto** (ícone `GripVertical`, só a pega é `draggable` para não interferir com o input de duração nem com os restantes botões) com **drag-and-drop por ponteiro** (`onDragStart`/`onDragOver`/`onDrop`/`onDragEnd`, com indicador visual do passo de destino e opacidade no que está a ser arrastado); os **botões de seta ‹ ›** mantêm-se (acessíveis, alvo ≥44px, cobrem **toque e teclado** — o drag nativo não funciona em toque) mas passam a mover via `moverPasso`/`ajustarKeyframeAposMover` em vez da troca adjacente inline. **(3) `components/campo/EditorCampo.tsx`:** a dica da timeline passa a explicar que se reordena arrastando pela pega (ou pelas setas) para qualquer posição. **(4) Testes:** `tests/campo.test.ts` — **+14 testes** para `moverPasso` (mover último→topo, primeiro→fim, para o meio, preservação de `posicoes`/`duracaoMs` + keyframes reconstruídos, array desordenado, no-ops, imutabilidade) e `ajustarKeyframeAposMover` (base intacta, passo seguido, deslocamentos ao fechar/abrir espaço, fora do intervalo). §11.3 alinhada (nota «Reordenar passos para qualquer posição»). `npm run typecheck` limpo · `npm run lint` limpo · **1580 testes verdes** (74 ficheiros). **Não toca em auth.**
- **2026-09-16** — **§8.13.1** — **Navegação — descoberta dos planos semanais/periodização (item «Treinos» está fora do menu).** O ecrã de gestão do **plano semanal** (`/treinos/planos`, horário recorrente que gera sessões) e a **periodização** (`/treinos/periodizacao`) ficaram órfãos de navegação desde que o item «Treinos» saiu do menu (decisão deliberada, changelog 2026-08-26b) — nenhum ecrã visível linkava para lá. Duas correções mínimas de descoberta, **sem reintroduzir «Treinos» no menu**, **sem alteração de schema, migração, Server Actions ou auth**. **(1) Detalhe da sessão (`app/(app)/treinos/[id]/page.tsx`):** quando a sessão foi gerada por um horário recorrente (`Sessao.planoSemanalId != null` — já disponível em `SessaoDetalhe` via `INCLUDE_DETALHE`, sem alterar a query), o cabeçalho mostra um **badge/link contextual «Faz parte do horário semanal · Gerir horário»** (ícone `CalendarClock`, pill na cor do clube `border-primary/30 bg-primary/10 text-primary`, alvo de toque `min-h-[44px]`) que navega para `/treinos/planos`. **(2) Agenda (`app/(app)/agenda/page.tsx`):** o cabeçalho ganha um **menu overflow «⋯»** (dropdown shadcn — `DropdownMenu` + `Button size="icon"` 44px, `aria-label`), junto às ações rápidas «Nova sessão»/«Novo jogo»/«Nova reunião», com atalhos **«Planos semanais»** (`/treinos/planos`, ícone `CalendarClock`) e **«Periodização»** (`/treinos/periodizacao`, ícone `CalendarRange`); agrupados no overflow para não sobrecarregar o cabeçalho no mobile (iPhone SE). §8.13.1 alinhada (Ações rápidas + ligação a partir do detalhe da sessão). UI 100% pt-PT, TypeScript strict. `npm run typecheck` limpo · `npm run lint` limpo · **1566 testes verdes**. **Não toca em auth.**
- **2026-09-16** — **Correção (BUG) — Detalhe do treino: exercícios de Guarda-redes apareciam duplicados (lista principal + Bloco de Guarda-redes) (§8.24.2).** No detalhe `/treinos/[id]`, os exercícios de `categoriaPrincipal=GUARDA_REDES` surgiam **duas vezes**: na **lista principal numerada** (`GestorExercicios`, com miniatura de campo) **e** no **"Bloco de Guarda-redes"** no fundo da página. Passam a aparecer **apenas no bloco** (RN-GR-3 / §8.24.2). **Correção — só na apresentação, sem alteração de schema, migração, Server Actions ou auth; sem props novas** (a `app/(app)/treinos/[id]/page.tsx` continua a passar **todos** os `SessaoExercicio` ao `GestorExercicios`). Em **`components/treinos/GestorExercicios.tsx`**: **(1)** derivam-se `exerciciosVisiveis` (não-GR) e `exerciciosGR` (`categoriaPrincipal === "GUARDA_REDES"`); **(2)** a lista principal — grupos por fase, numeração global, total de minutos, empty state e botão «Editar ordem/Editar» — passa a basear-se em **`exerciciosVisiveis`**, excluindo os de GR; **(3)** o seletor da biblioteca e o `jaAdicionados` ("+1"/"Adicionar") mantêm-se sobre **todos** os exercícios (um exercício de GR adicionado pelo diálogo passa a surgir no bloco); **(4)** **ponto crítico de correção — reordenação:** `mover()` reatribui `ordem` a **todos** os `SessaoExercicio` da sessão (os visíveis reordenados **+ os de GR anexados** por `ordem` crescente), evitando colisão no unique `[sessaoId, ordem]` que ocorreria se `reordenarExercicios` (`lib/actions/treinos.ts`) recebesse apenas o subconjunto visível e as linhas de GR retivessem o seu `ordem`. O **Bloco de Guarda-redes** (`components/treinos/BlocoGuardaRedes.tsx`) e o **modo de condução** (`IniciarTreinoBotao`/§8.8.2, que percorre todos os exercícios) ficam **inalterados** — os de GR continuam a existir na sessão. §8.24.2 clarificada (exclusão da lista principal + reatribuição de `ordem` a todas as linhas). `npm run typecheck` limpo · **1566 testes verdes**. **Não toca em auth.**
- **2026-09-16** — **Correção — Presenças: "Repor" volta a limpar as presenças (incluindo as já guardadas) (§8.8.2).** O botão **"Repor"** na grelha de presenças (`components/treinos/MarcadorPresencas.tsx`) não permitia limpar presenças marcadas por engano: (1) `repor()` **reposicionava o estado no que estava guardado no servidor** (não a vazio), pelo que com presenças já persistidas não limpava nada; (2) o botão estava **desativado quando não havia alterações pendentes** (`!alterado`), impedindo limpar presenças já guardadas; (3) `guardar()` **filtrava os atletas com `estado: null`**, nunca os enviando ao servidor; (4) a Server Action `marcarPresencas` (`lib/actions/treinos.ts`) só fazia **upsert**, sem caminho de remoção, e o `presencaSchema` (`lib/schemas/treino.ts`) **rejeitava `estado` nulo**. **Correção:** **(a)** `presencaSchema.estado` passa a **`nullable`** — `null` exprime "limpar/remover a presença deste atleta". **(b)** `marcarPresencas` separa o lote em **upsert** (estado válido) e **`deleteMany`** (estado `null`, idempotente por `sessaoId`+`atletaId`), mantendo as guardas de auth, tenant (atletas do clube) e sessão fechada. **(c)** `repor()` passa a repor **todos os atletas a "por marcar"** (`estado: null`) e o botão fica **desativado apenas quando não há nenhuma presença marcada** (`haMarcacoes`), ficando **ativo** quando existem presenças — mesmo já guardadas. **(d)** `guardar()` passa a **incluir** os atletas limpos que tinham presença guardada (`presencasIniciais`), enviando `estado: null` para os remover; a limpeza persiste ao confirmar **"Guardar presenças"** (fica ativo por o estado diferir do servidor). Novo teste de schema (aceita `estado: null`) e de `presencasAlteradas` (limpar presença guardada conta como alteração). **Sem alteração de schema Prisma, migração, dados de negócio ou auth.**

- **2026-09-15** — **§11.5 / §13.1** — **Editor de campo — jogador/adversário redesenhados como figura humana 3/4 (camisola numerada), substituindo o disco-com-cabeça.** Afinação **exclusiva** dos `case "jogador"` e `case "adversario"` de `components/campo/desenho.tsx` (as restantes peças — cone, bola, escadinha, barras, arco, baliza — ficam **intactas**, já estavam boas). Onde antes o jogador se lia como uma **ficha/disco** (disco com número + cabecinha por cima), passa agora a **avatar humano estilizado visto em ângulo 3/4** (de trás, como um jogador de camisola): **cabeça** (círculo em tom de pele quente `#EAC7A2` no próprio / cinza-ardósia `#94A3B8` no adversário) sobre um **pescoço** curto; **tronco = camisola** desenhada como *path* (ombros largos com **entalhe de gola** ao centro → cintura estreita com bainha curva) preenchida com a **cor da equipa** e sombreada com os gradientes partilhados `peca-rim` (lado sombra) + `peca-luz` (lado iluminado) para dar volume; **mangas/braços** (elipses verticais nos ombros que assomam nos lados) + **antebraços/mãos** (pequenos círculos de pele); **calções** (trapézio escurecido sob a bainha) e **pernas** (dois traços de pele) a reforçar a "figura de pé"; **sombra elíptica projetada** no chão sob os pés. O **número/rótulo** mantém-se renderizado como `<text>` (a lógica de `tamanhoEtiqueta`/truncagem **não muda**), apenas **reposicionado** para assentar centrado sobre o tronco/camisola (`y = cy + 0.6`). O **adversário mantém-se distinto** — tom cinza-ardósia + **contorno da camisola tracejado** (`strokeDasharray="3 2"`) + rótulo genérico "A". **Mecânica 100% preservada:** tudo é pintado **à volta do ponto lógico `(elemento.x, elemento.y)`** — o tronco fica centrado no ponto, coerente com o **hit-test em círculo de raio ~8** (`animacao.ts::elementoEmPonto`, `usePointerDrag.paraCoordenadas`, mapeamento afim 2D puro); **zero alterações** a props/assinaturas (`ElementoSVG`), `x/y` lógico, raio de hit, `viewBox`, coordenadas, `markerEnd`/geometria das setas, `id`/handlers de hit e seleção, schemas, actions ou auth; **sem transform 3D CSS** (só *paths*/formas 2D). Testes que dependem do render mantêm-se verdes (`tests/campo-fundos.test.ts` — número renderizado como `>7<`; `tests/campo-setas.test.ts` — orientação das setas). **Validação visual** (preview renderizado do código real: cena de campo à escala natural + zoom de cada peça, jogadores nº 7/10 e adversários "A"/nº 5). `npm run typecheck` limpo · `npm run lint` limpo · **1564 testes verdes**. **Só apresentação — não toca em auth.**
- **2026-09-15** — **§11.5 / §13.1** — **Editor de campo — peças mais ilustrativas/2.5D (figuras humanas, bola clássica, cones e obstáculos com volume).** Segundo passo de refinamento visual do INTERIOR de pintura de `components/campo/desenho.tsx`, aproximando as peças de uma ilustração 2.5D realista **sem tocar na mecânica**. **(1) Jogador/adversário:** deixa de ser um disco plano e passa a **figura humana estilizada vista de leve ângulo** — cabeça (círculo com tom de pele quente no próprio / cinza-ardósia no adversário), **ombros/mangas** (elipses atrás do tronco), **tronco = camisola** na cor da equipa (o disco que **carrega o número**, mantido em `r=8` **ancorado no centro lógico `elemento.x/y`**), **gola em V**, sombreado esférico e **sombra projetada** no chão; o adversário mantém contorno **tracejado** e rótulo genérico. **(2) Bola:** padrão clássico de futebol legível a ~8px — pentágono central preto + costuras finas + pequenos painéis de bordo, com sombreado esférico e sombra (substitui o efeito «estrela» da versão anterior). **(3) Cone:** corpo com **apex arredondado** (path com curva), **base/flange** elíptica larga sugerindo a base quadrada, gradiente vertical topo-claro→base-escura, banda refletora e sombra. **(4) Escadinha e barras (obstáculos):** ganham **relevo** — fio escuro de sombra deslocado sob os trilhos/degraus e realce de luz no topo (escadinha); contorno escuro do lado sombreado, realce e **sombra projetada** aos pés (barras). **(5) Fundo:** vinheta radial ligeiramente reforçada nos bordos (**só nos `<defs>`/gradiente** — a contagem de `<rect>`/`<circle>`/`<path>` do fundo mantém-se, `tests/campo-fundos.test.ts` inalterado). **Mecânica 100% preservada:** o mapeamento ponteiro→coordenada (`usePointerDrag.paraCoordenadas`, afim 2D via `getBoundingClientRect`) exige o **plano do campo geometricamente plano** (sem transform 3D CSS) e o hit-test é por distância ao centro lógico — pelo que **só muda a pintura à volta de `elemento.x/y`**; zero alterações a props/assinaturas (`ElementoSVG`, `LinhasCampo`, `SetaMarker`), `viewBox`, coordenadas, `markerEnd`/geometria das setas, `id`/handlers de hit e seleção, schemas ou actions. Setas/linhas mantêm-se (já tinham halo branco + `strokeLinecap/Linejoin=round`). **Só apresentação — sem alteração de schema, dados, Server Actions ou auth.** Validação visual (painel renderizado do código real: cena à escala + zoom de cada peça) + `npm run typecheck` limpo · `npm run lint` limpo · **1564 testes verdes**. **Não toca em auth.**
- **2026-09-14** — **§11.5 / §13.1** — **Editor de campo — redesenho visual das peças com volume 3D (mantendo a cor do clube).** Redesenho do INTERIOR de pintura de `components/campo/desenho.tsx` para dar **volume e realismo** às peças, mantendo o estilo esquemático top-down e **a mecânica/interação intactas**. **Decisões de design:** o **fundo mantém-se a cor do clube** (`var(--cor-primaria, #F0531E)`) — **não** relva — enriquecido com uma **vinheta radial** (brilho central + escurecimento nos bordos) em vez das linhas de piso «parquet» da tentativa anterior; **apenas os tipos de peça já existentes** ganham volume (sem escadas/hurdles/postes). **(1) `<defs>` partilhados** (`FundoDefs`, `PecaDefs`) definidos uma vez e referenciados por `url(#id)`: sombra elíptica de chão, brilho especular, escurecimento de bordo (rim), gradiente vertical de cone, relevo das linhas (`feDropShadow`). **(2) Peças:** cones com gradiente topo-claro→base-escura + banda refletora + sombra projetada; bola com painéis (pentágono + costuras) e sombreado esférico; jogadores como disco com sombreado esférico, bisel e sombra, número com contorno (`paintOrder`); setas com halo branco e cabeça sólida; baliza/aro com rede sugerida e sombra. **(3) Mecânica preservada:** zero alterações a props/assinaturas exportadas (`LinhasCampo`, `ElementoSVG`, `SetaMarker`), `viewBox`, sistema de coordenadas, tamanhos/posições lógicas, `id`/handlers de hit e seleção usados por `EditorCampo.tsx` — as peças continuam ancoradas em `elemento.x/y` com os mesmos raios; **só muda a pintura**. **Só apresentação — sem alteração de schema, dados, Server Actions ou auth.** Validação visual (painel comparativo ANTES/DEPOIS renderizado do código real, futsal e futebol) + `tsc --noEmit` limpo em `desenho.tsx`. **Não toca em auth.**
- **2026-09-14** — **§3.7 / §8.20.1** — **Métricas configuráveis — novo tipo `ESCALA_1_3` (escala de 1 a 3).** O enum `TipoMetrica` ganha o valor **`ESCALA_1_3`** (a par de `NUMERO`, `BOOLEANO`, `ESCALA` 1–5), para métricas com escala curta de **1 a 3**. **(1) Schema (`prisma/schema.prisma`):** `enum TipoMetrica` acrescenta `ESCALA_1_3`. **(2) Schemas Zod (`lib/schemas/metrica.ts`):** `LABEL_TIPO` ganha `ESCALA_1_3: "Escala 1–3"` (surge automaticamente no seletor de tipo do CRUD de métricas em Definições, que itera sobre `LABEL_TIPO`); limites de domínio `LIMITES_ESCALA.ESCALA_1_3 = { min: 1, max: 3 }` e revalidação server-side em `valorMetricaValido`. **(3) UI de input — os três pontos onde o valor da métrica se regista** (`components/jogos/JogoDetalhe.tsx` — grelha de estatísticas do jogo; `components/treinos/GestorMetricasSessao.tsx` — grelha da sessão de treino; `components/treinos/SessaoExternaGRForm.tsx` — sessão externa de GR) passam a renderizar, para `ESCALA_1_3`, **três botões toggle inline (1/2/3)** — aparência compacta, alvo de toque **≥44px** (`h-11`), cor da marca no selecionado (`bg-primary`/`--cor-primaria`), `role="group"` + `aria-pressed`; clicar no valor ativo limpa a seleção (volta a `null`). O valor guardado é o **número** (1, 2 ou 3), coerente com `ValorMetrica`/`ValorMetricaSessao` (`Int`). **(4) Análise:** `obterAnaliticoAtleta`/agregadores tratam `ESCALA_1_3` como escala (soma/média, a par de `ESCALA`). UI 100% pt-PT, TypeScript strict. `npm run typecheck` limpo · `npm run lint` limpo · **1564 testes verdes**. **Não toca em auth.**
- **2026-09-14** — **§8.8.2** — **Modo treino — adicionar exercícios da biblioteca ao aquecimento em condução ao vivo (ligação ao ecrã).** Ligado ao modo condução o **seletor rápido «＋ Adicionar ao aquecimento»** já descrito em §8.8.2 (ponto «Adicionar ao aquecimento»): durante o treino em campo, o treinador pode juntar um exercício da biblioteca à **fase de aquecimento** desta sessão sem sair da condução. **Sem alteração de schema, migração, Server Action nova ou auth** — reutiliza a action existente **`adicionarExercicioSessao(sessaoId, exercicioId, "AQUECIMENTO")`** (§7.3 — valida a fase com `parteTreinoSessaoSchema`, isola por clube, exige `TREINOS_GERIR` e revalida `/treinos/[id]`) e a lógica pura testada **`filtrarBibliotecaAquecimento`** (`lib/treino-aquecimento.ts`). **(1) `components/treinos/ModoTreino.tsx`:** nova prop opcional **`biblioteca?: ExercicioBibliotecaAquecimento[]`** (default `[]`); botão **«＋ Adicionar ao aquecimento»** (ícone `Plus`) por baixo da barra de fases, visível só quando há biblioteca; novo estado `aquecimentoAberto`; **guarda de `Escape`** — com o diálogo aberto, `Escape` fecha o diálogo em vez de terminar a sessão; render do **`AdicionarAquecimentoDialog`**. **(2) `components/treinos/IniciarTreinoBotao.tsx`:** aceita e propaga `biblioteca` até ao `ModoTreino`. **(3) `app/(app)/treinos/[id]/page.tsx`:** passa a biblioteca da sessão (já carregada via `listarExercicios`) ao `IniciarTreinoBotao`, mapeada para `{ id, nome, categoriaPrincipal, duracaoMin }`. Componentes `AdicionarAquecimentoDialog`/`lib/treino-aquecimento.ts` e testes `tests/treino-aquecimento.test.ts` já existentes reutilizados sem alterações. UI 100% pt-PT, TypeScript strict. `npm run typecheck` limpo · `npm run lint` limpo · testes verdes. **Não toca em auth.**
- **2026-09-13** — **§8 / §8.11 / §8.13.1** — **Navegação — «Jogos» volta a ser item de menu + filtro por tipo (Oficial/Amigável) na lista.** A pedido, a rota `/jogos` (vista de gestão de jogos) volta a ser alcançável **diretamente pelo menu**, além do acesso pela Agenda. **Sem alteração de schema, migração ou auth.** **(1) `components/layout/Navegacao.tsx`:** importado o ícone **`Shield`** (e o tipo `LucideIcon`); novo tipo `ItemNav` (com campo opcional `excluir`) e novo item em `ITENS_BASE` — `{ href: "/jogos", label: "Jogos", icon: Shield, excluir: ["/jogos/competicoes"] }` — posicionado **a seguir a «Agenda» e antes de «Exercícios»** (índice 3). Como a bottom-nav móvel expõe os 4 primeiros itens, «Jogos» passa a integrar a bottom-nav e **«Exercícios» transita para o menu «Mais»**. Novo helper `ativoDe(item)` que respeita `excluir`, evitando que «Jogos» fique ativo em `/jogos/competicoes` (item próprio «Competições»); os quatro pontos de render (sidebar, menu «Mais», bottom-nav e botão «Mais») passam a usá-lo. **(2) Filtro por tipo na lista de jogos (§8.11):** `listarJogos` (`lib/actions/jogos.ts`) ganha o 4.º parâmetro opcional `tipo?: TipoJogo` (filtro server-side `{ tipo }`, aplicado a par dos filtros de escalão, modalidade e estado de fecho já existentes). `app/(app)/jogos/page.tsx` valida `?tipo=OFICIAL|AMIGAVEL` (Zod), acrescenta `tipo` ao construtor de `href` (preservado entre os restantes filtros) e renderiza um segmented control **«Todos · Oficial · Amigável»** (rótulos de `LABEL_TIPO_JOGO`), ao lado do filtro de estado. **(3) §8, §8.11 e §8.13.1 alinhadas.** UI 100% pt-PT, TypeScript strict. `npm run typecheck` limpo · `npm run lint` limpo. **Não toca em auth.**
- **2026-09-13** — **§11.5 / §13.1** — **Correção (visual) — fundo do campo passou a piso de pavilhão (parquet) em vez de faixas de relva.** Uma melhoria visual anterior tinha adicionado **faixas de relva** (bandas verticais brancas translúcidas, «campo aparado») ao fundo do campo em `components/campo/desenho.tsx`. Como a modalidade base é **futsal** — jogado em **pavilhão** com piso de madeira/sintético, **não em relva** — o efeito foi substituído por uma textura de **piso de pavilhão (parquet)**: linhas **horizontais** muito finas e de baixíssima opacidade (`stroke="#000000"`, `strokeWidth={0.5}`, `opacity={0.05}`, `PISO_TABUAS = 14`) a simular as juntas das tábuas. **Só apresentação — sem alteração de schema, dados de negócio, Server Actions ou auth.** A **cor base do fundo mantém-se a cor do clube** (`var(--cor-primaria, #F0531E)`, constante renomeada `RELVA` → `PISO`) e as **linhas de campo mantêm-se a branco**. As tábuas são desenhadas como `<line>` (não `<rect>`), pelo que a **contagem de rects do fundo continua a ser 2** (`tests/campo-fundos.test.ts` inalterado). Aplica-se a todos os fundos (o `Relvado`/piso é partilhado por futsal e futebol). `npm run typecheck` limpo · `npm run lint` limpo · **1555 testes verdes**. **Não toca em auth.**
...
- **2026-09-12** — **§8.24.3 / §8.24.5 / §8.24.6** — **Treino específico de GR — Server Actions e validações (passo seguinte ao schema+seed).** Implementada a camada de actions/validações de §8.24, sobre a migração `gr_treino_especifico` já aplicada; aditivo e sem alteração de schema nem auth. **(1) `lib/schemas/treino.ts`:** novo **`SessaoExternaGRSchema`** (+ tipo `SessaoExternaGRFormData`) para §8.24.6 — `data` (obrigatória), `duracaoMin`/`local`/`entidadeExterna`/`objetivo`/`notas` opcionais, `escalaoId` (cuid), `atletasIds` (≥1 cuid) e `metricas` opcionais (`{ metricaId, atletaId, valor:int }[]`, default `[]`; `valor` inteiro = coluna `ValorMetricaSessao.valor Int`). **(2) Novo `lib/actions/treino-gr.ts`:** `criarSessaoExternaGR` (impõe `tipoSessao=EXTERNA_GR`, `planeamentoId`/`planoSemanalId`/`planoSemanalDiaId`/`rpeSessao` nulos — RN-GR-5; ≥1 participante GR; valida no servidor que cada `atletaId` pertence ao clube, participa na época ativa e tem `GUARDA_REDES` nas `posicoes`; métricas restritas a `aplicaSoGuardaRedes=true` + `contexto ∈ {TREINO,AMBOS}` — RN-GR-2; presenças `PRESENTE` + métricas gravadas em transação), `listarSessoesExternasGR(escalaoId)` (época ativa, participantes + contagem de métricas, `data DESC`), `obterSessaoExternaGR(sessaoId)` (isolamento multi-tenant + `EXTERNA_GR` + leitura do escalão; participantes e métricas com `metricaConfig{nome,tipo}`), `atualizarSessaoExternaGR(sessaoId, Partial<…>)` (metadados; diff de presenças com remoção das métricas dos participantes retirados; upsert/delete de métricas por `(sessaoId,metricaId,atletaId)`; mesmas validações de GR), `apagarSessaoExternaGR(sessaoId)` (delete com cascata de presenças/métricas). Todas com `"use server"`, `Resultado<T>`, `TREINOS_GERIR` no escalão e multi-tenant por clube. **(3) `lib/actions/metricas.ts` — `guardarMetricasSessao`:** validação de servidor (RN-GR-2) que **rejeita** valores de métricas `aplicaSoGuardaRedes=true` para atletas sem `GUARDA_REDES` (não confia na UI); `listarMetricasSessao` continua a devolver o `MetricaConfig` completo (inclui `aplicaSoGuardaRedes`) para a grelha desativar a coluna nos atletas não-GR. **(4) `lib/actions/analise.ts` — `obterAnaliticoAtleta`:** novo campo aditivo **`metricasGR: MetricaGREvolucao[]`** (§8.24.5) — evolução por sessão (NORMAL + EXTERNA_GR) de cada métrica `aplicaSoGuardaRedes=true`, agrupada por métrica e ordenada por data; retorno existente intacto. **Testes:** `tests/treino-gr.test.ts` (11, novo) + `tests/metricas-treino.test.ts` (+2 GR, mock de `posicoes`/`aplicaSoGuardaRedes` atualizado). `npm run typecheck` limpo · `npm run lint` limpo · **1547 testes verdes**. **Não toca em auth.**
- **2026-09-12** — **§23.4 / §23.5 / §23.7 / §23.8** — **Gestão de Competições (passo 1.6) — evolução da UI de competições.** Implementação (código) da camada de apresentação sobre as actions/schemas de §23 já entregues (passos 1.2–1.5), **só componentes cliente + um rótulo partilhado** — sem alteração de schema, actions ou auth. **(1) `lib/schemas/competicao.ts`:** novo rótulo `LABEL_ESTADO_RESULTADO` (pt-PT: Agendado/Realizado/Cancelado/Walkover), fonte única para a UI. **(2) `components/competicoes/WizardCompeticao.tsx`:** Passo 1 (base) ganha **selector de âmbito** (dois cards radio «Externa (seguimento)» | «Própria (gerida pelo clube)» com descrição contextual; nota informativa em EXTERNA) e **«Pontos por vitória»** (radio 3 | 2, com empate 1 / derrota 0 fixos); Passo 2 (equipas) pré-adiciona, em `PROPRIA` com escalão escolhido, a **equipa do escalão** como primeira linha destacada com badge «Própria» (não removível, `tipo=PROPRIO` + `escalaoVinculadoId`), restantes participantes `EXTERNO`; geração de quadro, numeração e validações passam a usar a lista derivada de participantes (própria + convidadas); submit passa `ambito`, `pontosVitoria/pontosEmpate/pontosDerrota`, `golosWalkover` e `equipas` com `tipo`/`escalaoVinculadoId`. **(3) `components/competicoes/ResultadoExternoForm.tsx`:** evoluído para **registo de confronto** — campo `estado` (Select AGENDADO|REALIZADO|CANCELADO|WALKOVER), golos só em REALIZADO, `walkoverVencedor` (Casa|Fora) só em WALKOVER; submete para a action **`registarConfronto`** (por nome, cria participante `EXTERNO` on-the-fly); default REALIZADO com golos (retrocompatível). **(4) `components/competicoes/TabelaClassificacao.tsx`:** usa `l.posicao` (em vez de índice) e destaca a linha `l.ehProprio` (cor de acento do clube); removido o prop `nomeEquipaPropria` (redundante). **(5) `components/competicoes/QuadroAgendamento.tsx`:** selo de estado por confronto (AGENDADO neutro, REALIZADO verde, CANCELADO vermelho, WALKOVER âmbar) + **dropdown de contexto** para alterar estado (chama `definirEstadoConfronto`, incl. WO casa/fora vence); confrontos passam a agrupar-se em «Por disputar» (AGENDADO) e «Resultados» (REALIZADO/WALKOVER/CANCELADO), garantindo que os novos estados são visíveis. **(6) `components/competicoes/CompeticaoDetalhe.tsx`:** badge de âmbito («Externa» | «Própria» via `LABEL_AMBITO_COMPETICAO`); tab «Resultados externos» renomeada para **«Confrontos»**; passa `walkoverVencedor` ao quadro. `npm run typecheck` limpo · `npm run lint` limpo · **testes de competições/classificação verdes (49/49)** (2 ficheiros de teste alheios — `metricas-treino`, `onboarding` — falham independentemente destas alterações, por trabalho em curso no schema/seed). **Não toca em auth.**
- **2026-09-12** — **§23.3 / §23.5 / §23.7 / §23.8** — **Gestão de Competições (passos 1.2–1.5) — schema Zod, cálculo da classificação, Server Actions e testes.** Implementação (código) da spec §23 já documentada, sobre o schema Prisma aplicado (enums `AmbitoCompeticao`/`TipoParticipanteCompeticao`, `EstadoResultado` com `CANCELADO`/`WALKOVER`, colunas novas em `Competicao`/`EquipaCompeticao`/`ResultadoCompeticao`/`Jogo`). **(1) `lib/schemas/competicao.ts`:** `criarCompeticaoSchema` e `criarCompeticaoCompletaSchema` ganham `ambito` (default `PROPRIA`), `pontosVitoria` (1–5, default 3), `pontosEmpate` (0–2, default 1), `pontosDerrota` (0–1, default 0) e `golosWalkover` (1–9, default 3) — todos com default (retrocompatíveis); novos `registarConfrontoSchema` (equipa por FK ou nome, `estado` default `REALIZADO`, refine `WALKOVER ⇒ walkoverVencedor`), `definirEstadoConfrontoSchema` e `ligarJogoAConfrontoSchema`; `equipaCompeticaoSchema` ganha `tipo` (default `EXTERNO`) + `escalaoVinculadoId?`/`clubeVinculadoId?`; rótulos `LABEL_AMBITO_COMPETICAO` e `LABEL_TIPO_PARTICIPANTE_COMPETICAO`. **Nota de retrocompatibilidade:** `registarResultadoExternoSchema` (contrato golos 0–99, `equipaCasa`/`equipaFora`) é **mantido intacto** como schema próprio a par do novo `registarConfrontoSchema`, para não quebrar `registarResultadoExterno` nem o código/testes legados. **(2) `lib/classificacao.ts`:** `calcularClassificacao` evoluída (retrocompatível, parâmetros opcionais com defaults 3/1/0 e `golosWalkover` 3) — pontuação configurável (só LIGA; TORNEIO/TAÇA continuam sem pontos), só contam confrontos `REALIZADO`+`WALKOVER` (`AGENDADO`/`CANCELADO` ignorados; `estado` ausente = `REALIZADO`), WO conta `golosWalkover`–0 a favor de `walkoverVencedor` (ignora golos inseridos); `LinhaClassificacao` ganha `ehProprio` e `posicao`; desempate pontos → DG → GM → nome. **(3) `lib/actions/competicoes.ts`:** `criarCompeticao`/`criarCompeticaoCompleta` persistem os campos novos; em `PROPRIA`, `criarCompeticaoCompleta` garante a equipa do próprio escalão como participante `PROPRIO` (`escalaoVinculadoId`); novas actions `registarConfronto(competicaoId, dados)` (resolve participante por FK ou cria `EXTERNO` on-the-fly, tolera P2002, persiste FKs + texto legado, WALKOVER limpa golos), `definirEstadoConfronto`, `ligarJogoAConfronto`/`desligarJogoDeConfronto` (validam mesmo clube/escalão/época); `obterClassificacao` lê os campos de pontuação, o participante `PROPRIO` (fallback = nome do escalão) e filtra `estado ∈ {REALIZADO, WALKOVER}`; `removerEquipaCompeticao` bloqueia com confrontos `REALIZADO` **ou** `WALKOVER` (por nome ou FK). `registarResultadoExterno` mantido como action retrocompatível. **(4) Testes:** `tests/classificacao.test.ts` (+7 → 18) e `tests/competicoes.test.ts` (+16 → 31) — pontos 2/1/0 e `pontosDerrota`, WALKOVER (ignora golos, respeita vencedor CASA/FORA e `golosWalkover`), `CANCELADO`/`AGENDADO` ignorados, `ehProprio`, `posicao`, e schemas de confronto/estado. `npm run typecheck` limpo · `npm run lint` limpo · **1534 testes verdes** (todos os existentes intactos). **Não toca em auth.**
- **2026-09-12** — **§1.4 / §2 / §3.8 / §6 / §8.4 / §8.5 / §8.14 / §8.15 / §8.24.4 / §10.1 / §10.6 / §16 / §20.1 / Apêndice C** — **Remoção das features Habilidades e Caderneta.** As features **Habilidades** (catálogo de "moves" técnicos por nível, em Definições) e **Caderneta** (progresso/desbloqueio de habilidades por atleta/época, incl. caderneta de GR §8.24.4) foram **removidas por completo** do produto. **Schema:** eliminados os modelos `Habilidade` e `ProgressoHabilidade` e os enums `NivelHabilidade` e `EstadoHabilidade`; removidas as relações `Clube.habilidades`, `Atleta.progressos`, `Epoca.progressos`; removido o campo `Habilidade.aplicaSoGuardaRedes` (GR). Migração aditiva **`remove_habilidades_caderneta`** (`20260912160000`): `DROP TABLE "ProgressoHabilidade"`, `DROP TABLE "Habilidade"`, `DROP TYPE "EstadoHabilidade"`, `DROP TYPE "NivelHabilidade"`. **Permissões:** removidas as capacidades `CATALOGO_HABILIDADES` e `CADERNETA_GERIR` do catálogo (`lib/permissoes-catalogo.ts`) e dos perfis de arranque (Diretor Técnico, Adjunto). **Código:** removidos `lib/actions/habilidades.ts`, `lib/actions/caderneta.ts`, `lib/schemas/habilidade.ts`, `lib/schemas/caderneta.ts`, `components/definicoes/HabilidadesLista.tsx`, `components/plantel/CadernetaAtleta.tsx`, a rota `app/(app)/definicoes/habilidades/` e a aba **Caderneta** do perfil do atleta; expurgadas as referências em `lib/actions/analise.ts` (campo `caderneta` do analítico e `habilidades` da evolução multi-época), `components/analiticos/PainelAtleta.tsx`, seeds (`prisma/seed.ts`, `prisma/seed-rico.ts`, `prisma/data-migrations/seed_sport_lisboa_evora_extra*.ts`) e instaladores de arranque (`lib/biblioteca-arranque-instalar.ts`, `-futebol.ts`, `-gr.ts`). **Nota importante:** as **subcategorias de exercícios** (`SubcategoriaExercicio`, §8.6) — por vezes chamadas coloquialmente "habilidades dos exercícios" — são uma feature **distinta** e **permanecem**; o mesmo para as **subcategorias e métricas técnicas de GR** (§8.24.2/§8.24.3). As secções §3.8, §8.14 e §8.24.4 são mantidas como placeholders «(removido)» para não invalidar referências históricas de numeração. `typecheck` limpo. **Não toca em auth.**
...
- **2026-09-12** — **§8.24.4** — Caderneta de GR removida: módulo Habilidades/Caderneta descontinuado.
...
- **2026-09-11** — **§3.1 / §3.11 / §5.2 / §7.3 / §8.1 / §8.4 / §12 / §17.2 / §17.5** — **Rework de registo/licenças (Fase 4) — modo Individual materializado no modelo.** O produto **Individual** deixa de ser um clube «normal» e materializa-se no modelo: **`Clube.clubeTecnico=true`** é o **single source of truth** do modo Individual (definido na criação, `registar`/`criarClube` quando `tier=INDIVIDUAL`) — clube invisível/pessoal, de utilizador único. **Titularidade da licença:** a licença Individual é titulada por **`utilizadorId`** (nunca `clubeId`); a **`Carteira`** da pessoa é criada no arranque do modo Individual, **idempotente** (`upsert`). **Modalidade:** `Licenca.modalidade` é atributo **exclusivo** do Individual, **persistido na criação da licença** (dentro da transação — **não** por `updateMany` pós-transação) e é a **fonte de verdade** do produto vendido; **`null` em licenças de Clube** (a modalidade do clube deriva das secções, pricing por secção via `numSeccoes`). **Resolução a jusante:** `temLicencaValida`, `obterLicencaPendente` e `obterLicenca` resolvem o titular por **`clubeId` OU `utilizadorId`** (derivados **exclusivamente** da sessão — defesa contra IDOR). **Onboarding/transação:** o ramo Individual produz **atomicamente** `clubeTecnico=true` + licença em `utilizadorId` + `Carteira` + **uma única secção** da modalidade. **Guards (defesa em profundidade):** o servidor **recusa** o convite de membros em clube técnico (`convidarMembro`) e **bloqueia** a 2ª modalidade em clube técnico (`garantirSeccaoParaModalidade`); a ocultação de UI é **cosmética**. **Ocultação:** a gestão de clube (Clube/Branding, Secções, Equipa técnica, Perfis) é ocultada na **página de Definições** no modo Individual (flag `ocultarIndividual` derivada de `clubeTecnico`) — a **navegação de topo mantém-se inalterada** (esses módulos são sub-entradas de Definições); permanecem visíveis Escalões, Épocas, Métricas, Habilidades, Subcategorias, Licença e Integrações. **Fix code review M1:** `obterLicenca` (leitura da licença **ATIVA**) passa a resolver por `clubeId` **ou** `utilizadorId`, corrigindo **Definições · Licença** para utilizadores Individual (antes só resolvia por `clubeId`). Resolvida a decisão em aberto de §3.11/§17.2 sobre a fonte de verdade de `Licenca.modalidade`. **Não toca em auth.**
- **2026-09-11** — **§8.1 / §12 / §17.5 / §3.11** — **Rework de registo/licenças (Fase 3) — registo consolidado num passo.** O registo passa a ser um **passo único**: `registarSchema` **unificado** recolhe **conta + clube + plano (tier) + modalidade**, com **`modalidade` obrigatória** (removido o default silencioso "FUTSAL"). A action **`registar()`** cria **atomicamente** (numa transação) conta + clube + secção da modalidade + época ativa + perfis de arranque + membro Administrador + **`Licenca` PENDENTE** e **notifica o admin** por email (template «novo registo» da Fase 2, com `{ nome, email, clube, plano, modalidade }`) em modo **best-effort** — a falha de email não aborta o registo. O **wizard de onboarding fica bloqueado enquanto a licença for `PENDENTE`** (removida a isenção de rota em `deveBloquearPorLicenca`); o gating passa a ser por **`onboardingConcluido`** via `destinoOnboarding` (`false` → `/onboarding`; `true` → `/dashboard`). O **formulário de registo** foi enriquecido com **seletor de plano** (cartões com preço lido de `lib/billing.ts`, «Clube Pequeno» = mais popular) e **seletor de modalidade** (2 cartões, sem pré-seleção; radios `sr-only`, alvos ≥44px, ativo `border-primary`), reutilizando o componente-cartão do onboarding. O **`callbackUrl` do registo** passa a **`/sem-licenca`** (paywall). Nota de arquitetura: `/sem-licenca` e `/criar-clube` vivem **fora** do grupo `(app)`, logo não passam pela guarda de licença — evita ciclos de redirect. O fallback autenticado `/criar-clube` (+ `criarClubeSchema`, `default("FUTSAL")`) mantém-se por retrocompatibilidade. **Não toca em auth-core.**
- **2026-09-11** — **§17.5** — **Rework de registo/licenças (Fase 3) — infra de email transacional (Resend).** Nova camada server-side **`lib/email/`** (SDK **Resend**), **distinta e independente** de `lib/email.ts` (recuperação de password, §6.2). `enviarEmail` é **best-effort** — nunca parte o fluxo e degrada graciosamente na ausência de `RESEND_API_KEY`. Dois **templates transacionais PT-PT** prontos a consumir por fases futuras: **(a) novo registo → admin** (assunto `Novo registo — [clube]`; destinatário `ADMIN_EMAIL`, com fallback para o email de suporte) e **(b) conta ativada → utilizador** (assunto `A tua conta Mister está pronta`). Templates com **marca Mister** e valores do utilizador **escapados** (anti-injeção). Nova variável de ambiente **`ADMIN_EMAIL`**; reutiliza `RESEND_API_KEY`/`EMAIL_FROM`. Nova dependência **`resend`**. Sem alterações a schema nem a auth.
- **2026-09-11** — **§17.5** — **Rework de registo/licenças (Fase 2) — resolução de licença pendente por dupla via + ativação exclusiva do backoffice.** `obterLicencaPendente` passa a resolver o plano **`PENDENTE`** também por licença **Individual** (`utilizadorId`), além da licença de **Clube** (`clubeId`) — ambas derivadas **exclusivamente da sessão autenticada** (defesa contra IDOR); o paywall `/sem-licenca` mostra o valor a transferir em qualquer dos ramos. **Removida a ativação self-service de «licença de demonstração»** por admin de clube (antiga action `criarLicencaDemostracao` + botão `ActivarDemoButton`), que emitia licença **`ATIVA` fora do fluxo de pagamento** — a ativação (`PENDENTE → ATIVA`) passa a ser **exclusiva do backoffice** (admin de plataforma, §21.2), após confirmação de pagamento; a página **Definições · Licença** deixa de oferecer ativação e sinaliza esse fluxo. A criação de licença de onboarding foi **centralizada** num helper interno que aceita o titular por **`clubeId`** ou **`utilizadorId`** (preparação para o ramo Individual), **sem alterar** o comportamento do ramo Clube. **Não toca em auth.**
- **2026-09-11** — **§17.5** — **Rework de registo/licenças (Fase 1) — seed de desenvolvimento cria licença ATIVA + Carteira (dev-only).** O `prisma/seed.ts` passa a criar, para o clube de demonstração, uma `Licenca` com **`estado: ATIVA`** (`tipo: CLUBE`, `tier: MEDIO`, `ciclo: MENSAL`, `dataFim` a 1 ano) e a `Carteira` do administrador (`saldoCentimos: 0`), de forma **idempotente** (`upsert` pelas chaves únicas `clubeId`/`utilizadorId`), resolvendo o bloqueio no paywall (chicken-and-egg) e permitindo entrar sem paywall e passar a guarda de licença (§3.11) em desenvolvimento/testes. **Nunca em produção:** protegido por **`NODE_ENV !== "production"`** — em produção a ativação continua a ser apenas pela via legítima de billing. Artefacto de conveniência de dev, **não estado de negócio**. Sem alterações a schema nem a auth.
- **2026-09-11** — **§3.7 / §8.20.1** — **Métricas configuráveis em treinos — pontuações por atleta e por sessão (ex.: empenho, desempenho), agregadas ao longo da época.** As métricas configuráveis deixam de ser exclusivas dos jogos e passam a poder registar-se **também nas sessões de treino**, colmatando o gap em que se criavam métricas sem sítio para as usar fora do jogo. **(1) Schema (`prisma/schema.prisma` + migração `metricas_treino`):** `MetricaConfig` ganha o campo **`contexto`** (novo enum `ContextoMetrica { JOGO TREINO AMBOS }`, default **`JOGO`** — retrocompatível: métricas existentes mantêm-se só de jogo); novo model **`ValorMetricaSessao`** (`metricaId` + `sessaoId` + `atletaId` + `valor Int`, `@@unique([metricaId, sessaoId, atletaId])`, cascade em métrica/sessão/atleta, índices por sessão/atleta/métrica); novas relações `Sessao.valoresMetricas` e `Atleta.valoresMetricasSessao`. **(2) Schemas Zod (`lib/schemas/metrica.ts`):** `metricaSchema` ganha `contexto` (default `JOGO`); novos `guardarMetricasSessaoSchema`, `LABEL_CONTEXTO`. **(3) Server Actions (`lib/actions/metricas.ts`):** `listarMetricas` ganha filtro opcional por contexto (`{ in: [ctx, "AMBOS"] }`); nova `listarMetricasSessao(sessaoId)` (métricas de treino ativas + mapa `valores[atletaId][metricaId]`, exige leitura do escalão); nova `guardarMetricasSessao(sessaoId, dados)` (exige `TREINOS_GERIR`, sessão fechada ⇒ erro; só métricas de treino ativas e atletas do clube; upsert **delete-then-create** pelos atletas submetidos — valores limpos são removidos). **(4) UI:** novo `components/treinos/GestorMetricasSessao.tsx` (grelha «Métricas do treino» — linha por atleta **presente**, input adapta ao tipo NUMERO/BOOLEANO/ESCALA, guarda em lote; só-leitura em sessão fechada), integrado em `app/(app)/treinos/[id]/page.tsx`; `components/definicoes/MetricasLista.tsx` ganha o seletor «Onde se regista» (contexto) e mostra o contexto na lista. **(5) Análise:** `obterAnaliticoAtleta` (`lib/actions/analise.ts`) devolve `metricasTreino` (agregação por sessão ao longo da época, reutilizando `agregarMetricasAtleta`); `components/analiticos/PainelAtleta.tsx` mostra a secção «Métricas de treino» (média · nº de sessões). **(6) Testes:** novo `tests/metricas-treino.test.ts` (14 testes — schema, validação/permissão/isolamento de `guardarMetricasSessao`, delete-then-create, `listarMetricasSessao`); mocks de `valorMetricaSessao` adicionados a `tests/analise-f9.test.ts`, `tests/futebol-fase28.test.ts`, `tests/export-csv.test.ts`. UI 100% pt-PT, TypeScript strict (zero `any`). `npm run typecheck` limpo · `npm run lint` limpo · **1486 testes verdes**. **Não toca em auth.**
- **2026-09-11** — **§8.13.1** — **Correção (BUG) — Agenda (vista lista): eventos truncados em ~30 dias + sem distinção passado/futuro.** Na vista de lista da Agenda (`/agenda?vista=lista`), a janela temporal de `obterAgendaClube` (`lib/actions/agenda.ts`) usava, na ausência de `mes`/`ano`, um limite fixo de **30 dias a partir de hoje**, pelo que qualquer evento mais distante (ex.: já em outubro) **desaparecia** da lista — dava a sensação de a agenda «ficar parada». Além disso, todas as entradas surgiam com o **mesmo estilo**, sem distinguir o que já passou do que está por vir. **Correção — só apresentação/derivação de janela, sem alteração de schema, migração, dados de negócio ou auth.** **(1) `lib/actions/agenda.ts` (`resolverJanela`):** a janela por defeito (vista lista) deixa de ser «próximos 30 dias» e passa a abranger **toda a época ativa** — `gte = Epoca.dataInicio` (início do dia) a `lte = Epoca.dataFim` (fim do dia). O teto é a `dataFim` da época (não um limite artificial de dias); as sessões/jogos já eram filtrados por `epocaId` e as reuniões passam a respeitar a mesma janela da época. A vista de calendário (com `mes`/`ano`) mantém-se **inalterada** (janela do mês focado). **(2) `app/(app)/agenda/page.tsx`:** cada entrada da lista calcula `treinoConcluido(ev.data)` (`lib/semana.ts`) — as **realizadas** (data anterior ao início do dia de hoje) ganham **estilo amortecido** (fundo `cinza-50`, texto `cinza-500`, selo «Realizado», sem `shadow-card`), as **futuras** mantêm o estilo normal/ativo; textos de subtítulo e estado vazio atualizados de «próximos 30 dias» para «toda a época». **(3) Testes:** `tests/agenda.test.ts` — mock de `Epoca` ganha `dataInicio`/`dataFim` e novo teste garante que a vista lista abrange toda a época (sem o limite de 30 dias). UI 100% pt-PT, TypeScript strict (zero `any`). **Não toca em auth.**
- **2026-09-10** — **§3.2 / §5.5 / §5.6 / §8.5 / §15.1** — **Implementado upload de fotografias de atletas com compressão WebP via Supabase Storage (backend).** Substitui o stub tipado por uma implementação real, mantendo a assinatura consumida pela UI. **(1) Dependências:** `@supabase/supabase-js` + `server-only`. **(2) `lib/supabase-storage.ts` (novo, server-only):** cliente singleton do Supabase Storage protegido por `import "server-only"` (usa `NEXT_PUBLIC_SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY`, esta última **nunca** exposta ao browser); constante `BUCKET_ATLETAS` (bucket **público** com paths UUID não-adivinháveis — decisão documentada: signed URLs expirariam a cada page load, sem benefício real neste contexto); helpers `obterSupabaseStorage` (desliga-se sem env vars), `obterHostnameStorage` e `extrairPathDoStorage` (distingue ficheiros nossos de URLs externos). **(3) `lib/actions/atletas.ts` — `uploadFotoAtleta(atletaId, formData): Promise<Resultado<{ fotoUrl }>>`:** valida `atletaId` (Zod cuid) → `auth`/isolamento por clube → `PLANTEL_GERIR` num escalão do atleta → extrai o `File` de `formData.get("foto")` → **valida magic bytes** (JPEG `FF D8 FF`, PNG `89 50 4E 47`, WebP `RIFF`+`WEBP` fourcc — nunca a extensão/content-type) → **valida tamanho** (≤10 MB) → re-encode `sharp` para **WebP 256×256** (q80, `fit: cover`, `withoutEnlargement`) → upload ao bucket `atletas` no path `atletas/{clubeId}/{atletaId}-{uuid}.webp` → persiste o `fotoUrl` público → **apaga a foto anterior** do Storage (só se for nossa) → `revalidatePath` (`/plantel`, `/plantel/[id]`, `/dashboard`). **(4) Ciclo de vida no hard-delete:** `apagarAtletaDefinitivamente` passa a selecionar `fotoUrl` e a apagar o ficheiro do Storage (best-effort, não bloqueia o delete RGPD) antes de `prisma.atleta.delete`. **(5) `lib/schemas/atleta.ts`:** novo `uploadFotoAtletaSchema`. **(6) `next.config.js`:** `experimental.serverActions.bodySizeLimit = "5mb"`. **(7) `.env.example`:** `NEXT_PUBLIC_SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` (server-only) documentadas. **(8) `prisma/schema.prisma`:** comentário de `fotoUrl` atualizado (sem migração). **(9) Testes:** `tests/upload-foto-atleta.test.ts` — rejeição de mime inválido (magic bytes errados) e de ficheiro grande exercitam o **código real** de validação (Storage mocado apenas como dependência externa); verificação de que o hard-delete tenta apagar o ficheiro do Storage. TypeScript strict (zero `any`), UI 100% pt-PT. §16 ganha a **Fase 35**. **Não toca em auth.**
- **2026-09-10** — **§8.5** — **UI de upload de foto de atleta com compressão no browser (FotoUpload).** O campo de foto do atleta passa de **input de URL** para um **avatar editável** com picker de ficheiro, compressão no browser e upload imediato. **(1) Dependência:** `browser-image-compression@^2.0.2`. **(2) `components/plantel/FotoUpload.tsx` (novo, «use client»):** input `type=file` (aceita `image/jpeg,image/png,image/webp`); ao selecionar, comprime (`maxWidthOrHeight 512`, `maxSizeMB 2`, web worker) e faz **upload imediato** via `uploadFotoAtleta(atletaId, formData)` (o servidor re-encoda para 256×256 WebP e devolve o `fotoUrl` persistido); **pré-visualização** por object URL (revogado no cleanup); botão **«Remover foto»** (limpa o estado local — o save do formulário remove na BD); estado de **loading** (spinner no avatar); **mensagens de erro** claras (tipo inválido, ficheiro grande, erro de upload); avatar editável (ícone de câmara, ≥80×80px); acessível (`Label` associado, `aria-describedby`/`role=alert` para erros); alvos de toque ≥44px. O `fotoUrl` viaja num **input escondido** para o save do formulário preservar/remover a foto. **(3) `components/plantel/AtletaForm.tsx`:** remove o input de texto `fotoUrl`; em **edição** rende `FotoUpload`; na **criação** mostra «Guarda primeiro o atleta para adicionar uma fotografia» (o upload precisa de `atletaId`). **(4) `components/plantel/AvatarAtleta.tsx`:** passa a «use client» e ganha **fallback para as iniciais** quando a imagem falha ao carregar (guardando o URL falhado, para reagir a um novo `fotoUrl` válido); URLs do Supabase Storage (público) renderizam diretamente; URL externo legado continua a funcionar. **(5) Stub temporário:** `uploadFotoAtleta` foi adicionada a `lib/actions/atletas.ts` como **stub tipado** (assinatura `(atletaId, formData) => Promise<Resultado<{ fotoUrl }>>`), a **substituir** pela implementação real (Supabase Storage) do bff-backend-specialist. UI 100% pt-PT, sem dark mode, TypeScript strict (zero `any`). **Não toca em auth.**
- **2026-09-08** — **§8.8.2 / §11.3** — **Diagrama do exercício em grande — auto-reprodução ao abrir + desenho progressivo das setas.** No plano de exercícios da sessão (`components/treinos/GestorExercicios.tsx`), clicar na miniatura de um exercício abre o modal do diagrama (`components/treinos/ModalDiagramaExercicio.tsx`), que passa a **reproduzir o exercício automaticamente** ao abrir; **clicar na imagem** volta a reproduzir. **Só apresentação — sem alteração de schema, migração, Server Actions, dados de negócio ou auth.** **(1)** Novo componente unificado **`components/campo/DiagramaReproduzivel.tsx`** («use client») que escolhe o modo de reprodução: **(a)** diagrama **com passos/keyframes** (§11.2) → delega no motor existente **`CampoAnimado`** com `autoPlay` (movimento interpolado dos jogadores/bola + controlos de play/loop/velocidade — reutilização, zero duplicação); **(b)** diagrama **sem passos mas com setas/linhas** → **desenha as setas progressivamente e em sequência** (efeito «draw»), com botão **play/replay** sobreposto (ícone ▶ na 1.ª vez, ↻ depois; alvo grande, `focus-visible` na cor do clube); a `key` dos elementos animados inclui um contador de reprodução para **reiniciar** a animação CSS a cada play; **(c)** sem setas nem keyframes → `MiniaturaCampo` estática (sem botão). **(2)** `ElementoSVG` (`components/campo/desenho.tsx`) ganha a prop **opcional e aditiva `animarTrajeto?: { atrasoMs; duracaoMs } | null`**: quando presente, o `<path>` da **seta/linha** recebe `pathLength={1}` + `style` com `stroke-dasharray:1; stroke-dashoffset:1` e a animação `campo-desenhar-trajeto` (draw). Ausente → render estático de sempre (**zero regressão** no editor e nos testes `tests/campo-setas.test.ts`, que renderizam sem a prop). **(3)** Novo keyframe CSS **`@keyframes campo-desenhar-trajeto`** (`app/globals.css`, `stroke-dashoffset` 1→0). **(4)** `ModalDiagramaExercicio` passa a usar `DiagramaReproduzivel` (`autoReproduzir`) em vez de `MiniaturaCampo` estática. **UX:** cada seta desenha-se em **~820 ms** com **~420 ms** de atraso entre setas (dentro de 600–1200 ms), `ease-in-out`; respeita **`prefers-reduced-motion`** (não anima — mostra o diagrama já desenhado); funciona em **dark mode** (o campo usa a cor do clube via `--cor-primaria`, linhas a branco). §8.8.2 e §11.3 atualizadas (nova nota «▶️ v7 — Auto-reprodução ao ver o diagrama em grande»). UI 100% pt-PT, zero `any`. `npm run typecheck` limpo · `npm run lint` limpo · **1462 testes verdes**. **Não toca em auth.**
- **2026-09-08** — **§2 / §12** — **Correção (BUG) — fuso horário: datas/horas apresentadas em UTC em vez de Europe/Lisbon em produção.** Em produção o processo Node corre em **UTC**, pelo que os métodos nativos de `Date` dependentes do fuso do runtime (`getHours()`, `getMinutes()`, `getDay()`, `getDate()`, `getMonth()`, `getFullYear()`, `setHours()`, `toLocaleDateString("pt-PT", …)` sem `timeZone`) devolviam a **hora/dia UTC** e não a **hora de parede de Lisboa** que o utilizador espera — um treino às **20:30 Lisboa** (verão, WEST=UTC+1) aparecia como **«Treino hoje às 19:30»**, e eventos junto à meia-noite eram colocados no **dia errado** (calendários, agrupamento por semana, indicador «evento hoje», janelas do dia). **Correção — puramente de apresentação/fronteiras de dia, ancorada a `Europe/Lisbon` reutilizando os helpers existentes de `lib/utils-datas.ts` (`partesDataLisboa`, `wallClockLisbonToInstant`, `formatarDataHoraLisboa`) e `lib/plano-semanal.ts` (`inicioDoDia`, `fimDoDia`, `diaSemanaISO`) — sem novos helpers, sem alteração de schema, migração, Server Actions, dados de negócio ou auth.** **(1) `lib/dashboard-lembretes.ts`:** `mesmoDia()` e `horaCurta()` passam a derivar dia/hora de `partesDataLisboa` (corrige o título «Treino/Jogo hoje às HH:MM» e a filtragem de eventos de hoje). **(2) `lib/semana.ts`:** `segundaFeira()`, `domingo()` e `inicioDoDia()` reancorados a Lisboa (via `diaSemanaISO` + meia-noite de parede de Lisboa); `numeroSemana()`/`treinoConcluido()` corrigem-se por delegação. **(3) `app/(app)/dashboard/page.tsx` e `app/(app)/layout.tsx`:** a janela do dia de hoje passa a usar `inicioDoDia`/`fimDoDia` (Lisboa) em vez de `setHours(0,0,0,0)`/`setHours(23,59,59,999)` (UTC). **(4) `components/treinos/CalendarioTreinos.tsx` e `components/agenda/CalendarioAgenda.tsx`:** o mapeamento de sessões/eventos a dias e o realce «hoje» passam a usar `partesDataLisboa`. **(5) `app/(app)/treinos/page.tsx`:** `formatarIntervaloSemana()` e `isoData()` passam a usar `partesDataLisboa`/`formatarDataHoraLisboa`. **(6) Polimento:** dez `formatarData`/`dataAbrev` que usavam `toLocaleDateString("pt-PT", …)` **sem `timeZone`** passam a `formatarDataHoraLisboa` (páginas de analíticos de exercícios, integração de calendário, licença + movimentos, detalhe do modelo de jogo; componentes `PainelEscalao`, `PainelTreinoAtleta`, `UsoExercicioCard`, `CarreiraAtleta`, `ParticipacoesAtleta`). Comportamento idêntico em desenvolvimento (runtime já em Europe/Lisbon) e agora **correto em produção (UTC)**. `npm run typecheck` limpo · **1462 testes verdes**. **Não toca em auth.**
- **2026-09-07** — **§3.2 / §8.5** — **Modelo do atleta — dados pessoais adicionais (contactos, documento de identificação, FPF) e action de atualização de participação.** Expansão do modelo `Atleta` com **campos pessoais adicionais, todos opcionais/nullable** (zero regressão em atletas existentes): `email`, `telefone`, `peDominante` (**novo enum** `PeDominante { DIREITO ESQUERDO AMBIDEXTRO }`), `paisNascimento`, `nacionalidade`, documento de identificação do atleta (`docTipo` — **novo enum** `DocTipo { NIC PASSAPORTE AR CR TR }` —, `docNumero`, `docValidade`), FPF (`estatutoFPF` — **novo enum** `EstatutoFPF { PORTUGUES ESTRANGEIRO UNIAO_EUROPEIA IGUALDADE }` —, `numeroLicencaFPF`) e documento do encarregado de educação (`encarregadoDocTipo`, `encarregadoDocNumero`, `encarregadoDocValidade`). **(1) `lib/schemas/atleta.ts`:** `atletaPessoalSchema` (partilhado por criar/atualizar) ganha os novos campos como opcionais — enums via helper `enumOpcional` («» de `<select>` → `undefined`), datas via `dataOpcional` («» → `undefined`); novos label maps + arrays para os seletores da UI (`LABEL_PE_DOMINANTE`/`PES_DOMINANTES`, `LABEL_DOC_TIPO`/`TIPOS_DOC`, `LABEL_ESTATUTO_FPF`/`ESTATUTOS_FPF`) e re-export dos tipos `PeDominante`/`DocTipo`/`EstatutoFPF`. **(2) `lib/actions/atletas.ts`:** `criarAtleta` e `atualizarAtleta` persistem os novos campos (ausente → `null`); `AtletaPessoal`/`SELECT_PESSOAL` incluem-nos na leitura (pré-preenchimento do formulário de edição). **(3) `lib/schemas/participacao.ts` + `lib/actions/participacoes.ts`:** nova action **`atualizarParticipacaoAtleta(atletaId, escalaoId, { numero?, tipoParticipacao? })`** (schema `atualizarParticipacaoAtletaSchema`) para editar o **número de camisola** e/ou o **tipo** de uma participação ativa na época ativa — valida input (Zod), exige `PLANTEL_GERIR` no escalão, e ao mudar o tipo respeita o invariante do «principal por modalidade» (§9, em transação Serializable, despromovendo o principal anterior). `revalidatePath` em `/plantel`, `/plantel/[id]` e `/dashboard`. `npx prisma generate` executado. `npm run typecheck` limpo · `npm run lint` limpo · **1462 testes verdes**. **Não toca em auth.**
- **2026-09-07** — **§8.8.2** — **Modo condução do treino — ordenação por fase e navegação direta por fase.** O modo treino (`ModoTreino`, botão «Iniciar treino») apresentava os exercícios apenas pela `ordem` global, ignorando a fase; e só permitia avançar um a um. **Correção + melhoria (só apresentação/navegação — sem alteração de schema, migração, Server Actions, dados de negócio ou auth).** **(1) Ordenação canónica das fases:** nova lógica pura e testada em **`lib/treino-fases.ts`** — `ORDEM_FASES` (**Aquecimento → Parte principal → Jogo → Retorno à calma**, com bucket «sem fase» no fim), `ordemDaFase`, `ordenarExerciciosPorFase` (**sort estável**: ordena pela fase e preserva a `ordem` dentro de cada fase) e `fasesComExercicios` (fases presentes, na ordem canónica, com o índice do 1.º exercício de cada uma). É a **mesma sequência** que o plano (`GestorExercicios`) já apresenta agrupada. **(2) `components/treinos/ModoTreino.tsx`:** passa a conduzir sobre `exerciciosOrdenados = ordenarExerciciosPorFase(exercicios)` (via `useMemo`); todo o índice/progresso/«Anterior/Próximo» opera sobre a lista ordenada. Nova **barra de navegação por fase** abaixo do cabeçalho — lista **apenas as fases presentes** (com exercícios), **clicar salta para o 1.º exercício** dessa fase (reinicia o cronómetro, como a navegação exercício-a-exercício), **fase atual destacada** (`aria-current="step"`, fundo `primary`), alvos ≥44px, scroll horizontal; só aparece com **>1 fase**. As constantes de ordenação foram **centralizadas** no novo `lib/treino-fases.ts` (o `ModoTreino` mantém apenas `LABEL_FASE` para os rótulos da barra). **(3) Testes:** novo `tests/treino-fases.test.ts` (8 testes) cobre a ordem canónica, a preservação de `ordem` dentro da fase (sort estável), o bucket «sem fase» no fim, a imutabilidade da entrada e a lista de fases presentes com o índice de arranque. §8.8.2 ponto 3 e casos-limite atualizados. TypeScript strict (zero `any`). `npm run typecheck` limpo · `npm run lint` limpo · **1462 testes verdes**. **Não toca em auth.**
- **2026-09-07** — **§11.2** — **Editor de campo — rótulo personalizado do jogador.** O elemento `Jogador` do diagrama passa a suportar um **rótulo personalizado** opcional que substitui o número apresentado dentro do círculo (ex.: «Ala», «Pivot», «Fixo»), permitindo nomear jogadores por função. **Alteração aditiva e retrocompatível — diagramas existentes (sem `rotulo`) continuam a mostrar o número/«A» e validam sem migração.** **(1) `lib/schemas/exercicio.ts`:** novo campo `rotulo: z.string().max(6).optional()` no `jogadorSchema` (máx. 6 chars — cabe «Pivot»); persiste automaticamente no JSON do `diagrama` por fazer parte do `Jogador`. **(2) `components/campo/desenho.tsx` (`ElementoSVG`):** a `etiqueta` do jogador deriva agora de `rotulo ?? (numero ?? (adversário ? "A" : null))`; o `fontSize` do `<text>` **adapta-se** ao comprimento (8 até 2 chars, 6 até 3, 5 acima) para caber no círculo (r=8) — testado com «Ala»/«Pivot». **(3) `components/campo/EditorCampo.tsx`:** nova função `mudarRotuloJogador(id, rotulo)` (análoga a `mudarCorCone`) que regista **histórico/undo**, aplica o rótulo (recortado a 6 chars) e, quando vazio, **remove** a propriedade do elemento (volta ao número); novo painel contextual no modo **Selecionar** para o jogador selecionado — `Input` (shadcn/ui, `maxLength=6`, placeholder «ex: Ala», `value = rotulo ?? ""`) com **rótulo** «Rótulo:» e botão **X** (ícone `lucide-react`, alvo ≥44px) para limpar. §11.2 atualizada (nova nota «🏷️ v7 — Rótulo personalizado do jogador»). TypeScript strict (zero `any`). `npm run typecheck` limpo · `npm run lint` limpo · **1462 testes verdes**. **Não toca em auth.**
- **2026-09-07** — **§8.23.1 / §8.23.2** — **Analíticos de treino — enriquecimento da UI: subcategoria, parte do treino e cadência de treino.** A página de analíticos de treino passa a expor **todas as dimensões** já disponíveis na camada de dados (`obterAnaliticoTreinoEscalao`/`obterAnaliticoTreinoAtleta` em `lib/actions/analise.ts`), várias das quais estavam a ser calculadas mas **não renderizadas**. **Só apresentação — sem alteração de schema, migração, Server Actions, dados de negócio ou auth** (os campos consumidos foram adicionados às Server Actions pela camada de dados; esta alteração é a UI que os desenha). **(1) `components/analiticos/PainelTreinoEscalao.tsx` (aba «Treino» de `/escaloes/[id]/analiticos`):** a secção **«Volume de treino»** ganha uma segunda linha de KPIs — **exercícios dados** (`totalExercicios`), **exercícios/sessão** (`mediaExerciciosPorSessao`), **treinos/semana** (`frequenciaSemanalMedia`) e **treinos/mês** (`frequenciaMensalMedia`), com «—» quando `null`; a secção **«Composição dos treinos»** (grelha 2-up `sm:grid-cols-2`) passa a incluir, além de «Por tipo de sessão» e «Por categoria», os gráficos **«Por parte do treino»** (`distribuicaoParteTreino`, rótulos via `LABEL_PARTE_TREINO`) e **«Por subcategoria»** (`distribuicaoSubcategoria`, rótulo `subcategoria`, top 10) — cada gráfico só é desenhado quando tem dados. **(2) `components/analiticos/PainelTreinoAtleta.tsx` (secção «Treino» do perfil do atleta):** ao lado de «Exercícios por categoria» surge **«Exercícios por subcategoria»** (`exerciciosPorSubcategoria`, top 10), na mesma grelha `lg:grid-cols-2`, só quando há dados; a condição de renderização dos gráficos passa a considerar também as subcategorias. Reutiliza o `GraficoBarrasH` partilhado (não alterado). §8.23.1 (novas linhas A2 e E2) e §8.23.2 (nova linha C2) atualizadas. UI 100% pt-PT, alvos de toque ≥44px, sem dark mode novo. `npm run typecheck` limpo · `npm run lint` limpo · **1454 testes verdes**. **Não toca em auth.**
- **2026-09-07** — **§8.8.2 / §11.5** — **Correção (BUG, seguimento) — impressão/PDF do plano de treino: campos ainda a laranja e exercícios sem agrupamento por fase.** A correção anterior de 2026-09-07 (mesma secção) **não resolveu** dois dos problemas na `/treinos/[id]/print`, confirmado em uso. **(1) Campos ainda a laranja:** a tentativa anterior injetava `--cor-primaria` **apenas num `<div>` interno** por `style`. O fundo do campo (`RELVA` em `components/campo/desenho.tsx`) consome essa variável como **atributo de apresentação do SVG** (`fill="var(--cor-primaria, #F0531E)"`), que na prática **só é resolvido de forma fiável quando a variável está no elemento raiz (`<html>`)** — é exatamente por isto que `components/layout/TemaClubeGlobal.tsx` foi criado (2026-09-04, §11.5, para os portais do Radix). Fora do grupo `(app)` a página **não** montava o `TemaClubeGlobal`, pelo que o SVG caía no *fallback* laranja da marca apesar do `<div>` temado. **Correção:** `app/treinos/[id]/print/page.tsx` passa a montar `TemaClubeGlobal` (espelha `--cor-primaria`/`--cor-secundaria` de `Clube.corPrimaria`/`corSecundaria` em `document.documentElement`), mantendo também o `style` no `<div>` (acentos por herança). `components/campo/desenho.tsx` **não** foi alterado. **(2) Exercícios sem agrupamento:** a tentativa anterior apenas **ordenava** os exercícios num `<ol>` plano (fase → `ordem`), sem os **agrupar** por fase como o ecrã do treino (`GestorExercicios`). **Correção:** `components/treinos/TreinoPrintTemplate.tsx` passa a **agrupar por fase** com a **mesma lógica do ecrã** — constantes `PARTES_TREINO`/`ORDEM_FASES` (Aquecimento → Parte principal → Jogo → Retorno à calma → «Sem fase»), `LABEL_FASE`, cabeçalho por fase com contagem, ordenação interna por `ordem` e **numeração global fase a fase**; o *chip* «Fase» por exercício foi removido (redundante com o cabeçalho do grupo). **(3) Bloqueador de build:** `tsconfig.json` passa a **excluir `scripts/`** da compilação do Next.js (scripts de importação *ad-hoc*, executados à parte, com *imports* a *scratchpads* efémeros que quebravam o `next build`). Puramente de apresentação/build — sem alteração de schema, migração, Server Actions, dados de negócio ou auth. `npm run typecheck` limpo · `npm run lint` limpo · `npm run build` verde (exit 0, 57/57 páginas). **Não toca em auth.**
- **2026-09-07** — **§10.2 / §12.1** — **Correção (BUG) — cartões de gráfico dos analíticos de escalão apareciam com fundo «preto» no tema escuro.** Apesar da correção de 2026-09-04 (tokens `--grafico-*` que tornaram a **grelha/rótulos/realce** do `GraficoBarrasH` legíveis no escuro), os gráficos de barras continuavam a **ler-se como um bloco preto**: o problema não estava no SVG (fundo transparente) mas no **cartão que o envolve**. Tanto o `PainelEscalao` como o `PainelTreinoEscalao` usavam o wrapper **idêntico** `rounded-lg border border-cinza-200 bg-white p-5` — ou seja, **não havia** um «token diferente» num painel que funcionasse melhor no outro; ambos remapeiam **`bg-white → #1c1b22`** (cartão base) no `.dark`, praticamente indistinguível do fundo da app (`#0F0E13`), fazendo o gráfico parecer flutuar no vazio. (A sugestão `dark:bg-cinza-800` não produziria efeito: a escala `cinza` **não tem** o tom `800`, e o remap não-*layered* `.dark .bg-white` venceria sempre um utilitário `dark:`.) **Correção (puramente visual — sem alteração de schema, migração, Server Actions, dados de negócio ou auth):** nova classe de componente **`.card-grafico`** em `app/globals.css` — branca em tema claro (via `@apply … bg-white`, **zero regressão no claro**) e, no `.dark`, na **superfície elevada** da marca **`#2a2933`** (§12.1, com borda `#34333d`), mais clara que o cartão base `#1c1b22` e que o fundo `#0F0E13`, para o gráfico ler como um **painel distinto**. A regra `.dark .card-grafico` é declarada **sem `@layer`** para vencer o remap `.dark .bg-white` herdado pelo `@apply`. A classe é aplicada aos **cartões de gráfico das duas abas** da página de analíticos do escalão — `CartaoGrafico` do `PainelTreinoEscalao` (aba «Treino») e os três cartões de `GraficoBarrasH` do `PainelEscalao` (melhores marcadores, melhores assistentes, jogadores mais utilizados — aba «Jogos») —, mantendo **ambas consistentes** (o objetivo do pedido: a aba «Treino» a condizer com a «Jogos»). O plural **«1 utilização»** já estava correto (todos os usos de `unidade="utilizações"` passam `unidadeSingular="utilização"`, e o `GraficoBarrasH` renderiza o singular quando `valor === 1`) — **nada a alterar** nesse ponto. §10.2/§12.1 alinhadas. `npm run typecheck` limpo · `npm run lint` limpo · testes verdes. **Não toca em auth.**
- **2026-09-07** — **§8.8.2** — **Correção (BUG) — impressão/PDF do plano de treino: modo claro forçado no ecrã, cor do clube nos campos e ordem por fase.** A página `/treinos/[id]/print` tinha três problemas: **(1)** a **pré-visualização no ecrã saía escura** (fundo preto/texto branco) quando o tema do produto estava no escuro (default) — a rota vive **fora do grupo `(app)`** mas continua sob o `ThemeProvider` (`next-themes`, `class` no `<html>`), pelo que herdava a classe `dark` e os *overrides* `.dark .bg-white`/`.dark .text-cinza-900` do `globals.css`; **(2)** os **diagramas de campo apareciam a laranja** (`#F0531E`, laranja da marca) em vez da **cor do clube** — o fundo do campo (`RELVA` em `components/campo/desenho.tsx`) usa `var(--cor-primaria, #F0531E)` e a rota **não herda** o `--cor-primaria` do layout `(app)` (definido em `app/(app)/layout.tsx`), caindo no *fallback*; **(3)** os **exercícios saíam pela `ordem` crua da tabela de junção**, podendo colocar um exercício da «Parte principal» **no fim**, em desalinhamento com a visualização no ecrã (`GestorExercicios`), que **agrupa por fase**. **Correção (puramente de apresentação — sem alteração de schema, migração, Server Actions, dados de negócio ou auth):** **(a)** novo *client component* `components/treinos/ForcarModoClaroImpressao.tsx` que, enquanto montado, remove `dark`/adiciona `light` no `<html>` **sem** tocar no `localStorage` (repõe o estado original ao desmontar), forçando **claro no ecrã** — complementa o bloco `@media print` do `globals.css`, que já forçava claro em papel; **(b)** a página `app/treinos/[id]/print/page.tsx` injeta `--cor-primaria` (de `Clube.corPrimaria`, via `obterMembroAtual`) no seu contentor por `style`, para `MiniaturaCampo`/`CampoDesenho` pintarem o campo com a **cor do clube**; **(c)** a lista de exercícios passa a ser **ordenada por fase** (`ParteTreino`, ordem canónica `PARTES_TREINO`: Aquecimento → Parte principal → Jogo → Retorno à calma; sem fase no fim) e, dentro de cada fase, pela `ordem` da junção — espelhando o agrupamento do ecrã. §8.8.2 atualizada. `npm run typecheck` limpo · `npm run lint` limpo · `npm run build` verde (57/57 páginas). **Não toca em auth.**
- **2026-09-07** — **§8.15 / §12** — **Correção (BUG) — o logótipo do clube não aparecia no PDF dos analíticos para logótipos WebP/SVG/GIF.** O relatório imprimível do «Dossier do Treinador» (`lib/pdf/gerar-pdf.ts`) **embute** o logótipo do clube como *data URI* (garante a imagem pronta antes de o browser abrir o diálogo de impressão) e ambos os templates (`components/pdf/PdfEstatisticaIndividual.ts` → `cabecalhoHtml`; `components/pdf/PdfEstatisticaGeral.ts` → escudo grande) já o renderizam. **Causa raiz:** o *helper* `carregarLogo` só aceitava `image/png`/`image/jpeg`, pelo que qualquer clube com `logoUrl` em **WebP, SVG ou GIF** (formatos válidos em todo o resto da app — `LogoClube`/`MarcaAguaClube` aceitam qualquer imagem `https:`, e `next.config.js` até declara WebP) via o logótipo em **toda a UI menos no PDF**, onde caía silenciosamente no *placeholder* com a inicial do clube. Também falhava quando o servidor de imagem devolvia um `content-type` genérico (`application/octet-stream`/vazio). **Correção (só o pipeline do PDF — sem alteração de schema, migração, Server Actions, dados de negócio ou auth):** `carregarLogo` passa a aceitar **PNG/JPEG/WebP/GIF/SVG** (`MIME_LOGO`) e, quando o `content-type` não é uma imagem reconhecível, **infere o MIME pela extensão do URL** (`mimePorExtensao` — `application/octet-stream` com `.svg` → `image/svg+xml`, etc.); mantém o limite de 2 MB, o *timeout* de 4 s e o **fallback gracioso** para o disco com a inicial quando `logoUrl` é `null` ou o formato não é reconhecível (SVG renderizado dentro de `<img>`, sem execução de scripts). Novos testes em `tests/pdf.test.ts` (embute PNG como data URI sem *placeholder*; embute WebP via `content-type`; infere SVG a partir de `octet-stream`+extensão; *fallback* para *placeholder* quando o formato é irreconhecível). §8.15/§12 alinhadas (o logótipo do clube no cabeçalho dos PDF respeita agora todos os formatos suportados pela app). `npm run typecheck` limpo · `npm run lint` limpo · **1454 testes verdes**. **Não toca em auth.**
- **2026-09-04** — **§10.2 / §12.1** — **Analíticos de treino do escalão — legibilidade dos gráficos em tema escuro e densidade 2-up (correção visual).** O `PainelTreinoEscalao` (`components/analiticos/PainelTreinoEscalao.tsx`) aparecia com os gráficos de barras «apagados» (fundo escuro do cartão + rótulos/grelha ilegíveis) e com um layout mais **pesado** que o `PainelEscalao`, apesar de ambos usarem o **mesmo** `GraficoBarrasH` com wrapper idêntico (`rounded-lg border border-cinza-200 bg-white p-5`). **Diagnóstico:** (a) o **tema escuro é o default** do produto (`next-themes`, `defaultTheme="dark"`) e remapeia `bg-white → #1c1b22`; os neutros do `GraficoBarrasH` (grelha `#E4E1DB`, rótulos `#57514A`/`#98938D`, realce de linha `…8%, white`) estavam **hardcoded para tema claro**, ficando ilegíveis sobre superfície escura — problema **partilhado** por todos os painéis, mas só visível onde há barras (no clube demo, os gráficos ofensivos do `PainelEscalao` costumam estar vazios); (b) as barras seguem `var(--cor-primaria)` — a cor **real** do clube; o clube demo **JSC** tem `corPrimaria: "#1A2FD4"` (**azul**) no seed, pelo que as barras azuis são a cor do clube e **não** um bug (o `PainelEscalao` mostraria igualmente azul para este clube); (c) o `PainelTreinoEscalao` empilhava cada gráfico numa secção de **largura total**, enquanto o `PainelEscalao` agrupa gráficos **2-up**. **Correção (puramente visual — sem alteração de schema, migração, Server Actions, dados de negócio ou auth):** **(1)** novos tokens **adaptáveis ao tema** em `app/globals.css` — `--grafico-grid`, `--grafico-texto`, `--grafico-texto-muted`, `--grafico-realce` — com valores claros no `:root` (iguais aos anteriores, **zero regressão no tema claro**) e valores legíveis no `.dark` (grelha `#2a2933`, rótulos `#d8d3cb`/`#928c84`, realce `…20%, #1c1b22`); a **cor das barras mantém-se `--cor-primaria`** (cor do clube). **(2)** `components/graficos/GraficoBarrasH.tsx` passa a consumir esses tokens (`C_GRID`/`C_TEXTO`/`C_TEXTO_MUTED`/novo `C_REALCE`) em vez das constantes hardcoded — mudança **partilhada** que mantém `PainelEscalao` e `PainelTreinoEscalao` **consistentes** e corrige a legibilidade em ambos. **(3)** `PainelTreinoEscalao` reorganizado para a **mesma densidade** do `PainelEscalao`: «Tipos de sessão» + «Por categoria» passam a uma grelha `grid gap-6 sm:grid-cols-2` (padrão «Rankings ofensivos»), sob uma única secção «Composição dos treinos»; «Top 10 exercícios» mantém-se em largura total (até 10 linhas). §10.2/§12.1 alinhadas. `npm run typecheck` limpo · `npm run lint` limpo · **1450 testes verdes**. **Não toca em auth.**
- **2026-09-04** — **§10.2** — **Analíticos de escalão — tabela por atleta e separação ativos/inativos (backend).** `obterAnaliticoEscalao` (`lib/actions/analise.ts`) passa a devolver uma **tabela com TODOS os participantes da época** do escalão (qualquer estado — histórico persistente, §10.1) e a **distinguir atletas ativos de inativos**. **Só leitura/agregação server-side aditiva — sem alteração de schema, migração, dados de negócio ou auth.** **(1)** Novo tipo exportado **`LinhaAtletaEscalao`** (`atletaId`, `nome`, `posicoes`, `estadoParticipacao: EstadoParticipacao`, `atletaAtivo`, `golos`, `assistencias`, `jogosUtilizados`, `jogosConvocado`, `presencas`, `taxaPresenca` 0–1, `tempoJogo` em minutos). **(2)** `AnaliticoEscalao` ganha `nAtletasAtivos`, `nAtletasInativos` e `tabelaAtletas: LinhaAtletaEscalao[]`; **`nAtletas` mantém a semântica original** (= `nAtletasAtivos`, denominador das médias) para **zero regressão** downstream (KPIs, `taxaPresencaMedia`, `presencaMensal`, export CSV/PDF). **(3)** A query que **apenas contava** atletas ativos (`atletaEscalao.count`) foi substituída por `atletaEscalao.findMany` (todos os estados, com `atleta.nome/posicoes/ativo`) + `convocatoria.groupBy` (convocatórias por atleta, já filtradas por `data ≤ agora` via `filtroJogo`) — **sem query adicional líquida** (uma passou de count a findMany, outra é nova). `nAtletasAtivos` = participantes com `estado === "ATIVO"` **e** `atleta.ativo`; `nAtletasInativos` = restantes (INATIVO/TRANSICAO ou atleta desativado). **(4)** `tabelaAtletas` é construída **reutilizando os mapas de agregação já existentes** (`golosMap`, `assistMap`, `utilMap`, `assiduidadeMap`) + as convocatórias por atleta, ordenada por nome (`localeCompare` pt); `taxaPresenca` usa o **mesmo denominador** (sessões executadas) de `rankingAssiduidade`/`taxaPresencaMedia`, com cap a 1. Novos testes em `tests/analise-f9.test.ts` (tabela inclui todos os participantes, separa ativos de inativos, valores por atleta) e mocks de `tests/export-csv.test.ts` atualizados (`atletaEscalao.findMany` + `convocatoria.groupBy`). §10.2 alinhada. `npm run typecheck` limpo · `npm run lint` limpo · **1450 testes verdes**. **Não toca em auth.**
- **2026-09-04** — **§8.23.2 / §10.1** — **Perfil do atleta — colapso das abas «Estatísticas» e «Analytics» numa única aba «Estatísticas».** Decisão de produto: o perfil do atleta (`app/(app)/plantel/[id]/page.tsx`) deixa de ter duas abas separadas («Estatísticas» e «Analytics») e passa a ter **uma única aba «Estatísticas»** (nome mais familiar ao treinador). **Só apresentação/reorganização de UI — sem alteração de schema, migração, Server Actions, dados de negócio ou auth.** **(1)** A aba **«Analytics»** é **eliminada** da navegação (`TabsList`) e o seu `TabsContent` fundido no da aba «Estatísticas». **(2)** Dentro da aba «Estatísticas», renderiza-se **primeiro** o `EstatisticasAtleta` (tiles + gráficos base — visível para **todos**) e, **a seguir** (separado por `border-t`), o conteúdo completo do `PainelAtleta` (rácios, comparações directa e com a equipa, evolução multi-época, treino) mais os botões **«Exportar histórico»/«Relatório»** — **condicionalmente**, só para utilizadores com a permissão **`RELATORIOS_VER`** (`podeVerRelatorios`). Utilizadores **sem** `RELATORIOS_VER` veem **apenas** os tiles e gráficos base (comportamento idêntico ao da antiga aba «Estatísticas»). **(3)** Os componentes `EstatisticasAtleta` e `PainelAtleta` mantêm-se **inalterados** — muda apenas onde/como são compostos (sequencialmente, no mesmo `TabsContent`). Como o painel avançado já só é renderizado sob `podeVerRelatorios`, removeu-se o ramo de erro «Sem permissão» (agora inalcançável), mantendo-se o estado «Analytics indisponíveis» para falhas de dados com permissão presente. §8.23.2/§10.1 alinhadas. `npm run typecheck` limpo · `npm run lint` limpo · testes verdes. **Não toca em auth.**
- **2026-09-04** — **§10.2 / §10.3** — **Correção (BUG) — balanço da época contava jogos futuros no KPI «jogos».** Na vista de analíticos de escalão (`PainelEscalao`, alimentado por `obterAnaliticoEscalao` em `lib/actions/analise.ts`), o KPI **«jogos»** do «Balanço da época» (`AnaliticoEscalao.jogos = jogos.length`) contava **todos os jogos da época/escalão**, incluindo os **agendados no futuro** (ex.: mostrava «2 jogos» quando esses jogos ainda não tinham sido disputados) — assimétrico com o V/E/D (que só conta jogos com resultado) e com `sessoesExecutadas` (que já filtrava por `data < agora`). **Correção (só leitura/agregação — sem alteração de schema, migração, dados de negócio ou auth):** **(1)** `obterAnaliticoEscalao` — adicionado `data: { lte: <agora> }` ao objeto `filtroJogo`, que é **reutilizado** na query de `jogo.findMany` e nas de `estatisticaAtleta`/`eventoJogo`/`valorMetrica` (`where: { jogo: filtroJogo }`), pelo que o filtro se aplica de forma **consistente** a tudo o que deriva dos jogos (contagem de jogos, V/E/D, golos e médias por jogo, marcadores/assistentes, mais utilizados, eventos, rankings por métrica). **(2)** `obterAnaliticoClubeEpoca` (painel do clube, §10.3, balanço P2-06 e comparação por escalão) — o mesmo `data: { lte: <agora> }` na query de `jogo.findMany` (aplicado **só** a essa query, já que `filtroEpocaEscaloes` é partilhado com o `sessao.groupBy` das sessões programadas); corrige a assimetria com `sessoesExecutadas` (que já usava `data < agora`). Jogos futuros continuam a aparecer na Agenda/lista de jogos, mas **não** entram nos analíticos. Testes de `tests/analise-f9.test.ts` atualizados para refletir o novo `where` (incl. `data: { lte: expect.any(Date) }`). `npm run typecheck` limpo · **1449 testes verdes**. **Não toca em auth.**
- **2026-09-04** — **§8.8.2** — **Treinos — filtros de categoria e subcategoria no seletor «Adicionar exercício da biblioteca».** O diálogo de adicionar exercício à sessão (`components/treinos/GestorExercicios.tsx`) ganha **dois novos filtros** a juntar aos existentes («Adicionar à fase» e «Filtrar por fase»): **«Filtrar por categoria»** (todos os valores do enum `CategoriaExercicioPrincipal` via `CATEGORIAS`/`LABEL_CATEGORIA` de `lib/schemas/subcategoria.ts`, com sentinela «Todas as categorias») e **«Filtrar por subcategoria»** (subcategorias customizáveis do clube, com sentinela «Todas as subcategorias»). **Só apresentação + leitura server-side aditiva — sem alteração de schema, migração, Server Actions, dados de negócio ou auth.** **(1)** As opções de subcategoria são **dinâmicas**: derivam das subcategorias efetivamente presentes nos exercícios já filtrados por **fase + categoria** (distintas, ordenadas por nome, `localeCompare` pt); quando a categoria muda, a subcategoria selecionada é **reposta em «todas»** (e, por segurança, um valor de subcategoria que deixe de existir não esvazia a lista). O select de subcategoria fica **desativado** quando não há subcategorias disponíveis. **(2)** A biblioteca listada é filtrada por **todos os critérios ativos em simultâneo** (fase ∧ categoria ∧ subcategoria); o estado vazio passa a uma mensagem genérica «Nenhum exercício corresponde aos filtros selecionados.» (antes só referia a fase). **(3)** Para alimentar o filtro, `app/(app)/treinos/[id]/page.tsx` chama `listarSubcategorias()` (`lib/actions/subcategorias.ts`) no `Promise.all` e passa a `GestorExercicios` a nova prop `subcategorias: {id, nome, categoria}[]`, além de incluir `subcategoriaId` em cada item da biblioteca (já vinha no `Exercicio`). A lógica de «Adicionar à fase» e da convocatória mantém-se inalterada. Os quatro filtros passam para uma grelha `grid-cols-1 sm:grid-cols-2` (2×2 no desktop), mantendo o estilo dos selects existentes. UI 100% pt-PT, alvos de toque ≥44px, sem dark mode, zero `any`. `npm run typecheck` limpo · `npm run lint` limpo · **1449 testes verdes**. **Não toca em auth.**
- **2026-09-04** — **§8.23.2 / §10.1** — **Correções de UI da vista analítica do atleta (tile de presenças, presença mensal duplicada, gráfico de exercícios por categoria).** Três correções **puramente de apresentação** na aba **Analytics** do perfil do atleta — **sem** alteração de schema, migração, Server Actions, dados de negócio ou auth. **(1) Tile de presenças com contagem — `components/analiticos/PainelAtleta.tsx` + `components/analiticos/CartaoKpi.tsx`:** o KPI «presenças» mostrava só a percentagem (ex.: «33%») sem indicar quantas presenças a suportam. Passa a mostrar **`X% (N/T)`** — N = `agregado.presencas` (sessões presentes), T = `agregado.sessoesTotais` — com a contagem a menor destaque (`text-base`, `text-cinza-400`). Para o permitir, a prop `valor` do `CartaoKpi` passou de `string | number` para **`ReactNode`** (superset — 100% retrocompatível, nenhum outro chamador afetado). **(2) «Taxa de presença por mês» duplicada — `components/analiticos/PainelTreinoAtleta.tsx`:** o gráfico mensal de presenças era desenhado **duas vezes** na mesma página — no `PainelAtleta` e outra vez na secção «Treino» aninhada (`PainelTreinoAtleta`). Removida a cópia do `PainelTreinoAtleta` (e o `GraficoBarrasV`/`pontosPresenca`/`temPresencaMensal` agora não usados); o gráfico passa a aparecer **só uma vez**, no `PainelAtleta` (alinhado com as notas «unificar»/«reutilizar existente» das linhas A/D de §8.23.2). **(3) «Exercícios por categoria» compacto — `components/analiticos/PainelTreinoAtleta.tsx`:** o `GraficoBarrasH` estava num cartão a **largura total**, e como escala pela largura do contentor (viewBox), ficava **desproporcionadamente alto** com barras enormes. Passa a viver numa **grelha de 2 colunas** (`grid gap-6 lg:grid-cols-2`) ao lado do gráfico de RPE — o mesmo padrão de `PainelEscalao`/`relatorios` —, ficando **compacto** e à mesma escala dos rankings dos outros painéis. O `GraficoBarrasH` é **partilhado** por vários painéis e **não** foi alterado (só o contentor). §8.23.2 atualizada com nota das três correções. `npm run typecheck` limpo · `npm run lint` limpo. **Não toca em auth.**
- **2026-09-04** — **§8.5** — **Simplificação do perfil do atleta — remoção da aba «Dados» e integração no formulário de edição.** A aba **«Dados»** do perfil do atleta (`app/(app)/plantel/[id]/page.tsx`) foi **removida**; as suas ações interativas migraram para o **formulário de edição** (`app/(app)/plantel/[id]/editar/page.tsx`), sem alteração de lógica de negócio, schema, Server Actions ou auth (só movimento de UI). **(1)** O switch de estado **«Atleta ativo»** (`ToggleAtivoAtleta`, mesmo label e descrição «Consta no plantel e aparece nas listas.»; chama `toggleAtivoAtleta`) passa a aparecer no ecrã de edição, num cartão após o formulário, **visível só com `PLANTEL_GERIR`** (o servidor mantém-se autoridade). **(2)** A **zona de perigo «Apagar definitivamente»** (`ApagarAtletaDefinitivamenteButton`, hard-delete RGPD — P1.3, mesmo aviso de irreversibilidade e diálogo de confirmação; chama `apagarAtletaDefinitivamente`) passa para o **fim do ecrã de edição**, visualmente separada com **borda vermelha** (`border-vermelho-600/40 bg-vermelho-600/5`), **visível só com `PLANTEL_GERIR`**. O botão **«Arquivar atleta»** (soft-delete) mantém-se no ecrã de edição como antes. A página de edição passa a chamar `obterMembroAtual()` para o gating de UI (`podeGerirPlantel`). Removidos do perfil os imports/funções agora não usados (`ToggleAtivoAtleta`, `ApagarAtletaDefinitivamenteButton`, `formatarData`); o gating `podeGerirPlantel` mantém-se no perfil por causa da aba **Participações**. UI 100% pt-PT, alvos de toque ≥44px. `npm run typecheck` limpo · **1448 testes verdes**. **Não toca em auth.**
- **2026-09-04** — **§11.5 / §12** — **Correção — fundo do campo (relva) aparecia laranja da marca em vez da cor do clube dentro de diálogos.** As miniaturas/campos SVG (`components/campo/*`) já pintam o fundo com `var(--cor-primaria, #F0531E)` (relva = cor do clube; fallback laranja Mister). Porém, o layout (`app/(app)/layout.tsx`) define `--cor-primaria`/`--cor-secundaria` **inline num `<div>` interno**, e os **portais do Radix** (Dialog, Select, DropdownMenu) montam em `document.body` — **fora** desse `<div>`. Consequência: qualquer campo renderizado **dentro de um diálogo** (ex.: «Adicionar exercício da biblioteca» em `components/treinos/GestorExercicios.tsx`; `components/treinos/ModalDiagramaExercicio.tsx`) **não herdava** a CSS custom property e caía no **fallback laranja**, mesmo com o clube em azul. **Correção (puramente visual — sem alteração de schema, migração, Server Actions, dados de negócio ou auth):** novo componente cliente `components/layout/TemaClubeGlobal.tsx` que **espelha `--cor-primaria`/`--cor-secundaria` no `:root` (`document.documentElement`)** via `useEffect` (com restauração no unmount), montado dentro do `<div>` temado do layout. Assim os portais **herdam** a cor do clube e o campo segue a cor primária dinâmica. **Não** toca em `--primary`/`--ring` (tema shadcn/botões), apenas nos acentos do clube. §11.5/§12 alinhadas. `npm run typecheck` limpo · `npm run lint` limpo · **1448 testes verdes**. **Não toca em auth.**
- **2026-09-04** — **§11.3** — **Correção — setas roubavam o clique de seleção no modo animação do editor de campo.** No **modo animação** (§11) do editor de campo (`components/campo/EditorCampo.tsx`), ao tentar selecionar uma **bola / cone / pessoa** para a animar, o clique selecionava **sempre a seta** por cima em vez do elemento-ponto pretendido. **Causa raiz:** as setas são desenhadas normalmente **depois** dos elementos que animam (índice mais alto no array) e o `elementoEmPonto` (`components/campo/animacao.ts`) percorre os elementos **de cima para baixo**, medindo a distância aos **segmentos** do trajecto com uma **faixa de toque larga** (`raioHit` ≥14 unidades) — pelo que uma seta que passa junto/sobre a bola vencia o hit-test. Somava-se a isto uma segunda via de seleção: a **faixa transparente de hit/foco** do trajecto (`hitPath` em `ElementoSVG`, `components/campo/desenho.tsx`), alvo de **foco de teclado**. **Correção (só editor/hit-test — sem alteração de schema, dados de negócio ou auth):** **(1)** `elementoEmPonto` passa a aceitar a opção `{ apenasPontos }` que **ignora setas/linhas** no hit-test; **(2)** o `EditorCampo` passa `apenasPontos: modoAnimacao` na seleção por ponteiro (case «selecionar» de `handlePointerDown`); **(3)** `ElementoSVG` ganha a prop `pathSelecionavel` (default `true`) que **desativa a faixa de hit/foco** do trajecto quando `false`, e o `EditorCampo` passa `pathSelecionavel={!modoAnimacao}` — eliminando também a seleção por foco de teclado das setas em animação. **Fora do modo animação**, setas e linhas mantêm-se totalmente selecionáveis, arrastáveis e apagáveis (comportamento inalterado). Novos testes de regressão em `tests/campo.test.ts` (bloco «apenasPontos (modo animação)»): sem a opção a seta por cima intercepta o clique; com a opção seleciona a bola; clique só sobre a seta não seleciona nada; elementos-ponto continuam selecionáveis pelo raio. §11.3 alinhada. `npm run typecheck` limpo · `npm run lint` limpo · testes verdes. **Não toca em auth.**
- **2026-09-04** — **§12.1** — **Tema escuro — polish da lista de treinos (cabeçalho de semana, sessões e badges).** Afinação de contraste/hierarquia da vista de **lista** de treinos (`app/(app)/treinos/page.tsx`) em **modo escuro**, sem qualquer alteração ao **modo claro** (permanece byte-idêntico) nem a schema, migração, dados de negócio ou auth. **Contexto técnico:** o tema escuro do produto não usa o prefixo `dark:` nos componentes — funciona por **remapeamentos não-encamadados** (`unlayered`) em `app/globals.css` (bloco pós-`@layer`), que vencem as utilidades `@layer utilities` do Tailwind. Consequência: classes com remap (`bg-white`, `text-cinza-*`, `border-cinza-*`, `divide-cinza-*`) **não** podem ser sobrepostas com `dark:` ao nível da página, mas variantes de opacidade e as classes `ambar`/`verde` (sem remap) **podem**. Bugs de dark mode corrigidos, todos **page-scoped**: **(1)** o **cabeçalho da semana** usava `bg-cinza-50/70` (variante de opacidade **sem** remap) → ficava **creme claro** em dark; passa a `dark:bg-[#24232c]` (barra elevada, distinta do corpo do cartão `#1c1b22`). **(2)** as **sessões concluídas** usavam `bg-cinza-50/40` (idem, sem remap) → wash claro; passam a `dark:bg-black/20` (linha «afundada», de-emphasis coerente com «concluído»). **(3)** os **badges «Por fechar»/«Sem exercícios»** usavam `text-ambar-600` (`#8A5A06`, castanho afinado para AA em fundo **claro**) → quase invisível em dark; ganham `dark:border-[#E0900A]/45 dark:bg-[#E0900A]/15 dark:text-[#F5B44A]` (âmbar claro legível). **(4)** o **badge «Fechado»** (`text-verde-600` `#1E9E5A`, contraste-limite) ganha `dark:border-[#1E9E5A]/45 dark:bg-[#1E9E5A]/15 dark:text-[#4FD98D]`. **(5)** os **separadores** entre sessões (`divide-cinza-100`, remapeado para `#232228` quase imperceptível — e não sobreponível por `dark:` devido ao remap não-encamadado) passam a valores **arbitrários** sem remap: `divide-[#EEEBE6] dark:divide-[#2c2b35]` (claro **idêntico** a `cinza-100`; escuro visível). **Zero regressão no modo claro** (todos os valores claros preservam os originais); **nenhum token partilhado alterado** → nenhuma outra página afetada. `npm run typecheck` limpo · `npm run lint` limpo. **Não toca em auth.**
...
- **2026-08-19** — **Criação da bíblia v7 (`Mister_Spec_v7.md`) — Mister passa a plataforma multi-desporto (futsal + futebol).** Novo ficheiro que sucede à `Mister_Spec_v6.md` (**mantida intacta como histórico**, à semelhança do que a v6 fez à v5). Atualização **só de documentação** (nenhuma alteração de código; **não toca em auth**). A v7 expande o produto de dedicado ao futsal para **multi-desporto**, mantendo **um único código, um único modelo de dados multi-tenant e a mesma filosofia**. Todas as decisões de produto abaixo estão **fechadas**.
  ...
  - **(I) Licenciamento multi-secção (§17):** Individual = **uma** modalidade (preço mantém-se €4,99/mês·€49/ano); Clube = **uma ou várias secções** (tier por total de escalões; **acréscimo por secção adicional** recomendado ≈ +50% do tier; enforcement de billing deferido). `Licenca.modalidade` regista a modalidade Individual.
...
Funciona igual em qualquer desporto. Inclui: contas/autenticação, clubes, membros, perfis, **épocas**, **presenças**, **comunicação (WhatsApp)**, **reuniões**, **lembretes**, **licenciamento/carteira**, **relatório partilhável** (contentor), **dashboard** (temporal), **agenda/conflitos de pavilhão**. **Regra:** esta camada **nunca** ramifica por modalidade.
...
- A **`Secção`** (§3.1.1) é a **única fonte de verdade da modalidade**. Tudo o que precisa de saber a modalidade sobe do `Escalao` → `Seccao.modalidade`. Não há campo `modalidade` disperso por atletas/jogos/exercícios (os campos `modalidade?` em exercício/template/métrica são **de organização/filtro**, não de derivação operacional).
...
8. **Biblioteca curada:** conteúdo de arranque do novo desporto (por formato/parte do treino).
...
**🔁 Isenção da guarda de licença em conta híbrida (Fase 5, 2026-09-11).** O admin de plataforma é **isento da guarda de licença** (§3.11) no shell `(app)` — a guarda passa a conceder acesso por **`eAdmin || licencaOk`**. Isto garante que o atalho **"Backoffice"** é **sempre alcançável** mesmo quando a licença do clube do admin híbrido está **`PENDENTE`/inválida** (concretiza o "sempre alcançável, mesmo em conta híbrida" acima — sem a isenção, o admin com licença pendente ficaria preso no paywall `/sem-licenca` antes de conseguir chegar ao `/admin`). A isenção afeta **apenas a guarda de billing** — **não** relaxa autorização nem isolamento multi-tenant: todas as actions cross-tenant do backoffice mantêm `exigirAdminPlataforma()`. Auth-core intocado.
...
- **Ativar** (`ativarLicenca`) → estado `PENDENTE → ATIVA`, após **confirmação de pagamento** (passo 4 do fluxo interino, §17.5)
- **Suspender** (`alterarEstadoLicenca`) → estado `SUSPENSA`
- **Cancelar** (`alterarEstadoLicenca`) → estado `CANCELADA`
...
**🔁 Ativação manual de pagamento (Fase 5, 2026-09-11).** A confirmação de pagamento tem uma **ação dedicada `ativarLicenca(licencaId)`** (`lib/actions/admin-licencas.ts`, gate `exigirAdminPlataforma()`), **distinta** de `alterarEstadoLicenca` (que mantém Suspender/Cancelar) — a separação isola o caminho crítico «PENDENTE→ATIVA após pagamento» das restantes transições de estado. Ao ativar, a ação:
- **Define a `dataFim` alinhada ao ciclo** quando ainda não existe: **MENSAL** → +1 mês, **ANUAL** → +1 ano a partir da ativação (licença válida até ao fim do ciclo pago). Se a `dataFim` já estiver definida, é preservada.
- **Notifica o titular por email** (template «conta ativada» / «A tua conta Mister está pronta», §17.5) em modo **best-effort** — a falha de envio **não desfaz** a ativação. O titular é resolvido nos **dois ramos do modelo polimórfico** (§3.11): **Individual** (`utilizadorId` → o próprio utilizador) e **Clube** (`clubeId` → o admin do clube, por capacidades efetivas).

**🔁 Apresentação de pendentes (Fase 5).** Os registos **`PENDENTE`** ganham **destaque visual** (borda esquerda + fundo âmbar suave, §12) e um **painel de contexto de confirmação de pagamento**, que reúne o necessário para conferir a transferência: **referência de pagamento** (nome do titular · email, conforme §17.5), **plano** (tipo · tier/modalidade · nº de secções), **valor por ciclo** (`precoCentimos`), datas e estado. O topo da página mostra a **contagem de pendentes** e a lista usa **ordenação PENDENTE-primeiro** (decisão de **apresentação** — a query não muda). O titular **Individual** apresenta **nome + email**.
...
- **Correção de golos de um confronto realizado:** `atualizarGolosConfronto(resultadoId, golosCasa, golosFora)` permite corrigir o placar de um confronto `REALIZADO` sem passar por remover/recriar. Só edita golos (0–99), **não altera o estado**; WALKOVER/CANCELADO não expõem edição de golos (usam resultado regulamentar / não têm placar). No UI, a `LinhaDecidido` do quadro ganha um botão «Golos» que abre edição inline (dois campos numéricos) para confrontos realizados.
- **Gestão de equipas no detalhe:** a página de detalhe (`/jogos/competicoes/[id]`) tem uma tab **«Equipas»** que lista os participantes, permite **adicionar** (nome livre → `adicionarEquipaCompeticao`), **remover** (`removerEquipaCompeticao`, com confirmação quando a equipa tem confrontos) e **gerar o quadro** (`gerarQuadroCompeticao`, visível quando o formato não é LIGA ou há equipas sem confrontos gerados). A equipa `PROPRIO` aparece destacada e **não é removível**.
...
atualizarGolosConfronto(resultadoId, golosCasa, golosFora)     // corrige placar de confronto REALIZADO (golos 0–99)
...
```
Schemas Zod em `lib/schemas/competicao.ts` (fonte única cliente/servidor); a lógica pura de tabela em `lib/classificacao.ts` (função testável isoladamente). Rota `/jogos/competicoes` (inalterada), acessível como **item de primeiro nível da navegação** (Competições, ícone `Trophy` — §8.13.1).
