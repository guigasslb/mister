...
1. **Arranque** — o treinador seleciona os **titulares** (o nº exato do formato — 5 em `FUTSAL_5`, 7/9/11 em futebol) e as respetivas **posições** (GR incluído), a partir da grelha de convocados; toca **«Iniciar Parte 1»** → o cronómetro arranca (`INICIO_PARTE`, `ENTRADA` de cada titular no segundo 0). O **nº de partes** já **não** se escolhe no arranque: é definido na **criação/edição do jogo** (`Jogo.numeroPartes`, 1..4) e a `SessaoJogoAoVivo` herda-o ao iniciar.
