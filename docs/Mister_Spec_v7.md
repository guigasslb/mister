...
A **Agenda** (`/agenda`) é a **vista central de eventos** do clube, unificando num só ecrã os três tipos de evento: **treinos** (sessões), **jogos** e **reuniões**. Substitui a antiga agenda condicional (dependente de flag de visibilidade) e passa a item primário da navegação (§8).
...
- **Export PDF dos analíticos («Dossier do Treinador»)** (2026-08-26): botão **«Download PDF»** nos painéis de **escalão** (estatística individual — tabela por atleta) e de **clube** (estatísticas gerais — KPIs, resultados e tabela por escalão), com a **identidade do clube** (logótipo, nome, época e cor primária; fallback laranja Mister). Gerado server-side com `@react-pdf/renderer` (`lib/pdf/gerar-pdf.tsx` + route handler `app/api/pdf`), reutilizando os mesmos dados de `obterAnaliticoEscalao`/`obterAnaliticoClubeEpoca` (auth + `RELATORIOS_VER` garantidos).
...
Conforme v6 §10.2: jogos/V/E/D, golos, taxa de presença média, melhores marcadores/assistentes (por `atletaId`), mais utilizados (blocos), distribuição de tipos de treino, rankings por métrica configurável, ranking de assiduidade (TOP 5), filtro por competição.
