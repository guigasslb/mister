 ...
 * campo abre a substituição (a troca inicia sempre pelo que sai — RN-JV-3); tocar
 * no banco não faz nada. Cada cartão mostra nome + número + tempo em campo.
 */
          ...
          {emCampo.map((a) => (
            <li key={a.id}>
              <button
                ...
                className="group flex min-h-[64px] w-full items-center gap-3 rounded-xl border border-white/15 bg-white/10 p-3 text-left transition active:scale-[0.98] enabled:hover:border-white/40 enabled:hover:bg-white/15 disabled:opacity-70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/60"
              ...
              >
                ...
                {interativo && (
                  <ArrowLeftRight
                    className="h-4 w-4 flex-shrink-0 text-white/40 transition group-hover:text-white/80"
                    aria-hidden
                  />
                )}
              </button>
